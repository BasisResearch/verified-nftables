...
Theorem optimize_rules_concat_correct : forall rs n d n' d' rs' base p,
  optimize_rules_concat n d rs = (n', d', rs') ->
  rules_clean rs = true ->
...
Proof.
  ...
  - rewrite optimize_rules_concat_cons2 in H. cbv zeta in H.
    destruct (concat_merge_pair r1 r2)
      as [[[[[[[f1 f2] a1] b1] a2] b2] body] |] eqn:Evm.
    + (* MERGE fires *)
      set (dn := {| sd_sets := (setname n, [(pack2 a1 b1, pack2 a1 b1);
                                            (pack2 a2 b2, pack2 a2 b2)]) :: sd_sets d;
                    sd_vmaps := sd_vmaps d; sd_maps := sd_maps d |}) in *.
      destruct (optimize_rules_concat (S n) dn rest) as [[m'' dd''] rr''] eqn:Erec.
      inversion H; subst n' d' rs'. clear H.
      ...
      destruct (concat_merge_pair_shape r1 r2 f1 f2 a1 b1 a2 b2 body Evm)
        as [Hr1 [Hr2 [Hfa1 [Hfa2 [Hfb1 Hfb2]]]]].
      set (qd  := set_env p (env_with_sets base dd'')) in *.
      (* certificate at qd for the merged head vs the two per-row conjunctions *)
      assert (Hcert : eval_matchcond (MConcatSet [f1; f2] false (setname n)) qd
              = orb (andb (eval_matchcond (MCmp f1 CEq a1) qd) (eval_matchcond (MCmp f2 CEq b1) qd))
                    (andb (eval_matchcond (MCmp f1 CEq a2) qd) (eval_matchcond (MCmp f2 CEq b2) qd))).
      { apply concat_two_fields_certificate.
        - exact Hlook.
        - intro Hld. apply (field_fixed_len_loaded f1 (length a1) qd Hfa1 Hld).
        - intro Hld. apply (field_fixed_len_loaded f2 (length b1) qd Hfb1 Hld).
        - intro Hld. apply (field_fixed_len_loaded f1 (length a2) qd Hfa2 Hld).
        - intro Hld. apply (field_fixed_len_loaded f2 (length b2) qd Hfb2 Hld). }
      ...
      (* Goal: eval_rules (merged :: rr'') qd = eval_rules (r1::r2::rest) @d.
         Step 1: collapse merged head -> the two per-row conjunction heads. *)
      transitivity (eval_rules (orig_rule2 f1 f2 a1 b1 body r1
                      :: orig_rule2 f1 f2 a2 b2 body r1 :: rr'') qd).
      { apply eval_rules_concat_merge2.
        - exact Hcert.
        - rewrite (concat_match_loadable_split f1 f2 (setname n) qd). reflexivity.
        - rewrite (concat_match_loadable_split f1 f2 (setname n) qd). reflexivity. }
      (* Step 2: tail rr'' -> rest (Htail'), then heads -> r1, r2 (Hr1,Hr2). *)
...
Theorem optimize_chain_concat_correct : forall n d c n' d' c' base p,
...
Proof.
  ...
  destruct (optimize_rules_concat n d (c_rules c)) as [[m'' dd''] rr''] eqn:Erec.
  inversion H; subst n' d' c'. cbn [c_rules c_policy].
  unfold eval_chain. cbn [c_rules c_policy].
  rewrite (optimize_rules_concat_correct (c_rules c) n d m'' dd'' rr'' base p Erec Hclean Hfresh).
  reflexivity.
Qed.
...
Theorem optimize_rules_concatN_correct : forall fuel rs n d n' d' rs' base p,
  optimize_rules_concatN fuel n d rs = (n', d', rs') ->
  rules_clean rs = true ->
  (forall k, n <= k -> ~ In (setname k) (map fst (sd_sets d))) ->
  eval_rules rs' (set_env p (env_with_sets base d'))
      ...
      * destruct (take_concat_run r1 (r2 :: rest)) as [ts rest'] eqn:Erun.
        ...
        -- cbv zeta in H.
           ...
           set (dn := {| sd_sets := (setname n, map pack_tuple tuples) :: sd_sets d;
                         sd_vmaps := sd_vmaps d; sd_maps := sd_maps d |}) in *.
           (* r1 :: (run) = the orig_rule2 shells over tuples *)
           ...
           { cbn [set_env pkt_env]. rewrite e_set_declared.
             ...
             - intros k Hk Heq. apply setname_inj in Heq. lia. }
           set (qd := set_env p (env_with_sets base dd'')) in *.
           (* the merged head = existsb over the run of the per-row conjunctions *)
           assert (Hcert : eval_matchcond (MConcatSet [f1; f2] false (setname n)) qd
           ...
           { apply (concat_two_fields_certificate_N f1 f2 tuples (setname n) qd).
             - exact Hlook.
             - intros a b Hin Hld.
               assert (Hfx : field_fixed_len f1 = Some (length a)).
               { destruct (take_concat_run_head_width r1 f1 a1 f2 b1 body r2 rest
                 ...
                 - apply (HwA a b Hin). }
               apply (field_fixed_len_loaded f1 (length a) qd Hfx Hld).
             - intros a b Hin Hld.
               assert (Hfx : field_fixed_len f2 = Some (length b)).
               { destruct (take_concat_run_head_width r1 f1 a1 f2 b1 body r2 rest
                             (t :: ts') rest' Ehd Erun ltac:(discriminate)) as [_ Hh2].
                 subst tuples. destruct Hin as [Hab | Hin].
                 - injection Hab as -> ->. exact Hh2.
                 - apply (HwB a b Hin). }
               apply (field_fixed_len_loaded f2 (length b) qd Hfx Hld). }
           (* collapse the merged rule into the whole run *)
           ...
           { apply (eval_rules_run_collapse
             ...
             - rewrite merged_rule2_applies. rewrite Hcert.
               ...
               (* RHS: existsb (rule_applies orig) = existsb (conj && walk)
                       = (existsb conj) && walk = LHS *)
...
Theorem optimize_chain_concatN_correct : forall n d c n' d' c' base p,
...
Proof.
  ...
  destruct (optimize_rules_concatN (length (c_rules c)) n d (c_rules c))
    as [[m'' dd''] rr''] eqn:Erec.
  inversion H; subst n' d' c'. cbn [c_rules c_policy].
  unfold eval_chain. cbn [c_rules c_policy].
  rewrite (optimize_rules_concatN_correct (length (c_rules c)) (c_rules c) n d
