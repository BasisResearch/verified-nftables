(** * Optimize_Normalize: a verdict-preserving head normalisation that lets the
    ...
    The merge recognisers ([Optimize_Merge.head_value], [Optimize_Concat.head_value2],
    ...
    (both [data_eqb (firstn (length v) (field_value f p)) v]; see [eval_cmp]'s [CEq]
...
Lemma normalize_mc_eval : forall m p,
  eval_matchcond_body (normalize_mc m) p = eval_matchcond_body m p.
...
Lemma normalize_mc_matchcond : forall m p,
  eval_matchcond (normalize_mc m) p = eval_matchcond m p.
Proof.
  intros m p. unfold eval_matchcond.
...
Lemma normalize_rule_applies_walk : forall body p,
  rule_applies_walk (map normalize_bi body) p = rule_applies_walk body p.
Proof.
  induction body as [| it b IH]; intro p; [reflexivity|].
...
Lemma normalize_end_loadable : forall r p,
  end_loadable (normalize_rule r) p = end_loadable r p.
...
Lemma normalize_rule_loadable : forall r p,
  rule_loadable (normalize_rule r) p = rule_loadable r p.
Proof.
  intros r p. unfold rule_loadable. rewrite normalize_r_body.
...
Lemma normalize_rule_applies : forall r p,
  rule_applies (normalize_rule r) p = rule_applies r p.
Proof.
  intros r p. unfold rule_applies. rewrite normalize_r_body.
...
Lemma normalize_outcome_core : forall r p,
  outcome_core (normalize_rule r) p = outcome_core r p.
Proof. intros r p. reflexivity. Qed.

Lemma normalize_outcome : forall r p,
  outcome (normalize_rule r) p = outcome r p.
Proof.
  intros r p. unfold outcome. rewrite normalize_r_body.
...
Lemma normalize_eval_rules : forall rs p,
  eval_rules (map normalize_rule rs) p = eval_rules rs p.
Proof.
  induction rs as [| r rs IH]; intro p; [reflexivity|].
  cbn [map eval_rules].
  rewrite normalize_rule_loadable, normalize_rule_applies, normalize_outcome.
  destruct (rule_loadable r p && rule_applies r p); [| apply IH].
  destruct (outcome r p) as [v|]; [destruct v|]; rewrite ?IH; reflexivity.
Qed.

Theorem normalize_chain_eval : forall c p,
  eval_chain (normalize_chain c) p = eval_chain c p.
Proof.
  intros c p. unfold eval_chain, normalize_chain. cbn [c_rules c_policy].
