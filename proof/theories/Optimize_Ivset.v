(** * Optimize_Ivset: INTERVAL / range set consolidation (nft -o's range merge).
    ...
    prefix equality).  We reuse [eval_rules_run_merge_abs] (Optimize_Merge) VERBATIM.
    Axiom-free. *)
...
Fixpoint optimize_rules_ivset (fuel : nat) (n : nat) (d : set_decls) (rs : list rule)
  ...
  | S fuel' =>
    ...
    | r1 :: ((_ :: _) as rest) =>
        ...
        | Some (f, lo1, hi1, body) =>
            ...
            | ([], _) =>
                let '(n'', d'', rest') := optimize_rules_ivset fuel' n d rest in
                (n'', d'', r1 :: rest')
            | ((_ :: _) as ivs, rest') =>
                ...
                let '(n'', d'', rest'') := optimize_rules_ivset fuel' (S n) d' rest' in
                (n'', d'', merged :: rest'')
            end
        | None =>
            let '(n'', d'', rest') := optimize_rules_ivset fuel' n d rest in
...
Definition optimize_chain_ivset (n : nat) (d : set_decls) (c : chain)
  : nat * set_decls * chain :=
  let '(n', d', rs') := optimize_rules_ivset (length (c_rules c)) n d (c_rules c) in
  (n', d', {| c_policy := c_policy c; c_rules := rs' |}).

Lemma optimize_rules_ivset_consSS : forall fuel n d r1 r2 rest,
  optimize_rules_ivset (S fuel) n d (r1 :: r2 :: rest) =
  match head_range r1 with
  | Some (f, lo1, hi1, body) =>
      match take_range_run r1 (r2 :: rest) with
      | ([], _) =>
          let '(n'', d'', rest') := optimize_rules_ivset fuel n d (r2 :: rest) in
          (n'', d'', r1 :: rest')
      | ((_ :: _) as ivs, rest') =>
          let name := setname n in
          let elems := (lo1, hi1) :: ivs in
...
(** *** Structural invariants (mirror concatN/concatM: mints [setname]s only). *)
Lemma optimize_rules_ivset_mono : forall fuel n d rs n' d' rs',
  optimize_rules_ivset fuel n d rs = (n', d', rs') -> n <= n'.
Proof.
  ...
  - destruct rs as [| r1 [| r2 rest] ].
    ...
    + rewrite optimize_rules_ivset_consSS in H.
      ...
      * destruct (take_range_run r1 (r2 :: rest)) as [ivs rest'] eqn:Erun.
        ...
        -- remember (optimize_rules_ivset fuel n d (r2 :: rest)) as tt eqn:Erec.
           destruct tt as [[m'' dd''] rr'']. cbv zeta in H. inversion H; subst.
           eapply (IH n d (r2 :: rest)); symmetry; exact Erec.
        -- cbv zeta in H.
           remember (optimize_rules_ivset fuel (S n) _ rest') as tt eqn:Erec.
...
Lemma optimize_rules_ivset_vmaps : forall fuel n d rs n' d' rs',
  optimize_rules_ivset fuel n d rs = (n', d', rs') -> sd_vmaps d' = sd_vmaps d.
Proof.
  induction fuel as [| fuel IH]; intros n d rs n' d' rs' H.
  - cbn in H. inversion H; subst; reflexivity.
  - destruct rs as [| r1 [| r2 rest] ].
...
Lemma optimize_rules_ivset_maps : forall fuel n d rs n' d' rs',
  optimize_rules_ivset fuel n d rs = (n', d', rs') -> sd_maps d' = sd_maps d.
Proof.
  induction fuel as [| fuel IH]; intros n d rs n' d' rs' H.
  - cbn in H. inversion H; subst; reflexivity.
  - destruct rs as [| r1 [| r2 rest] ].
...
Lemma optimize_rules_ivset_keys_bound : forall fuel n d rs n' d' rs' k,
        ...
        -- cbv zeta in H.
           ...
           ++ cbn [sd_sets map] in Hin_dn. destruct Hin_dn as [Heq | Hin_d].
              ** apply setname_inj in Heq. subst k. right.
                 pose proof (optimize_rules_ivset_mono fuel (S n) _ rest' m'' dd'' rr'' (eq_sym Erec)). lia.
              ** left; exact Hin_d.
           ++ right; exact Hlt.
      * remember (optimize_rules_ivset fuel n d (r2 :: rest)) as tt eqn:Erec.
        destruct tt as [[m'' dd''] rr'']. cbv zeta in H. injection H as Hn' Hd' Hr'.
...
Lemma optimize_rules_ivset_assoc_stable : forall fuel n d rs n' d' rs' nm X,
  optimize_rules_ivset fuel n d rs = (n', d', rs') ->
...
Lemma optimize_chain_ivset_mono : forall n d c n' d' c',
  optimize_chain_ivset n d c = (n', d', c') -> n <= n'.
Proof.
  intros n d c n' d' c' H. unfold optimize_chain_ivset in H.
  destruct (optimize_rules_ivset (length (c_rules c)) n d (c_rules c))
    as [[m'' dd''] rr''] eqn:E.
  inversion H; subst. apply (optimize_rules_ivset_mono _ _ _ _ _ _ _ E).
Qed.

Lemma optimize_chain_ivset_vmaps : forall n d c n' d' c',
  optimize_chain_ivset n d c = (n', d', c') -> sd_vmaps d' = sd_vmaps d.
Proof.
  intros n d c n' d' c' H. unfold optimize_chain_ivset in H.
  destruct (optimize_rules_ivset (length (c_rules c)) n d (c_rules c))
    as [[m'' dd''] rr''] eqn:E.
  inversion H; subst. apply (optimize_rules_ivset_vmaps _ _ _ _ _ _ _ E).
Qed.

Lemma optimize_chain_ivset_maps : forall n d c n' d' c',
  optimize_chain_ivset n d c = (n', d', c') -> sd_maps d' = sd_maps d.
Proof.
  intros n d c n' d' c' H. unfold optimize_chain_ivset in H.
  destruct (optimize_rules_ivset (length (c_rules c)) n d (c_rules c))
    as [[m'' dd''] rr''] eqn:E.
  inversion H; subst. apply (optimize_rules_ivset_maps _ _ _ _ _ _ _ E).
Qed.

Lemma optimize_chain_ivset_keys_bound : forall n d c n' d' c' k,
  optimize_chain_ivset n d c = (n', d', c') ->
  In (setname k) (map fst (sd_sets d')) ->
  In (setname k) (map fst (sd_sets d)) \/ k < n'.
Proof.
  intros n d c n' d' c' k H Hin. unfold optimize_chain_ivset in H.
  destruct (optimize_rules_ivset (length (c_rules c)) n d (c_rules c))
    as [[m'' dd''] rr''] eqn:E.
  inversion H; subst n' d' c'.
  apply (optimize_rules_ivset_keys_bound _ _ _ _ _ _ _ k E Hin).
Qed.
