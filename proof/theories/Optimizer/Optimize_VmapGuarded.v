(** * Optimize_VmapGuarded: the VERDICT-MAP merge over a single selector GUARDED by an
    implicit meta match (`tcp dport vmap { 22:drop, 80:accept, 443:drop }`).

    This is the vmap sibling of [Optimize_SetGuarded] (which builds a value->SET for a run
    of same-verdict guarded rules) and the guarded analogue of [Optimize_Vmap]'s
    [optimize_rules_vmap] (which folds an UNGUARDED single-selector run of
    ...
    unguarded [optimize_rules_vmap]) sees the GUARD as its head selector, whose
...
From Nft Require Import Bytes Packet Verdict Syntax Bytecode Semantics
  Optimize Optimize_ValueSet Optimize_Concat Optimize_ConcatGuarded Optimize_SetGuarded Optimize_Vmap.
Import ListNotations.
Local Open Scope nat_scope.

(** [guard_okn gm]: the NETWORK-LAYER counterpart of [Optimize_ConcatGuarded.guard_ok]
    (l4proto) and [Optimize_SetGuarded.guard_okl2] (iiftype).  A bare network-address
    ...
    address cmp, exactly the [Optimize_VmapGuarded] guarded single-selector shape.  Admitting
    ...
    implicit `meta iiftype == ARPHRD_ETHER (1)` dependency ([Optimize_SetGuarded.guard_okl2],
...
(** The guarded original: a pure-terminal rule [mk_vmap_base w] behind the two head
    matches [GUARD] then [MCmp f CEq v].  This is [Optimize_SetGuarded.orig_ruleGs] with
...
(** ** Executable N-WAY guarded vmap pass (mirrors [optimize_rules_vmap]). *)
...
Fixpoint optimize_rules_vmapguarded (fuel n : nat) (d : set_decls) (rs : list rule)
        ...
        | Some (gm, f, v1, body) =>
            ...
            | ((_ :: _) as es, rest') =>
                if has_distinct_verdict (r_verdict r1) es && body_vmap_safe body then
                  ...
                  let '(n'', d'', rest'') := optimize_rules_vmapguarded fuel' (S n) d' rest' in
                  (n'', d'', merged :: rest'')
                else
                  let '(n'', d'', rest') := optimize_rules_vmapguarded fuel' n d rest in
                  (n'', d'', r1 :: rest')
            | ([], _) =>
                let '(n'', d'', rest') := optimize_rules_vmapguarded fuel' n d rest in
                (n'', d'', r1 :: rest')
...
Definition optimize_chain_vmapguarded (n : nat) (d : set_decls) (c : chain)
  : nat * set_decls * chain :=
  let '(n', d', rs') := optimize_rules_vmapguarded (length (c_rules c)) n d (c_rules c) in
  (n', d', {| c_policy := c_policy c; c_rules := rs' |}).

Lemma optimize_rules_vmapguarded_consSS : forall fuel n d r1 r2 rest,
  optimize_rules_vmapguarded (S fuel) n d (r1 :: r2 :: rest) =
  match head_valueGs r1 with
  | Some (gm, f, v1, body) =>
      ...
      | ((_ :: _) as es, rest') =>
          if has_distinct_verdict (r_verdict r1) es && body_vmap_safe body then
            ...
            let '(n'', d'', rest') := optimize_rules_vmapguarded fuel n d (r2 :: rest) in
            (n'', d'', r1 :: rest')
      | ([], _) =>
          let '(n'', d'', rest') := optimize_rules_vmapguarded fuel n d (r2 :: rest) in
          (n'', d'', r1 :: rest')
...
Lemma optimize_rules_vmapguarded_assoc_stable : forall fuel n d rs n' d' rs' nm X,
