(** * Optimize_ConcatMulti: the N-FIELD ([K>=1]) concatenation-set membership certificate
...
From Nft Require Import Bytes Packet Verdict Syntax Bytecode Semantics Optimize Optimize_ValueSet Optimize_Concat.
...
Fixpoint optimize_rules_concatmulti (n : nat) (d : set_decls) (rs : list rule)
  ...
  | r1 :: ((r2 :: rest) as tl) =>
      ...
      | Some (fields, row1, row2, body) =>
          ...
          let '(n'', d'', rest') := optimize_rules_concatmulti (S n) d' rest in
          (n'', d'', merged :: rest')
      | None =>
          let '(n'', d'', tl') := optimize_rules_concatmulti n d tl in
          (n'', d'', r1 :: tl')
      end
  | _ => (n, d, rs)
  end.

Lemma optimize_rules_concatmulti_cons2 : forall n d r1 r2 rest,
  optimize_rules_concatmulti n d (r1 :: r2 :: rest) =
  match concat_mergeK_pair r1 r2 with
  | Some (fields, row1, row2, body) =>
      let name := setname n in
      let d' := {| sd_sets := (name, map pack_row [row1; row2]) :: sd_sets d;
      ...
      (n'', d'', merged :: rest')
  | None =>
      let '(n'', d'', tl') := optimize_rules_concatmulti n d (r2 :: rest) in
      (n'', d'', r1 :: tl')
  end.
Proof. reflexivity. Qed.

Definition optimize_chain_concatmulti (n : nat) (d : set_decls) (c : chain)
  : nat * set_decls * chain :=
  let '(n', d', rs') := optimize_rules_concatmulti n d (c_rules c) in
...
Lemma optimize_rules_concatmulti_assoc_stable : forall rs n d n' d' rs' nm X,
...
Proof.
  ...
  - cbn in H. inversion H; subst; reflexivity.
  - cbn in H. inversion H; subst; reflexivity.
  - rewrite optimize_rules_concatmulti_cons2 in H. cbv zeta in H.
    destruct (concat_mergeK_pair r1 r2) as [[[[fields row1] row2] body] |] eqn:Em.
    + destruct (optimize_rules_concatmulti (S n)
    ...
    + destruct (optimize_rules_concatmulti n d (r2 :: rest)) as [[m'' dd''] rr''] eqn:Erec.
      inversion H. subst n' d' rs'. clear H.
      eapply (H0 (r2 :: rest)); [ unfold ltof; cbn; lia | exact Erec | exact Hnm ].
Qed.

Lemma optimize_rules_concatmulti_vmaps : forall rs n d n' d' rs',
  optimize_rules_concatmulti n d rs = (n', d', rs') -> sd_vmaps d' = sd_vmaps d.
Proof.
  induction rs as [rs H0] using (induction_ltof1 _ (@length rule)).
  intros n d n' d' rs' H.
  destruct rs as [| r1 [| r2 rest] ].
  ...
  - rewrite optimize_rules_concatmulti_cons2 in H. cbv zeta in H.
    destruct (concat_mergeK_pair r1 r2) as [[[[fields row1] row2] body] |] eqn:Em.
    + destruct (optimize_rules_concatmulti (S n) _ rest) as [[m'' dd''] rr''] eqn:Erec.
      inversion H; subst n' d' rs'. clear H.
...
Lemma optimize_rules_concatmulti_maps : forall rs n d n' d' rs',
  optimize_rules_concatmulti n d rs = (n', d', rs') -> sd_maps d' = sd_maps d.
Proof.
  induction rs as [rs H0] using (induction_ltof1 _ (@length rule)).
  intros n d n' d' rs' H.
  destruct rs as [| r1 [| r2 rest] ].
...
Lemma optimize_rules_concatmulti_mono : forall rs n d n' d' rs',
  optimize_rules_concatmulti n d rs = (n', d', rs') -> n <= n'.
Proof.
  induction rs as [rs H0] using (induction_ltof1 _ (@length rule)).
  intros n d n' d' rs' H.
  destruct rs as [| r1 [| r2 rest] ].
...
Lemma optimize_rules_concatmulti_keys_bound : forall rs n d n' d' rs' k,
  optimize_rules_concatmulti n d rs = (n', d', rs') ->
    ...
    + remember (optimize_rules_concatmulti (S n)
      ...
      * cbn [sd_sets map] in Hin_dn. destruct Hin_dn as [Heq | Hin_d].
        -- apply setname_inj in Heq. subst k. right.
           pose proof (optimize_rules_concatmulti_mono rest (S n) _ m'' dd'' rr'' (eq_sym Erec)). lia.
        -- left; exact Hin_d.
      * right; exact Hlt.
...
Lemma optimize_chain_concatmulti_mono : forall n d c n' d' c',
  optimize_chain_concatmulti n d c = (n', d', c') -> n <= n'.
Proof.
  intros n d c n' d' c' H. unfold optimize_chain_concatmulti in H.
  destruct (optimize_rules_concatmulti n d (c_rules c)) as [[m'' dd''] rr''] eqn:E.
  inversion H; subst. apply (optimize_rules_concatmulti_mono _ _ _ _ _ _ E).
Qed.

Lemma optimize_chain_concatmulti_vmaps : forall n d c n' d' c',
  optimize_chain_concatmulti n d c = (n', d', c') -> sd_vmaps d' = sd_vmaps d.
Proof.
  intros n d c n' d' c' H. unfold optimize_chain_concatmulti in H.
  destruct (optimize_rules_concatmulti n d (c_rules c)) as [[m'' dd''] rr''] eqn:E.
  inversion H; subst. apply (optimize_rules_concatmulti_vmaps _ _ _ _ _ _ E).
Qed.

Lemma optimize_chain_concatmulti_maps : forall n d c n' d' c',
  optimize_chain_concatmulti n d c = (n', d', c') -> sd_maps d' = sd_maps d.
Proof.
  intros n d c n' d' c' H. unfold optimize_chain_concatmulti in H.
  destruct (optimize_rules_concatmulti n d (c_rules c)) as [[m'' dd''] rr''] eqn:E.
  inversion H; subst. apply (optimize_rules_concatmulti_maps _ _ _ _ _ _ E).
Qed.

Lemma optimize_chain_concatmulti_keys_bound : forall n d c n' d' c' k,
  optimize_chain_concatmulti n d c = (n', d', c') ->
  In (setname k) (map fst (sd_sets d')) ->
  In (setname k) (map fst (sd_sets d)) \/ k < n'.
Proof.
  intros n d c n' d' c' k H Hin. unfold optimize_chain_concatmulti in H.
  destruct (optimize_rules_concatmulti n d (c_rules c)) as [[m'' dd''] rr''] eqn:E.
  inversion H; subst n' d' c'.
  apply (optimize_rules_concatmulti_keys_bound _ _ _ _ _ _ k E Hin).
Qed.
