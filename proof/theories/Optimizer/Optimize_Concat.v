(** * Optimize_Concat: the [nft -o] CONCATENATION-SET merge, as an executable
    ...
    of the value->anonymous-SET merge ([optimize_rules_sets], [Optimize_ValueSet]) and
    the VERDICT-MAP merge ([optimize_rules_vmap2], [Optimize_Vmap]).  [nft -o]
...
Lemma concat_merge_pair_mutfree : forall r1 r2 x,
  concat_merge_pair r1 r2 = Some x -> rule_mutfree r1 = true.
Proof.
  intros r1 r2 x H. unfold concat_merge_pair in H.
...
(** ** The executable concat-set merge pass.
    ...
    tail and verdict. *)
Fixpoint optimize_rules_concat2 (n : nat) (d : set_decls) (rs : list rule)
  ...
  | r1 :: ((r2 :: rest) as tl) =>
      ...
      | Some (f1, f2, a1, b1, a2, b2, body) =>
          ...
          let '(n'', d'', rest') := optimize_rules_concat2 (S n) d' rest in
          (n'', d'', merged :: rest')
      | None =>
          let '(n'', d'', tl') := optimize_rules_concat2 n d tl in
          (n'', d'', r1 :: tl')
      end
  | _ => (n, d, rs)
  end.

Lemma optimize_rules_concat2_cons2 : forall n d r1 r2 rest,
  optimize_rules_concat2 n d (r1 :: r2 :: rest) =
  match concat_merge_pair r1 r2 with
  | Some (f1, f2, a1, b1, a2, b2, body) =>
      let name := setname n in
      let d' := {| sd_sets := (name, [(pack2 a1 b1, pack2 a1 b1);
      ...
      (n'', d'', merged :: rest')
  | None =>
      let '(n'', d'', tl') := optimize_rules_concat2 n d (r2 :: rest) in
      (n'', d'', r1 :: tl')
  end.
Proof. reflexivity. Qed.

Definition optimize_chain_concat2 (n : nat) (d : set_decls) (c : chain)
  : nat * set_decls * chain :=
  let '(n', d', rs') := optimize_rules_concat2 n d (c_rules c) in
...
Lemma optimize_rules_concat2_assoc_stable : forall rs n d n' d' rs' nm X,
  optimize_rules_concat2 n d rs = (n', d', rs') ->
...
Proof.
  ...
  - cbn in H. inversion H; subst; reflexivity.
  - rewrite optimize_rules_concat2_cons2 in H. cbv zeta in H.
    destruct (concat_merge_pair r1 r2)
      as [[[[[[[f1 f2] a1] b1] a2] b2] body] |] eqn:Evm.
    + destruct (optimize_rules_concat2 (S n)
    ...
    + destruct (optimize_rules_concat2 n d (r2 :: rest)) as [[m'' dd''] rr''] eqn:Erec.
      inversion H. subst n' d' rs'. clear H.
...
(** ** Executable N-WAY concat pass.
    ...
    and the leftover; [optimize_rules_concat] folds the whole run [r1 :: matched]
...
Fixpoint optimize_rules_concat (fuel n : nat) (d : set_decls) (rs : list rule)
  : nat * set_decls * list rule :=
  match fuel with
  | O => (n, d, rs)
  | S fuel' =>
    ...
    | r1 :: ((_ :: _) as rest) =>
        ...
        | Some (f1, a1, f2, b1, body) =>
            ...
            | ([], _) =>
                let '(n'', d'', rest') := optimize_rules_concat fuel' n d rest in
                (n'', d'', r1 :: rest')
            | ((_ :: _) as ts, rest') =>
                let name := setname n in
                ...
                let d' := {| sd_sets := (name, map pack_tuple tuples) :: sd_sets d;
                             sd_vmaps := sd_vmaps d; sd_maps := sd_maps d |} in
                let merged := merged_rule2 f1 f2 name body r1 in
                let '(n'', d'', rest'') := optimize_rules_concat fuel' (S n) d' rest' in
                (n'', d'', merged :: rest'')
            end
        | None =>
            let '(n'', d'', rest') := optimize_rules_concat fuel' n d rest in
...
Definition optimize_chain_concat (n : nat) (d : set_decls) (c : chain)
  : nat * set_decls * chain :=
  let '(n', d', rs') := optimize_rules_concat (length (c_rules c)) n d (c_rules c) in
  (n', d', {| c_policy := c_policy c; c_rules := rs' |}).

Lemma optimize_rules_concat_consSS : forall fuel n d r1 r2 rest,
  optimize_rules_concat (S fuel) n d (r1 :: r2 :: rest) =
  match head_value2 r1 with
  | Some (f1, a1, f2, b1, body) =>
      match take_concat_run r1 (r2 :: rest) with
      | ([], _) =>
          let '(n'', d'', rest') := optimize_rules_concat fuel n d (r2 :: rest) in
          (n'', d'', r1 :: rest')
      | ((_ :: _) as ts, rest') =>
          let name := setname n in
          let tuples := (a1, b1) :: ts in
...
Lemma optimize_rules_concat_assoc_stable : forall fuel n d rs n' d' rs' nm X,
