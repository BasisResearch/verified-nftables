(** * Optimize_DataMap: a verified DATA-VALUE-MAP consolidation pass (the first to
...
Proof.
  intros f v M k e p Hfx Hld. unfold body_writes; cbn [orig_map_rule r_body body_res_state body_step match_consume].
  rewrite (eval_mcmp_point f v e p Hld (field_fixed_len_loaded f (List.length v) e p Hfx Hld)).
  destruct (data_eqb (field_value f e p) v); [unfold body_writes; cbn [body_res_state body_step vsrc_loadable eval_vsrc match_consume] | reflexivity].
  reflexivity.
Qed.
...
Proof.
  ...
  unfold body_writes; cbn [mk_map_rule r_body body_res_state body_step match_consume].
  ...
  destruct (data_eqb v1 (field_value f e p) || data_eqb v2 (field_value f e p));
    [| reflexivity].
  unfold body_writes; cbn [body_res_state body_step vsrc_loadable fields_loadable forallb match_consume].
...
Proof.
  ...
  - (* field does NOT load: every head match fails, so no rule writes; all sides = p *)
    ...
    assert (Hmerged_p : body_writes (r_body (mk_map_rule f setname mapname k)) e p = (e, p))
      by (unfold body_writes; cbn [mk_map_rule r_body body_res_state body_step match_consume]; rewrite Hmcc; reflexivity).
...
(** Recognise a rule as EXACTLY the ORIGINAL shell (body AND end fields), using only
    ...
    and a ~42 MB blow-up; see [Optimize_ValueSet.rule_end_eqb].  Matching the record
...
Fixpoint optimize_rules_datamap (n : nat) (d : set_decls) (rs : list rule)
  ...
  | r1 :: ((r2 :: rest) as tl) =>
      ...
      | Some (f, v1, v2, M1, M2, k) =>
          ...
          let '(n'', d'', rest') := optimize_rules_datamap (S n) d' rest in
          (n'', d'', merged :: rest')
      | None =>
          let '(n'', d'', tl') := optimize_rules_datamap n d tl in
          (n'', d'', r1 :: tl')
      end
  | _ => (n, d, rs)
  end.

Lemma optimize_rules_datamap_cons2 : forall n d r1 r2 rest,
  optimize_rules_datamap n d (r1 :: r2 :: rest) =
  match map_merge_pair r1 r2 with
  | Some (f, v1, v2, M1, M2, k) =>
      let d' := {| sd_sets := (setname n, map2_set v1 v2) :: sd_sets d;
                   sd_vmaps := sd_vmaps d;
      ...
      (n'', d'', merged :: rest')
  | None =>
      let '(n'', d'', tl') := optimize_rules_datamap n d (r2 :: rest) in
      (n'', d'', r1 :: tl')
  end.
...
Lemma optimize_rules_datamap_mono : forall rs n d n' d' rs',
  optimize_rules_datamap n d rs = (n', d', rs') -> n <= n'.
Proof.
...
Lemma optimize_rules_datamap_vmaps : forall rs n d n' d' rs',
  optimize_rules_datamap n d rs = (n', d', rs') -> sd_vmaps d' = sd_vmaps d.
Proof.
  induction rs as [rs IH] using (induction_ltof1 _ (@List.length rule)).
  intros n d n' d' rs' H. destruct rs as [| r1 [| r2 rest]].
...
Lemma optimize_rules_datamap_assoc_stable : forall rs n d n' d' rs' nm X,
...
Lemma optimize_rules_datamap_maps_assoc_stable : forall rs n d n' d' rs' nm X,
  optimize_rules_datamap n d rs = (n', d', rs') ->
...
Lemma optimize_rules_datamap_keys_bound : forall rs n d n' d' rs' j,
  optimize_rules_datamap n d rs = (n', d', rs') ->
    ...
    + remember (optimize_rules_datamap (S n)
      ...
      * cbn [sd_sets map] in Hd0. destruct Hd0 as [Heq|Hd0].
        -- apply setname_inj in Heq. subst j. right.
           pose proof (optimize_rules_datamap_mono rest (S n) _ _ _ _ (eq_sym E)). lia.
        -- left; exact Hd0.
      * right; exact Hlt.
    + remember (optimize_rules_datamap n d (r2 :: rest)) as t eqn:E.
      destruct t as [[m'' dd''] rr'']. injection H as Hn Hd Hr; subst.
...
Definition optimize_chain_datamap (n : nat) (d : set_decls) (c : chain)
  : nat * set_decls * chain :=
  let '(n', d', rs') := optimize_rules_datamap n d (c_rules c) in
  (n', d', {| c_policy := c_policy c; c_rules := rs' |}).

Lemma optimize_chain_datamap_mono : forall n d c n' d' c',
  optimize_chain_datamap n d c = (n', d', c') -> n <= n'.
Proof.
  intros n d c n' d' c' H. unfold optimize_chain_datamap in H.
  destruct (optimize_rules_datamap n d (c_rules c)) as [[m d0] r0] eqn:E.
  inversion H; subst. exact (optimize_rules_datamap_mono _ _ _ _ _ _ E).
Qed.
Lemma optimize_chain_datamap_vmaps : forall n d c n' d' c',
  optimize_chain_datamap n d c = (n', d', c') -> sd_vmaps d' = sd_vmaps d.
Proof.
  intros n d c n' d' c' H. unfold optimize_chain_datamap in H.
  destruct (optimize_rules_datamap n d (c_rules c)) as [[m d0] r0] eqn:E.
  inversion H; subst. exact (optimize_rules_datamap_vmaps _ _ _ _ _ _ E).
Qed.
Lemma optimize_chain_datamap_keys_bound : forall n d c n' d' c' j,
  optimize_chain_datamap n d c = (n', d', c') ->
  In (setname j) (map fst (sd_sets d')) ->
  In (setname j) (map fst (sd_sets d)) \/ j < n'.
Proof.
  intros n d c n' d' c' j H Hin. unfold optimize_chain_datamap in H.
  destruct (optimize_rules_datamap n d (c_rules c)) as [[m d0] r0] eqn:E.
  inversion H; subst. exact (optimize_rules_datamap_keys_bound _ _ _ _ _ _ j E Hin).
Qed.
