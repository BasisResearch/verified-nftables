(** * Optimize_Vmap: the [nft -o] VERDICT-MAP merge, as an executable table-level
    ...
    in [Optimize_Merge] ([optimize_rules_sets]).  [nft -o] consolidates a run of
    ...
    a reference resolved at run time from [e_vmap (pkt_env p) "__vmapN"]).  So this
...
From Nft Require Import Bytes Packet Verdict Syntax Bytecode Semantics Optimize Optimize_Merge.
...
Lemma vmap_two_points : forall f nm v1 v2 w1 w2 body q,
  e_vmap (pkt_env q) nm = [(v1, v1, w1); (v2, v2, w2)] ->
  ...
  outcome_core (mk_vmap_rule f nm body) q
  = if eval_cmp CEq (field_value f q) v1 then Some w1
    else if eval_cmp CEq (field_value f q) v2 then Some w2
    else None.
Proof.
  intros f nm v1 v2 w1 w2 body q Hvm H1 H2 Hld.
  ...
  destruct (data_eqb v1 (field_value f q)) eqn:E1; [reflexivity|].
  destruct (data_eqb v2 (field_value f q)) eqn:E2; [reflexivity|].
...
Lemma orig_rule_loadable : forall f v body w p,
  rule_loadable (orig_rule f v body w) p =
    field_loadable f p &&
    (body_loadable_walk body p &&
     (if body_synproxy_stops body p then true else true)).
Proof.
  intros f v body w p. unfold orig_rule.
  rewrite rule_loadable_mk_head.
  cbn [match_loadable].
  (* end_loadable of mk_vmap_base = true (no vmap, terminal verdict loads) *)
  assert (Hend : forall q, end_loadable (mk_vmap_base w) q = true).
...
Lemma orig_rule_applies : forall f v body w p,
  rule_applies (orig_rule f v body w) p
  = eval_matchcond (MCmp f CEq v) p && rule_applies_walk body p.
Proof.
  intros. unfold orig_rule. apply rule_applies_mk_head.
Qed.

Lemma orig_rule_outcome : forall f v body w p,
  outcome (orig_rule f v body w) p =
...
Theorem eval_rules_vmap_merge2 : forall f nm v1 v2 w1 w2 body rest p,
  e_vmap (pkt_env p) nm = [(v1, v1, w1); (v2, v2, w2)] ->
  ...
  eval_rules (mk_vmap_rule f nm body :: rest) p
  = eval_rules (orig_rule f v1 body w1 :: orig_rule f v2 body w2 :: rest) p.
Proof.
  intros f nm v1 v2 w1 w2 body rest p Hvm Hfx1 Hfx2 Hw1 Hw2 Hsp Hnt.
  (* loadability of the merged rule *)
  assert (HmL : rule_loadable (mk_vmap_rule f nm body) p =
                body_loadable_walk body p &&
                (if field_loadable f p
                 then match assoc_verdict (field_value f p) (e_vmap (pkt_env p) nm) with
  ...
  { unfold rule_loadable, mk_vmap_rule. cbn [r_body].
    ...
    - destruct (assoc_verdict (field_value f p) (e_vmap (pkt_env p) nm)); reflexivity.
  ...
  { rewrite HmL. destruct (field_loadable f p);
      [ destruct (assoc_verdict (field_value f p) (e_vmap (pkt_env p) nm)) | ];
      rewrite ?Bool.andb_true_r, ?Bool.andb_false_r; reflexivity. }
  (* applicability of the merged rule: body only (no head match) *)
  assert (HmA : rule_applies (mk_vmap_rule f nm body) p = rule_applies_walk body p)
    by reflexivity.
  ...
  - (* f loads.  Build the two point-equality tests. *)
    set (b1 := eval_cmp CEq (field_value f p) v1).
    set (b2 := eval_cmp CEq (field_value f p) v2).
    (* outcome of the merged rule via vmap_two_points *)
    assert (Hmout : outcome (mk_vmap_rule f nm body) p =
    ...
    { unfold outcome, mk_vmap_rule. cbn [r_body]. rewrite Hsp.
      ...
      rewrite (vmap_two_points f nm v1 v2 w1 w2 body p Hvm); try assumption.
      - reflexivity.
      - intro. apply (field_fixed_len_loaded f (length v1) p Hfx1); assumption.
      - intro. apply (field_fixed_len_loaded f (length v2) p Hfx2); assumption. }
    ...
    (* eval_matchcond (MCmp f CEq vi) p = field_loadable f && b_i = b_i *)
    assert (Hm1 : eval_matchcond (MCmp f CEq v1) p = b1).
    { unfold eval_matchcond, eval_matchcond_body. cbn [match_loadable].
      rewrite Hfld. reflexivity. }
    assert (Hm2 : eval_matchcond (MCmp f CEq v2) p = b2).
    { unfold eval_matchcond, eval_matchcond_body. cbn [match_loadable].
      rewrite Hfld. reflexivity. }
    rewrite Hm1, Hm2.
    ...
    destruct (rule_applies_walk body p) eqn:HbA; cbn [andb].
    + (* body applies *)
...
Fixpoint optimize_rules_vmap (n : nat) (d : set_decls) (rs : list rule)
  ...
  | r1 :: ((r2 :: rest) as tl) =>
      ...
      | Some (f, v1, v2, w1, w2, body) =>
          ...
          let '(n'', d'', rest') := optimize_rules_vmap (S n) d' rest in
          (n'', d'', merged :: rest')
      | None =>
          let '(n'', d'', tl') := optimize_rules_vmap n d tl in
          (n'', d'', r1 :: tl')
      end
  | _ => (n, d, rs)
  end.

Lemma optimize_rules_vmap_cons2 : forall n d r1 r2 rest,
  optimize_rules_vmap n d (r1 :: r2 :: rest) =
  match vmap_merge_pair r1 r2 with
  | Some (f, v1, v2, w1, w2, body) =>
      let name := vmapname n in
      let d' := {| sd_sets := sd_sets d;
      ...
      (n'', d'', merged :: rest')
  | None =>
      let '(n'', d'', tl') := optimize_rules_vmap n d (r2 :: rest) in
      (n'', d'', r1 :: tl')
  end.
...
Definition optimize_chain_vmap (n : nat) (d : set_decls) (c : chain)
  : nat * set_decls * chain :=
  let '(n', d', rs') := optimize_rules_vmap n d (c_rules c) in
  (n', d', {| c_policy := c_policy c; c_rules := rs' |}).

(** *** Freshness bookkeeping for the minted vmap names (mirrors the set version). *)
Lemma optimize_rules_vmap_assoc_stable : forall rs n d n' d' rs' nm X,
  optimize_rules_vmap n d rs = (n', d', rs') ->
...
Proof.
  ...
  - cbn in H. inversion H; subst; reflexivity.
  - rewrite optimize_rules_vmap_cons2 in H. cbv zeta in H.
    destruct (vmap_merge_pair r1 r2) as [[[[[[f v1] v2] w1] w2] body] |] eqn:Evm.
    + destruct (optimize_rules_vmap (S n)
                  {| sd_sets := sd_sets d;
    ...
    + destruct (optimize_rules_vmap n d (r2 :: rest)) as [[m'' dd''] rr''] eqn:Erec.
      inversion H. subst n' d' rs'. clear H.
...
Theorem optimize_rules_vmap_correct : forall rs n d n' d' rs' base p,
  optimize_rules_vmap n d rs = (n', d', rs') ->
  rules_clean rs = true ->
  ...
  - rewrite optimize_rules_vmap_cons2 in H. cbv zeta in H.
    ...
    + (* MERGE fires *)
      set (dn := {| sd_sets := sd_sets d;
                    sd_vmaps := (vmapname n, [(v1,v1,w1);(v2,v2,w2)]) :: sd_vmaps d;
                    sd_maps := sd_maps d |}) in *.
      destruct (optimize_rules_vmap (S n) dn rest) as [[m'' dd''] rr''] eqn:Erec.
      inversion H; subst n' d' rs'. clear H.
      ...
      destruct (vmap_merge_pair_shape r1 r2 f v1 v2 w1 w2 body Evm)
        as [Hr1 [Hr2 [Hfx1 [Hfx2 [Hw1 Hw2]]]]].
      ...
      (* the shared body is clean (it is the tail of the clean r1) — so no stopping
         synproxy, no notrack, in the env qd. *)
      assert (HbodyClean : forallb bi_clean body = true).
      { (* r1 = orig_rule f v1 body w1 is clean; its body is BMatch(MCmp ..) :: body *)
        assert (Hbc1 : rule_clean r1 = true) by exact Hc1.
        rewrite Hr1 in Hbc1. unfold rule_clean, orig_rule, mk_head, mk_vmap_base in Hbc1.
        cbn [r_body] in Hbc1.
        apply Bool.andb_true_iff in Hbc1 as [Hbc1 _].
        apply Bool.andb_true_iff in Hbc1 as [Hbc1 _].
        apply Bool.andb_true_iff in Hbc1 as [Hbc1 _].
        apply Bool.andb_true_iff in Hbc1 as [Hbc1 _].
        apply Bool.andb_true_iff in Hbc1 as [Hbc1 _].
        apply Bool.andb_true_iff in Hbc1 as [Hbc1 _].
        cbn [forallb bi_clean mc_clean] in Hbc1.
        apply Bool.andb_true_iff in Hbc1 as [_ Hbc1]. exact Hbc1. }
      assert (Hsp : body_synproxy_stops body qd = false)
        by (apply body_synproxy_stops_clean; exact HbodyClean).
      assert (Hnt : body_has_notrack body = false)
        by (apply body_has_notrack_clean; exact HbodyClean).
      (* Goal: eval_rules (merged :: rr'') qd = eval_rules (r1::r2::rest) @d.
         Step 1: rr'' -> rest (clean tail). *)
      transitivity (eval_rules (mk_vmap_rule f (vmapname n) body :: rest) qd).
      { apply eval_rules_cons_cong. exact Htail'. }
      (* Step 2: vmap merge collapse (merged :: rest) -> (orig1 :: orig2 :: rest) @qd *)
      rewrite (eval_rules_vmap_merge2 f (vmapname n) v1 v2 w1 w2 body rest qd
                 Hlook Hfx1 Hfx2 Hw1 Hw2 Hsp Hnt).
      (* Step 3: orig1 = r1, orig2 = r2; whole clean list at dd'' equals at d. *)
      rewrite <- Hr1, <- Hr2.
...
Theorem optimize_chain_vmap_correct : forall n d c n' d' c' base p,
...
Proof.
  ...
  destruct (optimize_rules_vmap n d (c_rules c)) as [[m'' dd''] rr''] eqn:Erec.
  inversion H; subst n' d' c'. cbn [c_rules c_policy].
  unfold eval_chain. cbn [c_rules c_policy].
  rewrite (optimize_rules_vmap_correct (c_rules c) n d m'' dd'' rr'' base p Erec Hclean Hfresh).
  reflexivity.
Qed.
...
Fixpoint first_match (f : field) (q : packet) (l : list (data * verdict)) : option verdict :=
  match l with
  | [] => None
  | (v, w) :: tl => if eval_matchcond (MCmp f CEq v) q then Some w else first_match f q tl
  end.
...
Lemma assoc_verdict_points : forall es f q,
  (forall v w, In (v, w) es -> field_loadable f q = true ->
               length (field_value f q) = length v) ->
  field_loadable f q = true ->
  assoc_verdict (field_value f q) (map vmap_pt es) = first_match f q es.
Proof.
...
Lemma outcome_core_vmapN : forall es f q nm body,
  e_vmap (pkt_env q) nm = map vmap_pt es ->
  (forall v w, In (v, w) es -> field_loadable f q = true ->
               length (field_value f q) = length v) ->
  field_loadable f q = true ->
  outcome_core (mk_vmap_rule f nm body) q = first_match f q es.
Proof.
  intros es f q nm body Hvm Hlen Hld.
  unfold outcome_core, mk_vmap_rule. cbn [r_vmap vm_keyf vm_name vm_fields].
  cbn [apply_transforms fold_left]. rewrite Hvm.
  rewrite (assoc_verdict_points es f q Hlen Hld).
  destruct (first_match f q es) eqn:Efm; [reflexivity|].
...
Lemma orig_rule_outcome_clean : forall f v body w p,
  body_synproxy_stops body p = false ->
  body_has_notrack body = false ->
  terminal w = true ->
  outcome (orig_rule f v body w) p = Some w.
Proof.
  intros f v body w p Hsp Hnt Hw.
...
Lemma eval_rules_vmap_mergeN : forall f nm es body rest p,
  ...
  body_has_notrack body = false ->
  eval_rules (mk_vmap_rule f nm body :: rest) p
  = eval_rules (map (fun vw => orig_rule f (fst vw) body (snd vw)) es ++ rest) p.
Proof.
  intros f nm es body rest p Hvm Hfx Hterm Hsp Hnt.
  ...
  - (* f loads: merged outcome = first_match; the run scans the same keys *)
    ...
    assert (Hmout : outcome (mk_vmap_rule f nm body) p = first_match f p es).
    { unfold outcome, mk_vmap_rule. cbn [r_body]. rewrite Hsp.
      unfold body_thread. cbn [r_body]. rewrite Hnt.
      change ({| r_body := body; r_verdict := Continue;
                 r_vmap := Some {| vm_fields := [f]; vm_keyf := Some (f, []); vm_name := nm |};
                 r_nat := None; r_tproxy := None; r_fwd := None; r_queue := None;
                 r_after := [] |}) with (mk_vmap_rule f nm body).
      apply (outcome_core_vmapN es f p nm body Hvm); [| exact Hfld].
      intros v w Hin Hld. apply (field_fixed_len_loaded f (length v) p (Hfx v w Hin) Hld). }
    rewrite Hmout.
    destruct (body_loadable_walk body p) eqn:HbL; cbn [andb].
    + destruct (rule_applies_walk body p) eqn:HbA; cbn [andb].
      * (* body loads & applies: induct on es, matching first_match to the run *)
        ...
        -- cbn [eval_rules].
           rewrite orig_rule_loadable, orig_rule_applies,
                   (orig_rule_outcome_clean f v body w p Hsp Hnt
                      (Hterm v w (or_introl eq_refl))).
           cbn [match_loadable]. rewrite Hfld, HbL, Hsp. cbn [andb].
           assert (Hm : eval_matchcond (MCmp f CEq v) p
                        = eval_cmp CEq (field_value f p) v).
           { unfold eval_matchcond, eval_matchcond_body. cbn [match_loadable].
             rewrite Hfld. reflexivity. }
           rewrite Hm. rewrite HbA. cbn [andb].
           destruct (eval_cmp CEq (field_value f p) v) eqn:Ev.
...
(** ** Executable N-WAY vmap pass.
    ...
    [optimize_rules_vmapN] folds the whole run [r1 :: matched] into ONE
            ...
            | ((_ :: _) as es, rest') =>
                if has_distinct_verdict (r_verdict r1) es && body_vmap_safe body then
                  ...
                  let d' := {| sd_sets := sd_sets d;
                               ...
                               sd_maps := sd_maps d |} in
                  let merged := mk_vmap_rule f name body in
                  let '(n'', d'', rest'') := optimize_rules_vmapN fuel' (S n) d' rest' in
                  (n'', d'', merged :: rest'')
                else
                  let '(n'', d'', rest') := optimize_rules_vmapN fuel' n d rest in
                  (n'', d'', r1 :: rest')
            | ([], _) =>
                let '(n'', d'', rest') := optimize_rules_vmapN fuel' n d rest in
                (n'', d'', r1 :: rest')
...
Definition optimize_chain_vmapN (n : nat) (d : set_decls) (c : chain)
  : nat * set_decls * chain :=
  let '(n', d', rs') := optimize_rules_vmapN (length (c_rules c)) n d (c_rules c) in
  (n', d', {| c_policy := c_policy c; c_rules := rs' |}).

Lemma optimize_rules_vmapN_consSS : forall fuel n d r1 r2 rest,
  optimize_rules_vmapN (S fuel) n d (r1 :: r2 :: rest) =
  match head_value r1 with
  | Some (f, v1, body) =>
      ...
      | ((_ :: _) as es, rest') =>
          if has_distinct_verdict (r_verdict r1) es && body_vmap_safe body then
            ...
            let '(n'', d'', rest') := optimize_rules_vmapN fuel n d (r2 :: rest) in
            (n'', d'', r1 :: rest')
      | ([], _) =>
          let '(n'', d'', rest') := optimize_rules_vmapN fuel n d (r2 :: rest) in
          (n'', d'', r1 :: rest')
...
Lemma optimize_rules_vmapN_assoc_stable : forall fuel n d rs n' d' rs' nm X,
...
Theorem optimize_rules_vmapN_correct : forall fuel rs n d n' d' rs' base p,
  optimize_rules_vmapN fuel n d rs = (n', d', rs') ->
  rules_clean rs = true ->
  (forall k, n <= k -> ~ In (vmapname k) (map fst (sd_vmaps d))) ->
  eval_rules rs' (set_env p (env_with_sets base d'))
      ...
      * destruct (take_vmap_run r1 (r2 :: rest)) as [es rest'] eqn:Erun.
        ...
        -- destruct (take_vmap_run_head r1 f v1 body r2 rest (e :: es') rest' Ehd Erun
           ...
           { cbn [set_env with_pkt_env pkt_env]. rewrite e_vmap_env_with_sets.
             ...
             - intros k Hk Heq. apply vmapname_inj in Heq. lia. }
           set (qd := set_env p (env_with_sets base dd'')) in *.
           (* collapse the merged vmap rule into the whole run *)
           ...
           { unfold qd. apply (eval_rules_vmap_mergeN f (vmapname n) entries body rr''
             ...
             - apply body_synproxy_stops_clean.
               (* body is clean: it is the tail of clean r1 *)
             ...
             - apply body_has_notrack_clean.
               ...
               repeat (apply Bool.andb_true_iff in Hc1 as [Hc1 ?]).
               match goal with H : forallb bi_clean body = true |- _ => exact H end. }
           (* tail rr'' -> rest' (clean, env-stable) *)
           assert (Htail' : eval_rules rr'' qd = eval_rules rest' qd).
           { rewrite Htail. unfold qd.
             apply (eval_rules_clean_env rest' p base dn dd''). exact Hcrest'. }
...
Theorem optimize_chain_vmapN_correct : forall n d c n' d' c' base p,
...
Proof.
  ...
  destruct (optimize_rules_vmapN (length (c_rules c)) n d (c_rules c))
    as [[m'' dd''] rr''] eqn:Erec.
  inversion H; subst n' d' c'. cbn [c_rules c_policy].
  unfold eval_chain. cbn [c_rules c_policy].
  rewrite (optimize_rules_vmapN_correct (length (c_rules c)) (c_rules c) n d
