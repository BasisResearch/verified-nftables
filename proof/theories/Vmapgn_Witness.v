(** Fires-witness for the guarded value+verdict -> VERDICT MAP merge on the
    NETWORK-ADDRESS arm (Optimize_Vmapg.guard_okn): a run of adjacent
    ...
    verdict-register map).  This shows [optimize_rules_vmapNg] fires NON-VACUOUSLY on
    the nfproto-guarded network arm — the l4proto witness lives in
    [Optimize_Vmapg_Witness]. *)
...
Definition vmapgn_output :=
  optimize_rules_vmapNg (List.length vmapgn_input) 0 empty_decls vmapgn_input.
...
Example guard_okn_fires : Optimize_Vmapg.guard_okn gmn = true.
...
Example vmapgn6_fires :
  let '(n', d', rs') :=
    optimize_rules_vmapNg (List.length vmapgn6_input) 0 empty_decls vmapgn6_input in
