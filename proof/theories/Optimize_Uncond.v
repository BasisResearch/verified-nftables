...
From Nft Require Import Bytes Packet Verdict Syntax Bytecode Semantics
  ...
  Optimize_ConcatM Optimize_Setg Optimize_Ivset Optimize_Ivsetg Optimize_Ivmixg Optimize_Absorb Optimize_Ctmask
  Optimize_Dscp Optimize_Dscpv Optimize_Ivsett Optimize_Mapn Optimize_Dnat Optimize_Snat Optimize_Table_Inv Optimize_Table Optimize_Normalize.
...
Proof.
  ...
  - rewrite !set_untracked_set_env.
    apply (end_loadable_agree r (set_untracked p) base d1 d2 Hvmap Hmap).
  - apply (end_loadable_agree r p base d1 d2 Hvmap Hmap).
Qed.
...
Lemma eval_rules_cons_cong : forall r X Y q1 q2,
  rule_loadable r q1 = rule_loadable r q2 ->
  rule_applies r q1 = rule_applies r q2 ->
  outcome r q1 = outcome r q2 ->
  eval_rules X q1 = eval_rules Y q2 ->
  eval_rules (r :: X) q1 = eval_rules (r :: Y) q2.
Proof.
  intros r X Y q1 q2 HL HA HO HXY. cbn [eval_rules]. rewrite HL, HA, HO, HXY. reflexivity.
Qed.
      ...
      * destruct (take_value_run r1 (r2 :: rest)) as [vs rest'] eqn:Erun.
        ...
        -- (* no eligible neighbour: keep r1, recurse *)
           ...
           { apply (decls_agree_rule_setseam base d dd'' r1 n).
             ...
             - apply (optimize_rules_setsN_maps fuel n d (r2 :: rest) m'' dd'' rr'' (eq_sym Erec)).
        ...
        -- (* RUN of >= 2 rules: fold them all into one __setN *)
           ...
           { rewrite Htail. unfold qd.
             ...
             - apply (optimize_rules_setsN_maps fuel (S n) dn rest' m'' dd'' rr'' (eq_sym Erec)).
           ...
           { rewrite (optimize_rules_setsN_maps fuel (S n) dn rest' m'' dd'' rr'' (eq_sym Erec)).
             subst dn; reflexivity. }
    ...
    + rewrite optimize_rules_dscp_consSS in H.
      ...
      * destruct (take_dscp_run r1 (r2 :: rest)) as [vs rest'] eqn:Erun.
        ...
        -- cbv zeta in H.
           ...
           assert (Hcert : eval_matchcond (MSetT f [TBitAnd mask xor] false (setname n)) qd
                   = existsb (fun w => eval_matchcond (MMasked f false mask xor w) qd) vals).
           { apply (mmasked_set_existsb f mask xor vals (setname n) qd).
             - subst elems. exact Hlook.
             - intros w Hw Hld.
               assert (Hfxw : field_fixed_len f = Some (Datatypes.length w)) by (apply (Hallw w Hw)).
               assert (Hfl : Datatypes.length (field_value f qd) = Datatypes.length w)
                 by (apply (field_fixed_len_loaded f (Datatypes.length w) qd Hfxw Hld)).
               ...
               rewrite (data_bitops_length_eq (field_value f qd) mask xor).
               + exact Hfl.
               + lia.
               + lia. }
           transitivity (eval_rules
           ...
           { apply (eval_rules_run_merge_abs
                      ...
                      (MSetT f [TBitAnd mask xor] false (setname n)) rr'' qd).
             - subst vals. discriminate.
             - intros m Hm. apply (match_loadable_dscp_run f mask xor vals qd m Hm).
...
(** *** ivsett (host-order interval / range set consolidation, Optimize_Ivsett).
    Structurally identical to [ivset], but the run's heads are transformed ranges
...
Proof.
  ...
  - destruct rs as [| r1 [| r2 rest] ].
    + cbn in H. inversion H; subst; reflexivity.
    + cbn in H. inversion H; subst; reflexivity.
    + rewrite optimize_rules_ivsett_consSS in H.
      inversion Hrf as [| ? ? Hf1 Hrf_tail]; subst.
      ...
      * destruct (take_ivsett_run r1 (r2 :: rest)) as [ivs rest'] eqn:Erun.
        ...
        -- remember (optimize_rules_ivsett fuel n d (r2 :: rest)) as t eqn:Erec.
           destruct t as [[m'' dd''] rr'']. cbv zeta in H.
           injection H as Hn' Hd' Hr'. subst n' d' rs'.
           cbn [eval_rules].
           rewrite (IH (r2 :: rest) n d m'' dd'' rr'' base p (eq_sym Erec) Hfresh Hrf_tail).
           assert (Hda1 : decls_agree_rule base dd'' d r1).
           { apply (decls_agree_rule_setseam base d dd'' r1 n).
             - apply (optimize_rules_ivsett_vmaps fuel n d (r2 :: rest) m'' dd'' rr'' (eq_sym Erec)).
             - apply (optimize_rules_ivsett_maps fuel n d (r2 :: rest) m'' dd'' rr'' (eq_sym Erec)).
             - intros nm X Hf. apply (optimize_rules_ivsett_assoc_stable fuel n d (r2 :: rest)
                                       m'' dd'' rr'' nm X (eq_sym Erec) Hf).
             - exact Hf1. }
           rewrite (rule_loadable_agree_gen r1 p base dd'' d Hda1).
           rewrite (rule_applies_agree_gen r1 p base dd'' d Hda1).
           rewrite (outcome_agree_gen r1 p base dd'' d Hda1). reflexivity.
        -- cbv zeta in H.
           remember (optimize_rules_ivsett fuel (S n)
           ...
           assert (Hlook : e_set (pkt_env (set_env p (env_with_sets base dd''))) (setname n)
                           = ivsAll).
           { cbn [set_env with_pkt_env pkt_env]. rewrite e_set_declared.
             erewrite (optimize_rules_ivsett_assoc_stable fuel (S n) dn _ _ _ _
                         (setname n) _ (eq_sym Erec)).
             - subst dn; cbn [sd_sets assoc_str]. rewrite String.eqb_refl. reflexivity.
             - intros k Hk Heq. apply setname_inj in Heq. lia. }
           set (qd := set_env p (env_with_sets base dd'')) in *.
           assert (Hcert : eval_matchcond (MSetT f ts false (setname n)) qd
                   = existsb (fun iv0 => eval_matchcond (MRangeT f ts false (fst iv0) (snd iv0)) qd) ivsAll).
           { apply (msett_ivs_existsb f ts ivsAll (setname n) qd). exact Hlook. }
           transitivity (eval_rules
             (map (fun m => mk_head m body r1)
           ...
           { apply (eval_rules_run_merge_abs
                      (map (fun iv0 => MRangeT f ts false (fst iv0) (snd iv0)) ivsAll)
                      (fun q => field_loadable f q) body r1
                      (MSetT f ts false (setname n)) rr'' qd).
             - subst ivsAll. discriminate.
             - intros m Hm. apply (match_loadable_ranget_run f ts ivsAll qd m Hm).
           ...
           { rewrite Htail. unfold qd.
             ...
             intros r Hr. apply decls_agree_rule_sym.
             apply (decls_agree_rule_setseam base dn dd'' r (S n)).
             - apply (optimize_rules_ivsett_vmaps fuel (S n) dn rest' m'' dd'' rr'' (eq_sym Erec)).
             - apply (optimize_rules_ivsett_maps fuel (S n) dn rest' m'' dd'' rr'' (eq_sym Erec)).
             - intros nm X Hf. apply (optimize_rules_ivsett_assoc_stable fuel (S n) dn rest'
                                       m'' dd'' rr'' nm X (eq_sym Erec) Hf).
             - rewrite Forall_forall in Hrf_rest'. apply Hrf_rest'; exact Hr. }
           rewrite (eval_rules_app_cong
                      (map (fun iv0 => mk_head (MRangeT f ts false (fst iv0) (snd iv0)) body r1) ivsAll)
           ...
           rewrite <- Hrun_eq.
           assert (Hvm_dd : sd_vmaps dd'' = sd_vmaps d).
           { rewrite (optimize_rules_ivsett_vmaps fuel (S n) dn rest' m'' dd'' rr'' (eq_sym Erec)).
             subst dn; reflexivity. }
           assert (Hmaps_dd : sd_maps dd'' = sd_maps d).
           { rewrite (optimize_rules_ivsett_maps fuel (S n) dn rest' m'' dd'' rr'' (eq_sym Erec)).
             subst dn; reflexivity. }
           assert (Hassoc_dd : forall nm X, (forall k, n <= k -> nm <> setname k) ->
                     assoc_str nm (sd_sets dd'') X = assoc_str nm (sd_sets d) X).
           { intros nm X Hf.
             rewrite (optimize_rules_ivsett_assoc_stable fuel (S n) dn rest' m'' dd'' rr'' nm X
                        (eq_sym Erec) (fun k Hk => Hf k ltac:(lia))).
             subst dn; cbn [sd_sets assoc_str].
             destruct (String.eqb nm (setname n)) eqn:Eq.
             - apply String.eqb_eq in Eq. exfalso. apply (Hf n (Nat.le_refl n) Eq).
...
(** *** ivset (interval / range set consolidation, Optimize_Ivset).  Structurally a
    single-field set merge like setsN, but the run's heads are [MRange]s and the
...
Proof.
  ...
  - destruct rs as [| r1 [| r2 rest] ].
    + cbn in H. inversion H; subst; reflexivity.
    + cbn in H. inversion H; subst; reflexivity.
    + rewrite optimize_rules_ivset_consSS in H.
      inversion Hrf as [| ? ? Hf1 Hrf_tail]; subst.
      ...
      * destruct (take_range_run r1 (r2 :: rest)) as [ivs rest'] eqn:Erun.
        ...
        destruct ivs as [| iv ivs'].
        -- (* no eligible neighbour: keep r1, recurse *)
           remember (optimize_rules_ivset fuel n d (r2 :: rest)) as t eqn:Erec.
           destruct t as [[m'' dd''] rr'']. cbv zeta in H.
           injection H as Hn' Hd' Hr'. subst n' d' rs'.
           cbn [eval_rules].
           rewrite (IH (r2 :: rest) n d m'' dd'' rr'' base p (eq_sym Erec) Hfresh Hrf_tail).
           assert (Hda1 : decls_agree_rule base dd'' d r1).
           { apply (decls_agree_rule_setseam base d dd'' r1 n).
             - apply (optimize_rules_ivset_vmaps fuel n d (r2 :: rest) m'' dd'' rr'' (eq_sym Erec)).
             - apply (optimize_rules_ivset_maps fuel n d (r2 :: rest) m'' dd'' rr'' (eq_sym Erec)).
             - intros nm X Hf. apply (optimize_rules_ivset_assoc_stable fuel n d (r2 :: rest)
                                       m'' dd'' rr'' nm X (eq_sym Erec) Hf).
             - exact Hf1. }
           rewrite (rule_loadable_agree_gen r1 p base dd'' d Hda1).
           rewrite (rule_applies_agree_gen r1 p base dd'' d Hda1).
        ...
        -- (* RUN of >= 2 rules: fold into one interval __setN *)
           cbv zeta in H.
           remember (optimize_rules_ivset fuel (S n)
                       {| sd_sets := (setname n, (lo1, hi1) :: iv :: ivs') :: sd_sets d;
                          sd_vmaps := sd_vmaps d; sd_maps := sd_maps d |} rest')
             as t eqn:Erec.
           destruct t as [[m'' dd''] rr'']. cbv zeta in H.
           ...
           assert (Hlook : e_set (pkt_env (set_env p (env_with_sets base dd''))) (setname n)
                           = ivsAll).
           { cbn [set_env with_pkt_env pkt_env]. rewrite e_set_declared.
             erewrite (optimize_rules_ivset_assoc_stable fuel (S n) dn _ _ _ _
                         (setname n) _ (eq_sym Erec)).
             - subst dn; cbn [sd_sets assoc_str]. rewrite String.eqb_refl. reflexivity.
             - intros k Hk Heq. apply setname_inj in Heq. lia. }
           set (qd := set_env p (env_with_sets base dd'')) in *.
           assert (Hcert : eval_matchcond (MConcatSet [f] false (setname n)) qd
                   = existsb (fun iv0 => eval_matchcond (MRange f false (fst iv0) (snd iv0)) qd) ivsAll).
           { apply (concat_set_ivs_existsb f ivsAll (setname n) qd). exact Hlook. }
           transitivity (eval_rules
             (map (fun m => mk_head m body r1)
           ...
           { apply (eval_rules_run_merge_abs
                      ...
                      (MConcatSet [f] false (setname n)) rr'' qd).
             - subst ivsAll. discriminate.
             - intros m Hm. apply (match_loadable_range_run f ivsAll qd m Hm).
             - apply match_loadable_mconcat1.
             - rewrite Hcert. rewrite existsb_map_eq. reflexivity. }
           rewrite List.map_map.
           assert (Htail' : eval_rules rr'' qd = eval_rules rest' qd).
           { rewrite Htail. unfold qd.
             ...
             intros r Hr. apply decls_agree_rule_sym.
             apply (decls_agree_rule_setseam base dn dd'' r (S n)).
             - apply (optimize_rules_ivset_vmaps fuel (S n) dn rest' m'' dd'' rr'' (eq_sym Erec)).
             - apply (optimize_rules_ivset_maps fuel (S n) dn rest' m'' dd'' rr'' (eq_sym Erec)).
             - intros nm X Hf. apply (optimize_rules_ivset_assoc_stable fuel (S n) dn rest'
                                       m'' dd'' rr'' nm X (eq_sym Erec) Hf).
             - rewrite Forall_forall in Hrf_rest'. apply Hrf_rest'; exact Hr. }
           rewrite (eval_rules_app_cong
                      (map (fun iv0 => mk_head (MRange f false (fst iv0) (snd iv0)) body r1) ivsAll)
           ...
           rewrite <- Hrun_eq.
           assert (Hvm_dd : sd_vmaps dd'' = sd_vmaps d).
           { rewrite (optimize_rules_ivset_vmaps fuel (S n) dn rest' m'' dd'' rr'' (eq_sym Erec)).
             subst dn; reflexivity. }
           assert (Hmaps_dd : sd_maps dd'' = sd_maps d).
           { rewrite (optimize_rules_ivset_maps fuel (S n) dn rest' m'' dd'' rr'' (eq_sym Erec)).
             subst dn; reflexivity. }
           assert (Hassoc_dd : forall nm X, (forall k, n <= k -> nm <> setname k) ->
                     assoc_str nm (sd_sets dd'') X = assoc_str nm (sd_sets d) X).
           { intros nm X Hf.
             rewrite (optimize_rules_ivset_assoc_stable fuel (S n) dn rest' m'' dd'' rr'' nm X
                        (eq_sym Erec) (fun k Hk => Hf k ltac:(lia))).
             subst dn; cbn [sd_sets assoc_str].
             destruct (String.eqb nm (setname n)) eqn:Eq.
             - apply String.eqb_eq in Eq. exfalso. apply (Hf n (Nat.le_refl n) Eq).
...
(** *** ivsetg (GUARDED interval / range set consolidation, Optimize_Ivsetg).
    Combines the guarded run-collapse of [setg] ([existsb_guardhead_factor] factors
    ...
    + rewrite optimize_rules_ivsetg_consSS in H.
      ...
      * destruct (take_rangeg_run r1 (r2 :: rest)) as [ivs rest'] eqn:Erun.
        ...
        -- remember (optimize_rules_ivsetg fuel n d (r2 :: rest)) as tt eqn:Erec.
           ...
           assert (Hda1 : decls_agree_rule base dd'' d r1).
           { apply (decls_agree_rule_setseam base d dd'' r1 n).
             - apply (optimize_rules_ivsetg_vmaps fuel n d (r2 :: rest) m'' dd'' rr'' (eq_sym Erec)).
             - apply (optimize_rules_ivsetg_maps fuel n d (r2 :: rest) m'' dd'' rr'' (eq_sym Erec)).
             - intros nm X Hf. apply (optimize_rules_ivsetg_assoc_stable fuel n d (r2 :: rest)
                                       m'' dd'' rr'' nm X (eq_sym Erec) Hf).
             - exact Hf1. }
           rewrite (rule_loadable_agree_gen r1 p base dd'' d Hda1).
           rewrite (rule_applies_agree_gen r1 p base dd'' d Hda1).
        ...
        -- cbv zeta in H.
           ...
           { eapply Forall_impl; [intros r Hr; apply (rule_set_fresh_mono n (S n) r); [lia | exact Hr] |].
             ...
             exact Hsub. }
           assert (Hvm_dd : sd_vmaps dd'' = sd_vmaps d).
           { rewrite (optimize_rules_ivsetg_vmaps fuel (S n) dn rest' m'' dd'' rr'' (eq_sym Erec)).
             subst dn; reflexivity. }
           assert (Hmaps_dd : sd_maps dd'' = sd_maps d).
           { rewrite (optimize_rules_ivsetg_maps fuel (S n) dn rest' m'' dd'' rr'' (eq_sym Erec)).
             subst dn; reflexivity. }
           assert (Hassoc_dd : forall nm X, (forall k, n <= k -> nm <> setname k) ->
                     assoc_str nm (sd_sets dd'') X = assoc_str nm (sd_sets d) X).
           { intros nm X Hf.
             rewrite (optimize_rules_ivsetg_assoc_stable fuel (S n) dn rest' m'' dd'' rr'' nm X
                        (eq_sym Erec) (fun k Hk => Hf k ltac:(lia))).
             subst dn; cbn [sd_sets assoc_str].
             destruct (String.eqb nm (setname n)) eqn:Eq.
             - apply String.eqb_eq in Eq. exfalso. apply (Hf n (Nat.le_refl n) Eq).
           ...
           assert (Hlook : e_set (pkt_env (set_env p (env_with_sets base dd''))) (setname n)
                           = ivsAll).
           { cbn [set_env with_pkt_env pkt_env]. rewrite e_set_declared.
             erewrite (optimize_rules_ivsetg_assoc_stable fuel (S n) dn _ _ _ _
                         (setname n) _ (eq_sym Erec)).
             - subst dn; cbn [sd_sets assoc_str]. rewrite String.eqb_refl. reflexivity.
             - intros k Hk Heq. apply setname_inj in Heq. lia. }
           set (qd := set_env p (env_with_sets base dd'')) in *.
           ...
           { apply (concat_set_ivs_existsb f ivsAll (setname n) qd). exact Hlook. }
           transitivity (eval_rules
             (map (fun iv0 => orig_ruleGr f gm (fst iv0) (snd iv0) body r1) ivsAll ++ rr'') qd).
           { apply (eval_rules_run_collapse
                      (map (fun iv0 => orig_ruleGr f gm (fst iv0) (snd iv0) body r1) ivsAll)
                      (rule_loadable (merged_ruleGs f gm (setname n) body r1) qd)
                      (outcome (merged_ruleGs f gm (setname n) body r1) qd)
                      (merged_ruleGs f gm (setname n) body r1) rr'' qd).
             - subst ivsAll. discriminate.
             ...
             - rewrite merged_ruleGs_applies. rewrite Hcert.
               ...
               rewrite (existsb_ext _ _ _ ivsAll
                          (fun iv0 (_ : In iv0 ivsAll) =>
                             orig_ruleGr_applies f gm (fst iv0) (snd iv0) body r1 qd)).
           ...
           { rewrite Htail. unfold qd.
             ...
             intros r Hr. apply decls_agree_rule_sym.
             apply (decls_agree_rule_setseam base dn dd'' r (S n)).
             - apply (optimize_rules_ivsetg_vmaps fuel (S n) dn rest' m'' dd'' rr'' (eq_sym Erec)).
             - apply (optimize_rules_ivsetg_maps fuel (S n) dn rest' m'' dd'' rr'' (eq_sym Erec)).
             - intros nm X Hf. apply (optimize_rules_ivsetg_assoc_stable fuel (S n) dn rest'
                                       m'' dd'' rr'' nm X (eq_sym Erec) Hf).
             - rewrite Forall_forall in Hrf_rest'. apply Hrf_rest'; exact Hr. }
           rewrite (eval_rules_app_cong
                      (map (fun iv0 => orig_ruleGr f gm (fst iv0) (snd iv0) body r1) ivsAll)
...
(** *** ivmixg (GUARDED MIXED point+range set consolidation, Optimize_Ivmixg).
    Structurally identical to [ivsetg]; the only new content is the per-element
...
Proof.
  ...
  - destruct rs as [| r1 [| r2 rest] ].
    + cbn in H. inversion H; subst; reflexivity.
    + cbn in H. inversion H; subst; reflexivity.
    + rewrite optimize_rules_ivmixg_consSS in H.
      inversion Hrf as [| ? ? Hf1 Hrf_tail]; subst.
      ...
      * destruct (take_mix_run r1 (r2 :: rest)) as [es rest'] eqn:Erun.
        ...
        destruct es as [| e es'].
        -- remember (optimize_rules_ivmixg fuel n d (r2 :: rest)) as tt eqn:Erec.
           destruct tt as [[m'' dd''] rr'']. cbv zeta in H.
           injection H as Hn' Hd' Hr'. subst n' d' rs'.
           cbn [eval_rules].
           rewrite (IH (r2 :: rest) n d m'' dd'' rr'' base p (eq_sym Erec) Hfresh Hrf_tail).
           assert (Hda1 : decls_agree_rule base dd'' d r1).
           { apply (decls_agree_rule_setseam base d dd'' r1 n).
             - apply (optimize_rules_ivmixg_vmaps fuel n d (r2 :: rest) m'' dd'' rr'' (eq_sym Erec)).
             - apply (optimize_rules_ivmixg_maps fuel n d (r2 :: rest) m'' dd'' rr'' (eq_sym Erec)).
             - intros nm X Hf. apply (optimize_rules_ivmixg_assoc_stable fuel n d (r2 :: rest)
                                       m'' dd'' rr'' nm X (eq_sym Erec) Hf).
             - exact Hf1. }
           rewrite (rule_loadable_agree_gen r1 p base dd'' d Hda1).
           rewrite (rule_applies_agree_gen r1 p base dd'' d Hda1).
           rewrite (outcome_agree_gen r1 p base dd'' d Hda1). reflexivity.
        -- cbv zeta in H.
           remember (optimize_rules_ivmixg fuel (S n)
           ...
           { eapply Forall_impl; [intros r Hr; apply (rule_set_fresh_mono n (S n) r); [lia | exact Hr] |].
             ...
             exact Hsub. }
           assert (Hvm_dd : sd_vmaps dd'' = sd_vmaps d).
           { rewrite (optimize_rules_ivmixg_vmaps fuel (S n) dn rest' m'' dd'' rr'' (eq_sym Erec)).
             subst dn; reflexivity. }
           assert (Hmaps_dd : sd_maps dd'' = sd_maps d).
           { rewrite (optimize_rules_ivmixg_maps fuel (S n) dn rest' m'' dd'' rr'' (eq_sym Erec)).
             subst dn; reflexivity. }
           assert (Hassoc_dd : forall nm X, (forall k, n <= k -> nm <> setname k) ->
                     assoc_str nm (sd_sets dd'') X = assoc_str nm (sd_sets d) X).
           { intros nm X Hf.
             rewrite (optimize_rules_ivmixg_assoc_stable fuel (S n) dn rest' m'' dd'' rr'' nm X
                        (eq_sym Erec) (fun k Hk => Hf k ltac:(lia))).
             subst dn; cbn [sd_sets assoc_str].
             destruct (String.eqb nm (setname n)) eqn:Eq.
             - apply String.eqb_eq in Eq. exfalso. apply (Hf n (Nat.le_refl n) Eq).
           ...
           assert (Hlook : e_set (pkt_env (set_env p (env_with_sets base dd''))) (setname n)
                           = map melem_iv esAll).
           { cbn [set_env with_pkt_env pkt_env]. rewrite e_set_declared.
             erewrite (optimize_rules_ivmixg_assoc_stable fuel (S n) dn _ _ _ _
                         (setname n) _ (eq_sym Erec)).
             - subst dn; cbn [sd_sets assoc_str]. rewrite String.eqb_refl. reflexivity.
             - intros k Hk Heq. apply setname_inj in Heq. lia. }
           set (qd := set_env p (env_with_sets base dd'')) in *.
           assert (Hcert : eval_matchcond (MConcatSet [f] false (setname n)) qd
                   = existsb (fun iv0 => eval_matchcond (MRange f false (fst iv0) (snd iv0)) qd)
                             (map melem_iv esAll)).
           { apply (concat_set_ivs_existsb f (map melem_iv esAll) (setname n) qd). exact Hlook. }
           transitivity (eval_rules
             (map (fun e0 => orig_ruleGm f gm e0 body r1) esAll ++ rr'') qd).
           { apply (eval_rules_run_collapse
             ...
             - rewrite merged_ruleGs_applies. rewrite Hcert. rewrite existsb_map_eq.
               rewrite (existsb_ext _
                          (fun e0 => eval_matchcond
                                       (MRange f false (fst (melem_iv e0)) (snd (melem_iv e0))) qd)
                          (fun e0 => eval_matchcond (melem_mc f e0) qd) esAll
                          (fun e0 He0 => eq_sym (eval_melem_mrange f e0 qd (Hok_all e0 He0)))).
               symmetry. rewrite existsb_map_eq.
               rewrite (existsb_ext _ _ _ esAll
                          (fun e0 (_ : In e0 esAll) =>
                             orig_ruleGm_applies f gm e0 body r1 qd)).
           ...
           { rewrite Htail. unfold qd.
             ...
             intros r Hr. apply decls_agree_rule_sym.
             apply (decls_agree_rule_setseam base dn dd'' r (S n)).
             - apply (optimize_rules_ivmixg_vmaps fuel (S n) dn rest' m'' dd'' rr'' (eq_sym Erec)).
             - apply (optimize_rules_ivmixg_maps fuel (S n) dn rest' m'' dd'' rr'' (eq_sym Erec)).
             - intros nm X Hf. apply (optimize_rules_ivmixg_assoc_stable fuel (S n) dn rest'
                                       m'' dd'' rr'' nm X (eq_sym Erec) Hf).
             - rewrite Forall_forall in Hrf_rest'. apply Hrf_rest'; exact Hr. }
           rewrite (eval_rules_app_cong
                      (map (fun e0 => orig_ruleGm f gm e0 body r1) esAll)
      ...
      * destruct (take_concat_run r1 (r2 :: rest)) as [ts rest'] eqn:Erun.
        ...
        -- remember (optimize_rules_concatN fuel n d (r2 :: rest)) as tt eqn:Erec.
           ...
           { apply (decls_agree_rule_setseam base d dd'' r1 n).
             ...
             - apply (optimize_rules_concatN_maps fuel n d (r2 :: rest) m'' dd'' rr'' (eq_sym Erec)).
        ...
        -- cbv zeta in H.
           ...
           { rewrite (optimize_rules_concatN_vmaps fuel (S n) dn rest' m'' dd'' rr'' (eq_sym Erec)).
             subst dn; reflexivity. }
           assert (Hmaps_dd : sd_maps dd'' = sd_maps d).
           { rewrite (optimize_rules_concatN_maps fuel (S n) dn rest' m'' dd'' rr'' (eq_sym Erec)).
             subst dn; reflexivity. }
           assert (Hassoc_dd : forall nm X, (forall k, n <= k -> nm <> setname k) ->
                     assoc_str nm (sd_sets dd'') X = assoc_str nm (sd_sets d) X).
           { intros nm X Hf.
           ...
           { rewrite Htail. unfold qd.
             ...
             - apply (optimize_rules_concatN_maps fuel (S n) dn rest' m'' dd'' rr'' (eq_sym Erec)).
...
(** *** concatM (the guarded transport-key concat pass, Optimize_ConcatM).  Mirrors
    [concatN] verbatim EXCEPT the merged rule hoists the shared guard [gm] to the head
    ...
    + rewrite optimize_rules_concatM_consSS in H.
      ...
      * destruct (take_concatg_run r1 (r2 :: rest)) as [ts rest'] eqn:Erun.
        ...
        -- remember (optimize_rules_concatM fuel n d (r2 :: rest)) as tt eqn:Erec.
           ...
           assert (Hda1 : decls_agree_rule base dd'' d r1).
           { apply (decls_agree_rule_setseam base d dd'' r1 n).
             - apply (optimize_rules_concatM_vmaps fuel n d (r2 :: rest) m'' dd'' rr'' (eq_sym Erec)).
             - apply (optimize_rules_concatM_maps fuel n d (r2 :: rest) m'' dd'' rr'' (eq_sym Erec)).
             - intros nm X Hf. apply (optimize_rules_concatM_assoc_stable fuel n d (r2 :: rest)
                                       m'' dd'' rr'' nm X (eq_sym Erec) Hf).
             - exact Hf1. }
           rewrite (rule_loadable_agree_gen r1 p base dd'' d Hda1).
           rewrite (rule_applies_agree_gen r1 p base dd'' d Hda1).
        ...
        -- cbv zeta in H.
           ...
           { eapply Forall_impl; [intros r Hr; apply (rule_set_fresh_mono n (S n) r); [lia | exact Hr] |].
             ...
             exact Hsub. }
           assert (Hvm_dd : sd_vmaps dd'' = sd_vmaps d).
           { rewrite (optimize_rules_concatM_vmaps fuel (S n) dn rest' m'' dd'' rr'' (eq_sym Erec)).
             subst dn; reflexivity. }
           assert (Hmaps_dd : sd_maps dd'' = sd_maps d).
           { rewrite (optimize_rules_concatM_maps fuel (S n) dn rest' m'' dd'' rr'' (eq_sym Erec)).
             subst dn; reflexivity. }
           assert (Hassoc_dd : forall nm X, (forall k, n <= k -> nm <> setname k) ->
                     assoc_str nm (sd_sets dd'') X = assoc_str nm (sd_sets d) X).
           { intros nm X Hf.
             rewrite (optimize_rules_concatM_assoc_stable fuel (S n) dn rest' m'' dd'' rr'' nm X
                        (eq_sym Erec) (fun k Hk => Hf k ltac:(lia))).
             subst dn; cbn [sd_sets assoc_str].
             destruct (String.eqb nm (setname n)) eqn:Eq.
             - apply String.eqb_eq in Eq. exfalso. apply (Hf n (Nat.le_refl n) Eq).
           ...
           assert (Hlook : e_set (pkt_env (set_env p (env_with_sets base dd'')))
                             (setname n) = map pack_tuple tuples).
           { cbn [set_env with_pkt_env pkt_env]. rewrite e_set_declared.
             erewrite (optimize_rules_concatM_assoc_stable fuel (S n) dn rest' _ _ _
                         (setname n) _ (eq_sym Erec)).
             - subst dn; cbn [sd_sets assoc_str]. rewrite String.eqb_refl. reflexivity.
             - intros k Hk Heq. apply setname_inj in Heq. lia. }
           set (qd := set_env p (env_with_sets base dd'')) in *.
           ...
           { apply (concat_two_fields_certificate_N f1 f2 tuples (setname n) qd).
             ...
             - intros a b Hin Hld.
               ...
               apply (field_fixed_len_loaded f2 (Datatypes.length b) qd Hfx Hld). }
           transitivity (eval_rules
             (map (fun ab => orig_rule2g f1 f2 gm (fst ab) (snd ab) body r1) tuples ++ rr'') qd).
           { apply (eval_rules_run_collapse
                      (map (fun ab => orig_rule2g f1 f2 gm (fst ab) (snd ab) body r1) tuples)
                      (rule_loadable (merged_rule2g f1 f2 gm (setname n) body r1) qd)
                      (outcome (merged_rule2g f1 f2 gm (setname n) body r1) qd)
                      (merged_rule2g f1 f2 gm (setname n) body r1) rr'' qd).
             - subst tuples. discriminate.
             - intros r Hr. apply in_map_iff in Hr as [ab [Hab _]]. subst r.
             ...
             - rewrite merged_rule2g_applies. rewrite Hcert.
               rewrite existsb_map_eq.
               transitivity (existsb (fun ab =>
                   andb (andb (andb (eval_matchcond (MCmp f1 CEq (fst ab)) qd)
                                    (eval_matchcond gm qd))
                              (eval_matchcond (MCmp f2 CEq (snd ab)) qd))
                        (rule_applies_walk body qd)) tuples).
           ...
           { rewrite Htail. unfold qd.
             ...
             intros r Hr. apply decls_agree_rule_sym.
             apply (decls_agree_rule_setseam base dn dd'' r (S n)).
             - apply (optimize_rules_concatM_vmaps fuel (S n) dn rest' m'' dd'' rr'' (eq_sym Erec)).
             - apply (optimize_rules_concatM_maps fuel (S n) dn rest' m'' dd'' rr'' (eq_sym Erec)).
             - intros nm X Hf. apply (optimize_rules_concatM_assoc_stable fuel (S n) dn rest'
                                       m'' dd'' rr'' nm X (eq_sym Erec) Hf).
             - rewrite Forall_forall in Hrf_rest'. apply Hrf_rest'; exact Hr. }
           rewrite (eval_rules_app_cong
                      (map (fun ab => orig_rule2g f1 f2 gm (fst ab) (snd ab) body r1) tuples)
...
(** *** setg (the guarded single-field value->set pass, Optimize_Setg).  Mirrors
    [concatM] verbatim EXCEPT the single-field membership certificate is
    ...
    + rewrite optimize_rules_setg_consSS in H.
      ...
      * destruct (take_setg_run r1 (r2 :: rest)) as [vs rest'] eqn:Erun.
        ...
        -- remember (optimize_rules_setg fuel n d (r2 :: rest)) as tt eqn:Erec.
           ...
           assert (Hda1 : decls_agree_rule base dd'' d r1).
           { apply (decls_agree_rule_setseam base d dd'' r1 n).
             - apply (optimize_rules_setg_vmaps fuel n d (r2 :: rest) m'' dd'' rr'' (eq_sym Erec)).
             - apply (optimize_rules_setg_maps fuel n d (r2 :: rest) m'' dd'' rr'' (eq_sym Erec)).
             - intros nm X Hf. apply (optimize_rules_setg_assoc_stable fuel n d (r2 :: rest)
                                       m'' dd'' rr'' nm X (eq_sym Erec) Hf).
             - exact Hf1. }
           rewrite (rule_loadable_agree_gen r1 p base dd'' d Hda1).
           rewrite (rule_applies_agree_gen r1 p base dd'' d Hda1).
        ...
        -- cbv zeta in H.
           ...
           { eapply Forall_impl; [intros r Hr; apply (rule_set_fresh_mono n (S n) r); [lia | exact Hr] |].
             ...
             exact Hsub. }
           assert (Hvm_dd : sd_vmaps dd'' = sd_vmaps d).
           { rewrite (optimize_rules_setg_vmaps fuel (S n) dn rest' m'' dd'' rr'' (eq_sym Erec)).
             subst dn; reflexivity. }
           assert (Hmaps_dd : sd_maps dd'' = sd_maps d).
           { rewrite (optimize_rules_setg_maps fuel (S n) dn rest' m'' dd'' rr'' (eq_sym Erec)).
             subst dn; reflexivity. }
           assert (Hassoc_dd : forall nm X, (forall k, n <= k -> nm <> setname k) ->
                     assoc_str nm (sd_sets dd'') X = assoc_str nm (sd_sets d) X).
           { intros nm X Hf.
             rewrite (optimize_rules_setg_assoc_stable fuel (S n) dn rest' m'' dd'' rr'' nm X
                        (eq_sym Erec) (fun k Hk => Hf k ltac:(lia))).
             subst dn; cbn [sd_sets assoc_str].
             destruct (String.eqb nm (setname n)) eqn:Eq.
             - apply String.eqb_eq in Eq. exfalso. apply (Hf n (Nat.le_refl n) Eq).
           ...
           { cbn [set_env with_pkt_env pkt_env]. rewrite e_set_declared.
             erewrite (optimize_rules_setg_assoc_stable fuel (S n) dn rest' _ _ _
                         (setname n) _ (eq_sym Erec)).
             - subst dn; cbn [sd_sets assoc_str]. rewrite String.eqb_refl. reflexivity.
             - intros k Hk Heq. apply setname_inj in Heq. lia. }
           set (qd := set_env p (env_with_sets base dd'')) in *.
           assert (Hcert : eval_matchcond (MConcatSet [f] false (setname n)) qd
                   = existsb (fun v => eval_matchcond (MCmp f CEq v) qd) vals).
           { apply (concat_set_existsb f vals (setname n) qd).
             - exact Hlook.
             - intros v Hin Hld.
               ...
               { destruct Hin as [Hv | Hin].
                 ...
                 - apply (Hall v Hin). }
               apply (field_fixed_len_loaded f (Datatypes.length v) qd Hfx Hld). }
           transitivity (eval_rules
             (map (fun v => orig_ruleGs f gm v body r1) vals ++ rr'') qd).
           { apply (eval_rules_run_collapse
             ...
             - rewrite merged_ruleGs_applies. rewrite Hcert.
               ...
               rewrite (existsb_ext _ _ _ vals
                          (fun v (_ : In v vals) => orig_ruleGs_applies f gm v body r1 qd)).
               symmetry. apply existsb_guardhead_factor. }
           assert (Htail' : eval_rules rr'' qd = eval_rules rest' qd).
           { rewrite Htail. unfold qd.
             apply (eval_rules_agree_gen rest' p base dn dd'').
             intros r Hr. apply decls_agree_rule_sym.
             apply (decls_agree_rule_setseam base dn dd'' r (S n)).
             - apply (optimize_rules_setg_vmaps fuel (S n) dn rest' m'' dd'' rr'' (eq_sym Erec)).
             - apply (optimize_rules_setg_maps fuel (S n) dn rest' m'' dd'' rr'' (eq_sym Erec)).
             - intros nm X Hf. apply (optimize_rules_setg_assoc_stable fuel (S n) dn rest'
                                       m'' dd'' rr'' nm X (eq_sym Erec) Hf).
             - rewrite Forall_forall in Hrf_rest'. apply Hrf_rest'; exact Hr. }
           rewrite (eval_rules_app_cong
                      (map (fun v => orig_ruleGs f gm v body r1) vals)
  ...
  - rewrite optimize_rules_concatK_cons2 in H.
    ...
    + cbv zeta in H.
      ...
      { rewrite (optimize_rules_concatK_vmaps rest (S n) dn m'' dd'' rr'' (eq_sym Erec)).
        subst dn; reflexivity. }
      assert (Hmaps_dd : sd_maps dd'' = sd_maps d).
      { rewrite (optimize_rules_concatK_maps rest (S n) dn m'' dd'' rr'' (eq_sym Erec)).
        subst dn; reflexivity. }
      assert (Hassoc_dd : forall nm X, (forall k, n <= k -> nm <> setname k) ->
                assoc_str nm (sd_sets dd'') X = assoc_str nm (sd_sets d) X).
      { intros nm X Hf.
      ...
      { rewrite Htail. unfold qd. apply (eval_rules_agree_gen rest p base dn dd'').
        ...
        - apply (optimize_rules_concatK_maps rest (S n) dn m'' dd'' rr'' (eq_sym Erec)).
    ...
    + remember (optimize_rules_concatK n d (r2 :: rest)) as tt eqn:Erec.
      ...
      { apply (decls_agree_rule_setseam base d dd'' r1 n).
        ...
        - apply (optimize_rules_concatK_maps (r2 :: rest) n d m'' dd'' rr'' (eq_sym Erec)).
      ...
      * destruct (take_vmap_run r1 (r2 :: rest)) as [es rest'] eqn:Erun.
        ...
        -- remember (optimize_rules_vmapN fuel n d (r2 :: rest)) as tt eqn:Erec.
           ...
           { apply (decls_agree_rule_vmapseam base d dd'' r1 n).
             ...
             - apply (optimize_rules_vmapN_maps fuel n d (r2 :: rest) m'' dd'' rr'' (eq_sym Erec)).
        ...
        -- destruct (take_vmap_run_head r1 f v1 body r2 rest (e :: es') rest' Ehd Erun
           ...
           assert (Hmaps_dd : sd_maps dd'' = sd_maps dn)
             by (apply (optimize_rules_vmapN_maps _ _ _ _ _ _ _ (eq_sym Erec))).
           ...
           { rewrite Htail. unfold qd.
             ...
             - apply (optimize_rules_vmapN_maps fuel (S n) dn rest' m'' dd'' rr'' (eq_sym Erec)).
...
(** *** vmapNg.  The GUARDED verdict-map merge (Optimize_Vmapg), a verbatim mirror of
    the vmapN proof above EXCEPT the recogniser is [head_valueGs] (guard + selector),
    ...
    + rewrite optimize_rules_vmapNg_consSS in H.
      ...
      * destruct (take_vmapG_run r1 (r2 :: rest)) as [es rest'] eqn:Erun.
        ...
        -- remember (optimize_rules_vmapNg fuel n d (r2 :: rest)) as tt eqn:Erec.
           ...
           assert (Hda1 : decls_agree_rule base dd'' d r1).
           { apply (decls_agree_rule_vmapseam base d dd'' r1 n).
             - apply (optimize_rules_vmapNg_sets fuel n d (r2 :: rest) m'' dd'' rr'' (eq_sym Erec)).
             - apply (optimize_rules_vmapNg_maps fuel n d (r2 :: rest) m'' dd'' rr'' (eq_sym Erec)).
             - intros nm X Hf. apply (optimize_rules_vmapNg_assoc_stable fuel n d (r2 :: rest) m'' dd'' rr'' nm X (eq_sym Erec) Hf).
             - exact Hf1. }
           rewrite (rule_loadable_agree_gen r1 p base dd'' d Hda1).
           rewrite (rule_applies_agree_gen r1 p base dd'' d Hda1).
           rewrite (outcome_agree_gen r1 p base dd'' d Hda1). reflexivity.
        -- destruct (take_vmapG_run_head r1 gm f v1 body r2 rest (e :: es') rest' Ehd Erun
           ...
           { eapply Forall_impl; [intros r Hr; apply (rule_vmap_fresh_mono n (S n) r); [lia | exact Hr] |].
             exact Hrf_rest_n. }
           assert (Hsets_dd : sd_sets dd'' = sd_sets dn)
             by (apply (optimize_rules_vmapNg_sets _ _ _ _ _ _ _ (eq_sym Erec))).
           assert (Hsets_dn : sd_sets dn = sd_sets d) by (subst dn; reflexivity).
           assert (Hmaps_dd : sd_maps dd'' = sd_maps dn)
             by (apply (optimize_rules_vmapNg_maps _ _ _ _ _ _ _ (eq_sym Erec))).
           assert (Hmaps_dn : sd_maps dn = sd_maps d) by (subst dn; reflexivity).
           assert (Htail : eval_rules rr'' (set_env p (env_with_sets base dd''))
                           = eval_rules rest' (set_env p (env_with_sets base dn))).
           { eapply (IH rest' (S n) dn m'' dd'' rr'' base p (eq_sym Erec)); [| exact Hrf_rest'].
           ...
           assert (Hlook : e_vmap (pkt_env (set_env p (env_with_sets base dd'')))
                             (vmapname n) = map vmap_pt entries).
           { cbn [set_env with_pkt_env pkt_env]. rewrite e_vmap_env_with_sets.
             erewrite (optimize_rules_vmapNg_assoc_stable fuel (S n) dn _ _ _ _
                         (vmapname n) _ (eq_sym Erec)).
             - subst dn; cbn [sd_vmaps assoc_str]. rewrite String.eqb_refl. reflexivity.
             - intros k Hk Heq. apply vmapname_inj in Heq. lia. }
           set (qd := set_env p (env_with_sets base dd'')) in *.
           transitivity (eval_rules
             (map (fun vw => orig_ruleGv f gm (fst vw) body (snd vw)) entries ++ rr'') qd).
           { unfold qd. apply (eval_rules_vmap_mergeNg f gm (vmapname n) entries body rr''
                                 (set_env p (env_with_sets base dd''))).
             - exact Hlook.
             - intros v w Hin. subst entries. destruct Hin as [Hvw | Hin];
                 [ inversion Hvw; subst; exact HwK1 | apply (HwK v w Hin) ].
           ...
           { rewrite Htail. unfold qd.
             ...
             intros r Hr. apply decls_agree_rule_sym.
             apply (decls_agree_rule_vmapseam base dn dd'' r (S n)).
             - apply (optimize_rules_vmapNg_sets fuel (S n) dn rest' m'' dd'' rr'' (eq_sym Erec)).
             - apply (optimize_rules_vmapNg_maps fuel (S n) dn rest' m'' dd'' rr'' (eq_sym Erec)).
             - intros nm X Hf. apply (optimize_rules_vmapNg_assoc_stable fuel (S n) dn rest'
                                       m'' dd'' rr'' nm X (eq_sym Erec) Hf).
             - rewrite Forall_forall in Hrf_rest'. apply Hrf_rest'; exact Hr. }
           rewrite (eval_rules_app_cong
           ...
           ++ rewrite Hmaps_dd, Hmaps_dn; reflexivity.
           ++ intros nm X Hf.
              rewrite (optimize_rules_vmapNg_assoc_stable fuel (S n) dn rest' m'' dd'' rr'' nm X
                         (eq_sym Erec) (fun k Hk => Hf k ltac:(lia))).
              subst dn; cbn [sd_vmaps assoc_str].
              destruct (String.eqb nm (vmapname n)) eqn:Eq.
              ** apply String.eqb_eq in Eq. exfalso. apply (Hf n (Nat.le_refl n) Eq).
...
(** vmapNg output read-freshness (own [vmapname] namespace): the merged
...
Lemma optimize_rules_vmapNg_output_vmap_fresh : forall fuel n d rs n' d' rs',
  optimize_rules_vmapNg fuel n d rs = (n', d', rs') ->
  Forall (rule_vmap_fresh n) rs ->
  Forall (rule_vmap_fresh n') rs'.
Proof.
    ...
    + rewrite optimize_rules_vmapNg_consSS in H.
      ...
      * destruct (take_vmapG_run r1 (r2 :: rest)) as [es rest'] eqn:Erun.
        ...
        -- remember (optimize_rules_vmapNg fuel n d (r2 :: rest)) as tt eqn:Erec.
           ...
           injection H as Hn' Hd' Hr'. subst n' d' rs'.
           assert (Hmono : n <= m'')
             by (apply (optimize_rules_vmapNg_mono fuel n d (r2 :: rest) m'' dd'' rr'' (eq_sym Erec))).
        ...
        -- destruct (has_distinct_verdict (r_verdict r1) (e :: es') && body_vmap_safe body) eqn:Hdv.
           ...
           injection H as Hn' Hd' Hr'. subst n' d' rs'.
           assert (Hmono : S n <= m'')
             by (apply (optimize_rules_vmapNg_mono fuel (S n) _ rest' m'' dd'' rr'' (eq_sym Erec))).
           assert (Hrf_rest' : Forall (rule_vmap_fresh (S n)) rest').
           { eapply Forall_impl; [intros r Hr; apply (rule_vmap_fresh_mono n (S n) r); [lia | exact Hr] |].
             rewrite Hsplit in Hrf_tail. apply Forall_app in Hrf_tail. exact (proj2 Hrf_tail). }
           constructor.
...
Proof.
  ...
  - destruct rs as [| r1 [| r2 rest] ].
    + cbn in H. inversion H; subst; reflexivity.
    + cbn in H. inversion H; subst; reflexivity.
    + rewrite optimize_rules_dscpv_consSS in H.
      inversion Hrf as [| ? ? Hf1 Hrf_tail]; subst.
      destruct (head_dscp r1) as [[[[[f mask] xor] v1] body] |] eqn:Ehd.
      * destruct (take_dscpv_run r1 (r2 :: rest)) as [es rest'] eqn:Erun.
        ...
        { rewrite Hsplit in Hrf_tail. apply Forall_app in Hrf_tail. exact (proj2 Hrf_tail). }
        destruct es as [| e es'].
        -- remember (optimize_rules_dscpv fuel n d (r2 :: rest)) as tt eqn:Erec.
           destruct tt as [[m'' dd''] rr'']. cbv zeta in H.
           injection H as Hn' Hd' Hr'. subst n' d' rs'.
           cbn [eval_rules].
           rewrite (IH (r2 :: rest) n d m'' dd'' rr'' base p (eq_sym Erec) Hfresh Hrf_tail).
           assert (Hda1 : decls_agree_rule base dd'' d r1).
           { apply (decls_agree_rule_vmapseam base d dd'' r1 n).
             - apply (optimize_rules_dscpv_sets fuel n d (r2 :: rest) m'' dd'' rr'' (eq_sym Erec)).
             - apply (optimize_rules_dscpv_maps fuel n d (r2 :: rest) m'' dd'' rr'' (eq_sym Erec)).
             - intros nm X Hf. apply (optimize_rules_dscpv_assoc_stable fuel n d (r2 :: rest) m'' dd'' rr'' nm X (eq_sym Erec) Hf).
             - exact Hf1. }
           rewrite (rule_loadable_agree_gen r1 p base dd'' d Hda1).
           rewrite (rule_applies_agree_gen r1 p base dd'' d Hda1).
           rewrite (outcome_agree_gen r1 p base dd'' d Hda1). reflexivity.
        -- destruct (take_dscpv_run_head r1 f mask xor v1 body r2 rest (e :: es') rest' Ehd Erun
           ...
           destruct (has_distinct_verdict (r_verdict r1) (e :: es') && body_vmap_safe body) eqn:Hdv.
           2:{ remember (optimize_rules_dscpv fuel n d (r2 :: rest)) as tt eqn:Erec.
               destruct tt as [[m'' dd''] rr'']. cbv zeta in H.
               injection H as Hn' Hd' Hr'. subst n' d' rs'.
               cbn [eval_rules].
               rewrite (IH (r2 :: rest) n d m'' dd'' rr'' base p (eq_sym Erec) Hfresh Hrf_tail).
           ...
           apply Bool.negb_true_iff in Hns. apply Bool.negb_true_iff in Hnt.
           cbv zeta in H.
           remember (optimize_rules_dscpv fuel (S n)
                       {| sd_sets := sd_sets d;
                          sd_vmaps := (vmapname n,
                            map vmap_pt ((v1, r_verdict r1) :: e :: es')) :: sd_vmaps d;
                          sd_maps := sd_maps d |} rest')
           ...
           { eapply Forall_impl; [intros r Hr; apply (rule_vmap_fresh_mono n (S n) r); [lia | exact Hr] |].
             exact Hrf_rest_n. }
           assert (Hsets_dd : sd_sets dd'' = sd_sets dn)
             by (apply (optimize_rules_dscpv_sets _ _ _ _ _ _ _ (eq_sym Erec))).
           assert (Hsets_dn : sd_sets dn = sd_sets d) by (subst dn; reflexivity).
           assert (Hmaps_dd : sd_maps dd'' = sd_maps dn)
             by (apply (optimize_rules_dscpv_maps _ _ _ _ _ _ _ (eq_sym Erec))).
           assert (Hmaps_dn : sd_maps dn = sd_maps d) by (subst dn; reflexivity).
           assert (Htail : eval_rules rr'' (set_env p (env_with_sets base dd''))
                           = eval_rules rest' (set_env p (env_with_sets base dn))).
           { eapply (IH rest' (S n) dn m'' dd'' rr'' base p (eq_sym Erec)); [| exact Hrf_rest'].
           ...
           assert (Hlook : e_vmap (pkt_env (set_env p (env_with_sets base dd'')))
                             (vmapname n) = map vmap_pt entries).
           { cbn [set_env with_pkt_env pkt_env]. rewrite e_vmap_env_with_sets.
             erewrite (optimize_rules_dscpv_assoc_stable fuel (S n) dn _ _ _ _
                         (vmapname n) _ (eq_sym Erec)).
             - subst dn; cbn [sd_vmaps assoc_str]. rewrite String.eqb_refl. reflexivity.
             - intros k Hk Heq. apply vmapname_inj in Heq. lia. }
           set (qd := set_env p (env_with_sets base dd'')) in *.
           transitivity (eval_rules
             (map (fun vw => orig_rule_m f mask xor (fst vw) body (snd vw)) entries ++ rr'') qd).
           { unfold qd. apply (eval_rules_dscpv_mergeN f mask xor (vmapname n) entries body rr''
                                 (set_env p (env_with_sets base dd''))).
             - exact Hlook.
             - intros v w Hin. subst entries. destruct Hin as [Hvw | Hin];
                 [ inversion Hvw; subst; exact HwK1 | apply (HwK v w Hin) ].
           ...
           { rewrite Htail. unfold qd.
             ...
             intros r Hr. apply decls_agree_rule_sym.
             apply (decls_agree_rule_vmapseam base dn dd'' r (S n)).
             - apply (optimize_rules_dscpv_sets fuel (S n) dn rest' m'' dd'' rr'' (eq_sym Erec)).
             - apply (optimize_rules_dscpv_maps fuel (S n) dn rest' m'' dd'' rr'' (eq_sym Erec)).
             - intros nm X Hf. apply (optimize_rules_dscpv_assoc_stable fuel (S n) dn rest'
                                       m'' dd'' rr'' nm X (eq_sym Erec) Hf).
             - rewrite Forall_forall in Hrf_rest'. apply Hrf_rest'; exact Hr. }
           rewrite (eval_rules_app_cong
           ...
           ++ rewrite Hmaps_dd, Hmaps_dn; reflexivity.
           ++ intros nm X Hf.
              rewrite (optimize_rules_dscpv_assoc_stable fuel (S n) dn rest' m'' dd'' rr'' nm X
                         (eq_sym Erec) (fun k Hk => Hf k ltac:(lia))).
              subst dn; cbn [sd_vmaps assoc_str].
              destruct (String.eqb nm (vmapname n)) eqn:Eq.
              ** apply String.eqb_eq in Eq. exfalso. apply (Hf n (Nat.le_refl n) Eq).
...
(** dscpv output read-freshness (own [vmapname] namespace): the merged
...
Lemma optimize_rules_dscpv_output_vmap_fresh : forall fuel n d rs n' d' rs',
  optimize_rules_dscpv fuel n d rs = (n', d', rs') ->
  Forall (rule_vmap_fresh n) rs ->
  Forall (rule_vmap_fresh n') rs'.
Proof.
    ...
    + rewrite optimize_rules_dscpv_consSS in H.
      ...
      * destruct (take_dscpv_run r1 (r2 :: rest)) as [es rest'] eqn:Erun.
        ...
        -- remember (optimize_rules_dscpv fuel n d (r2 :: rest)) as tt eqn:Erec.
           ...
           injection H as Hn' Hd' Hr'. subst n' d' rs'.
           assert (Hmono : n <= m'')
             by (apply (optimize_rules_dscpv_mono fuel n d (r2 :: rest) m'' dd'' rr'' (eq_sym Erec))).
           constructor; [apply (rule_vmap_fresh_mono n m'' r1 Hmono Hf1)
                        | apply (IH n d (r2 :: rest) m'' dd'' rr'' (eq_sym Erec) Hrf_tail)].
        -- destruct (has_distinct_verdict (r_verdict r1) (e :: es') && body_vmap_safe body) eqn:Hdv.
           2:{ remember (optimize_rules_dscpv fuel n d (r2 :: rest)) as tt eqn:Erec.
           ...
           injection H as Hn' Hd' Hr'. subst n' d' rs'.
           assert (Hmono : S n <= m'')
             by (apply (optimize_rules_dscpv_mono fuel (S n) _ rest' m'' dd'' rr'' (eq_sym Erec))).
           assert (Hrf_rest' : Forall (rule_vmap_fresh (S n)) rest').
           { eapply Forall_impl; [intros r Hr; apply (rule_vmap_fresh_mono n (S n) r); [lia | exact Hr] |].
             rewrite Hsplit in Hrf_tail. apply Forall_app in Hrf_tail. exact (proj2 Hrf_tail). }
           constructor.
...
(** *** ivsett output-freshness (its own [setname] namespace + [vmap] pass-through). *)
...
Lemma optimize_rules_ivsett_output_set_fresh : forall fuel n d rs n' d' rs',
  optimize_rules_ivsett fuel n d rs = (n', d', rs') ->
  Forall (rule_set_fresh n) rs ->
  Forall (rule_set_fresh n') rs'.
Proof.
    ...
    + rewrite optimize_rules_ivsett_consSS in H.
      ...
      * destruct (take_ivsett_run r1 (r2 :: rest)) as [ivs rest'] eqn:Erun.
        ...
        -- remember (optimize_rules_ivsett fuel n d (r2 :: rest)) as t eqn:Erec.
           ...
           injection H as Hn' Hd' Hr'. subst n' d' rs'.
           assert (Hmono : n <= m'')
             by (apply (optimize_rules_ivsett_mono fuel n d (r2 :: rest) m'' dd'' rr'' (eq_sym Erec))).
           constructor.
           ++ apply (rule_set_fresh_mono n m'' r1 Hmono Hf1).
           ++ apply (IH n d (r2 :: rest) m'' dd'' rr'' (eq_sym Erec) Hrf_tail).
        -- cbv zeta in H.
           ...
           injection H as Hn' Hd' Hr'. subst n' d' rs'.
           assert (Hmono : S n <= m'')
             by (apply (optimize_rules_ivsett_mono fuel (S n) _ rest' m'' dd'' rr'' (eq_sym Erec))).
           assert (Hrf_rest' : Forall (rule_set_fresh (S n)) rest').
           { eapply Forall_impl; [intros r Hr; apply (rule_set_fresh_mono n (S n) r); [lia | exact Hr] |].
             rewrite Hsplit in Hrf_tail. apply Forall_app in Hrf_tail. exact (proj2 Hrf_tail). }
           constructor.
...
Lemma optimize_rules_ivsett_output_vmap_fresh : forall fuel n d rs n' d' rs',
  optimize_rules_ivsett fuel n d rs = (n', d', rs') ->
  Forall (rule_vmap_fresh n) rs ->
  Forall (rule_vmap_fresh n') rs'.
Proof.
...
(** *** ivset output-freshness (its own [setname] namespace + [vmap] pass-through). *)
Lemma optimize_rules_ivset_output_set_fresh : forall fuel n d rs n' d' rs',
  optimize_rules_ivset fuel n d rs = (n', d', rs') ->
  Forall (rule_set_fresh n) rs ->
  Forall (rule_set_fresh n') rs'.
Proof.
    ...
    + rewrite optimize_rules_ivset_consSS in H.
      ...
      * destruct (take_range_run r1 (r2 :: rest)) as [ivs rest'] eqn:Erun.
        ...
        -- remember (optimize_rules_ivset fuel n d (r2 :: rest)) as t eqn:Erec.
           ...
           injection H as Hn' Hd' Hr'. subst n' d' rs'.
           assert (Hmono : n <= m'')
             by (apply (optimize_rules_ivset_mono fuel n d (r2 :: rest) m'' dd'' rr'' (eq_sym Erec))).
           constructor.
           ++ apply (rule_set_fresh_mono n m'' r1 Hmono Hf1).
           ++ apply (IH n d (r2 :: rest) m'' dd'' rr'' (eq_sym Erec) Hrf_tail).
        -- cbv zeta in H.
           ...
           injection H as Hn' Hd' Hr'. subst n' d' rs'.
           assert (Hmono : S n <= m'')
             by (apply (optimize_rules_ivset_mono fuel (S n) _ rest' m'' dd'' rr'' (eq_sym Erec))).
           assert (Hrf_rest' : Forall (rule_set_fresh (S n)) rest').
           { eapply Forall_impl; [intros r Hr; apply (rule_set_fresh_mono n (S n) r); [lia | exact Hr] |].
             rewrite Hsplit in Hrf_tail. apply Forall_app in Hrf_tail. exact (proj2 Hrf_tail). }
           constructor.
...
Lemma optimize_rules_ivset_output_vmap_fresh : forall fuel n d rs n' d' rs',
  optimize_rules_ivset fuel n d rs = (n', d', rs') ->
  Forall (rule_vmap_fresh n) rs ->
  Forall (rule_vmap_fresh n') rs'.
Proof.
...
Lemma optimize_rules_setsN_output_nat_map_fresh : forall fuel n d rs n' d' rs',
  optimize_rules_setsN fuel n d rs = (n', d', rs') ->
  Forall (rule_nat_map_fresh n) rs ->
  Forall (rule_nat_map_fresh n') rs'.
Proof.
...
(** *** concatM output-freshness (its own [setname] namespace + [vmap] pass-through). *)
...
Lemma optimize_rules_concatM_output_set_fresh : forall fuel n d rs n' d' rs',
  optimize_rules_concatM fuel n d rs = (n', d', rs') ->
  Forall (rule_set_fresh n) rs ->
  Forall (rule_set_fresh n') rs'.
Proof.
    ...
    + rewrite optimize_rules_concatM_consSS in H.
      ...
      * destruct (take_concatg_run r1 (r2 :: rest)) as [ts rest'] eqn:Erun.
        ...
        -- remember (optimize_rules_concatM fuel n d (r2 :: rest)) as tt eqn:Erec.
           ...
           injection H as Hn' Hd' Hr'. subst n' d' rs'.
           assert (Hmono : n <= m'')
             by (apply (optimize_rules_concatM_mono fuel n d (r2 :: rest) m'' dd'' rr'' (eq_sym Erec))).
           constructor; [apply (rule_set_fresh_mono n m'' r1 Hmono Hf1)
                        | apply (IH n d (r2 :: rest) m'' dd'' rr'' (eq_sym Erec) Hrf_tail)].
        -- cbv zeta in H.
           remember (optimize_rules_concatM fuel (S n)
           ...
           injection H as Hn' Hd' Hr'. subst n' d' rs'.
           assert (Hmono : S n <= m'')
             by (apply (optimize_rules_concatM_mono fuel (S n) _ rest' m'' dd'' rr'' (eq_sym Erec))).
           assert (Hrf_rest' : Forall (rule_set_fresh (S n)) rest').
           { eapply Forall_impl; [intros r Hr; apply (rule_set_fresh_mono n (S n) r); [lia | exact Hr] |].
             rewrite Hsplit in Hrf_tail. apply Forall_app in Hrf_tail. exact (proj2 Hrf_tail). }
           constructor.
...
Lemma optimize_rules_concatM_output_vmap_fresh : forall fuel n d rs n' d' rs',
  optimize_rules_concatM fuel n d rs = (n', d', rs') ->
  Forall (rule_vmap_fresh n) rs ->
  Forall (rule_vmap_fresh n') rs'.
Proof.
...
(** *** setg output-freshness (its own [setname] namespace + [vmap] pass-through). *)
...
Lemma optimize_rules_setg_output_set_fresh : forall fuel n d rs n' d' rs',
  optimize_rules_setg fuel n d rs = (n', d', rs') ->
  Forall (rule_set_fresh n) rs ->
  Forall (rule_set_fresh n') rs'.
Proof.
    ...
    + rewrite optimize_rules_setg_consSS in H.
      ...
      * destruct (take_setg_run r1 (r2 :: rest)) as [vs rest'] eqn:Erun.
        ...
        -- remember (optimize_rules_setg fuel n d (r2 :: rest)) as tt eqn:Erec.
           ...
           injection H as Hn' Hd' Hr'. subst n' d' rs'.
           assert (Hmono : n <= m'')
             by (apply (optimize_rules_setg_mono fuel n d (r2 :: rest) m'' dd'' rr'' (eq_sym Erec))).
           constructor; [apply (rule_set_fresh_mono n m'' r1 Hmono Hf1)
                        | apply (IH n d (r2 :: rest) m'' dd'' rr'' (eq_sym Erec) Hrf_tail)].
        -- cbv zeta in H.
           remember (optimize_rules_setg fuel (S n)
           ...
           injection H as Hn' Hd' Hr'. subst n' d' rs'.
           assert (Hmono : S n <= m'')
             by (apply (optimize_rules_setg_mono fuel (S n) _ rest' m'' dd'' rr'' (eq_sym Erec))).
           assert (Hrf_rest' : Forall (rule_set_fresh (S n)) rest').
           { eapply Forall_impl; [intros r Hr; apply (rule_set_fresh_mono n (S n) r); [lia | exact Hr] |].
             rewrite Hsplit in Hrf_tail. apply Forall_app in Hrf_tail. exact (proj2 Hrf_tail). }
           constructor.
...
Lemma optimize_rules_setg_output_vmap_fresh : forall fuel n d rs n' d' rs',
  optimize_rules_setg fuel n d rs = (n', d', rs') ->
  Forall (rule_vmap_fresh n) rs ->
  Forall (rule_vmap_fresh n') rs'.
Proof.
...
(** *** ivsetg propagates freshness (its own [setname] namespace; adds NO vmap name).
    The guarded range head [MRange] contributes NO set name, so — like [setg] — the
...
Lemma optimize_rules_ivsetg_output_set_fresh : forall fuel n d rs n' d' rs',
  optimize_rules_ivsetg fuel n d rs = (n', d', rs') ->
  Forall (rule_set_fresh n) rs ->
  Forall (rule_set_fresh n') rs'.
Proof.
    ...
    + rewrite optimize_rules_ivsetg_consSS in H.
      ...
      * destruct (take_rangeg_run r1 (r2 :: rest)) as [ivs rest'] eqn:Erun.
        ...
        -- remember (optimize_rules_ivsetg fuel n d (r2 :: rest)) as tt eqn:Erec.
           ...
           injection H as Hn' Hd' Hr'. subst n' d' rs'.
           assert (Hmono : n <= m'')
             by (apply (optimize_rules_ivsetg_mono fuel n d (r2 :: rest) m'' dd'' rr'' (eq_sym Erec))).
           constructor; [apply (rule_set_fresh_mono n m'' r1 Hmono Hf1)
                        | apply (IH n d (r2 :: rest) m'' dd'' rr'' (eq_sym Erec) Hrf_tail)].
        -- cbv zeta in H.
           remember (optimize_rules_ivsetg fuel (S n)
           ...
           injection H as Hn' Hd' Hr'. subst n' d' rs'.
           assert (Hmono : S n <= m'')
             by (apply (optimize_rules_ivsetg_mono fuel (S n) _ rest' m'' dd'' rr'' (eq_sym Erec))).
           assert (Hrf_rest' : Forall (rule_set_fresh (S n)) rest').
           { eapply Forall_impl; [intros r Hr; apply (rule_set_fresh_mono n (S n) r); [lia | exact Hr] |].
             rewrite Hsplit in Hrf_tail. apply Forall_app in Hrf_tail. exact (proj2 Hrf_tail). }
           constructor.
...
Lemma optimize_rules_ivsetg_output_vmap_fresh : forall fuel n d rs n' d' rs',
  optimize_rules_ivsetg fuel n d rs = (n', d', rs') ->
  Forall (rule_vmap_fresh n) rs ->
  Forall (rule_vmap_fresh n') rs'.
Proof.
...
(** *** ivmixg propagates freshness (own [setname] namespace; adds NO vmap name). *)
...
Lemma optimize_rules_ivmixg_output_set_fresh : forall fuel n d rs n' d' rs',
  optimize_rules_ivmixg fuel n d rs = (n', d', rs') ->
  Forall (rule_set_fresh n) rs ->
  Forall (rule_set_fresh n') rs'.
Proof.
    ...
    + rewrite optimize_rules_ivmixg_consSS in H.
      ...
      * destruct (take_mix_run r1 (r2 :: rest)) as [es rest'] eqn:Erun.
        ...
        -- remember (optimize_rules_ivmixg fuel n d (r2 :: rest)) as tt eqn:Erec.
           ...
           injection H as Hn' Hd' Hr'. subst n' d' rs'.
           assert (Hmono : n <= m'')
             by (apply (optimize_rules_ivmixg_mono fuel n d (r2 :: rest) m'' dd'' rr'' (eq_sym Erec))).
           constructor; [apply (rule_set_fresh_mono n m'' r1 Hmono Hf1)
                        | apply (IH n d (r2 :: rest) m'' dd'' rr'' (eq_sym Erec) Hrf_tail)].
        -- cbv zeta in H.
           remember (optimize_rules_ivmixg fuel (S n)
           ...
           injection H as Hn' Hd' Hr'. subst n' d' rs'.
           assert (Hmono : S n <= m'')
             by (apply (optimize_rules_ivmixg_mono fuel (S n) _ rest' m'' dd'' rr'' (eq_sym Erec))).
           assert (Hrf_rest' : Forall (rule_set_fresh (S n)) rest').
           { eapply Forall_impl; [intros r Hr; apply (rule_set_fresh_mono n (S n) r); [lia | exact Hr] |].
             rewrite Hsplit in Hrf_tail. apply Forall_app in Hrf_tail. exact (proj2 Hrf_tail). }
           constructor.
...
Lemma optimize_rules_ivmixg_output_vmap_fresh : forall fuel n d rs n' d' rs',
  optimize_rules_ivmixg fuel n d rs = (n', d', rs') ->
  Forall (rule_vmap_fresh n) rs ->
  Forall (rule_vmap_fresh n') rs'.
Proof.
...
(** *** vmapNg (Optimize_Vmapg) chain-level wrappers. *)
Lemma optimize_chain_vmapNg_correct_uncond : forall n d c n' d' c' base p,
  ...
  = eval_chain c  (set_env p (env_with_sets base d)).
Proof.
  intros n d c n' d' c' base p H Hfv Hrf. unfold optimize_chain_vmapNg in H.
  destruct (optimize_rules_vmapNg (Datatypes.length (c_rules c)) n d (c_rules c))
    as [[m'' dd''] rr''] eqn:E.
  inversion H; subst n' d' c'. unfold eval_chain. cbn [c_rules c_policy].
  rewrite (optimize_rules_vmapNg_correct_uncond (Datatypes.length (c_rules c)) (c_rules c) n d
             m'' dd'' rr'' base p E Hfv Hrf). reflexivity.
Qed.

Lemma optimize_chain_vmapNg_output_vmap_fresh : forall n d c n' d' c',
  optimize_chain_vmapNg n d c = (n', d', c') ->
  Forall (rule_vmap_fresh n) (c_rules c) -> Forall (rule_vmap_fresh n') (c_rules c').
Proof.
  intros n d c n' d' c' H Hrf. unfold optimize_chain_vmapNg in H.
  destruct (optimize_rules_vmapNg (Datatypes.length (c_rules c)) n d (c_rules c)) as [[m'' dd''] rr''] eqn:E.
  inversion H; subst. cbn [c_rules].
  apply (optimize_rules_vmapNg_output_vmap_fresh _ _ _ _ _ _ _ E Hrf).
Qed.

(** vmapNg mints [vmapname]s bounded by [n']; thread vmapname-decl-freshness past
    the stage into the following vmapN stage. *)
Lemma optimize_chain_vmapNg_fresh_vmapname : forall n d c n' d' c',
  optimize_chain_vmapNg n d c = (n', d', c') ->
  (forall k, n <= k -> ~ In (vmapname k) (map fst (sd_vmaps d))) ->
  (forall k, n' <= k -> ~ In (vmapname k) (map fst (sd_vmaps d'))).
Proof.
  intros n d c n' d' c' H Hfresh k Hk Hin.
  pose proof (optimize_chain_vmapNg_mono n d c n' d' c' H) as Hmono.
  destruct (optimize_chain_vmapNg_keys_bound n d c n' d' c' k H Hin) as [Hin_d | Hlt].
  - apply (Hfresh k); [lia | exact Hin_d].
  - lia.
Qed.

(** *** dscpv (Optimize_Dscpv) chain-level wrappers. *)
Lemma optimize_chain_dscpv_correct_uncond : forall n d c n' d' c' base p,
  optimize_chain_dscpv n d c = (n', d', c') ->
  (forall k, n <= k -> ~ In (vmapname k) (map fst (sd_vmaps d))) ->
  Forall (rule_vmap_fresh n) (c_rules c) ->
  eval_chain c' (set_env p (env_with_sets base d'))
  = eval_chain c  (set_env p (env_with_sets base d)).
Proof.
  intros n d c n' d' c' base p H Hfv Hrf. unfold optimize_chain_dscpv in H.
  destruct (optimize_rules_dscpv (Datatypes.length (c_rules c)) n d (c_rules c))
    as [[m'' dd''] rr''] eqn:E.
  inversion H; subst n' d' c'. unfold eval_chain. cbn [c_rules c_policy].
  rewrite (optimize_rules_dscpv_correct_uncond (Datatypes.length (c_rules c)) (c_rules c) n d
             m'' dd'' rr'' base p E Hfv Hrf). reflexivity.
Qed.

Lemma optimize_chain_dscpv_output_vmap_fresh : forall n d c n' d' c',
  optimize_chain_dscpv n d c = (n', d', c') ->
  Forall (rule_vmap_fresh n) (c_rules c) -> Forall (rule_vmap_fresh n') (c_rules c').
Proof.
  intros n d c n' d' c' H Hrf. unfold optimize_chain_dscpv in H.
  destruct (optimize_rules_dscpv (Datatypes.length (c_rules c)) n d (c_rules c)) as [[m'' dd''] rr''] eqn:E.
  inversion H; subst. cbn [c_rules].
  apply (optimize_rules_dscpv_output_vmap_fresh _ _ _ _ _ _ _ E Hrf).
Qed.

Lemma optimize_chain_dscpv_fresh_vmapname : forall n d c n' d' c',
  optimize_chain_dscpv n d c = (n', d', c') ->
  (forall k, n <= k -> ~ In (vmapname k) (map fst (sd_vmaps d))) ->
  (forall k, n' <= k -> ~ In (vmapname k) (map fst (sd_vmaps d'))).
Proof.
  intros n d c n' d' c' H Hfresh k Hk Hin.
  pose proof (optimize_chain_dscpv_mono n d c n' d' c' H) as Hmono.
  destruct (optimize_chain_dscpv_keys_bound n d c n' d' c' k H Hin) as [Hin_d | Hlt].
  - apply (Hfresh k); [lia | exact Hin_d].
  - lia.
Qed.
...
Lemma optimize_chain_setsN_output_nat_map_fresh : forall n d c n' d' c',
  optimize_chain_setsN n d c = (n', d', c') ->
  Forall (rule_nat_map_fresh n) (c_rules c) -> Forall (rule_nat_map_fresh n') (c_rules c').
Proof.
  intros n d c n' d' c' H Hrf. unfold optimize_chain_setsN in H.
  destruct (optimize_rules_setsN (Datatypes.length (c_rules c)) n d (c_rules c)) as [[m'' dd''] rr''] eqn:E.
  inversion H; subst. cbn [c_rules].
  apply (optimize_rules_setsN_output_nat_map_fresh _ _ _ _ _ _ _ E Hrf).
Qed.
...
(** concatN preserves [setname]-freshness (mints names bounded by [n']) — threads
    freshness past concatN into the following concatM stage. *)
Lemma optimize_chain_concatN_fresh_setname : forall n d c n' d' c',
  optimize_chain_concatN n d c = (n', d', c') ->
  (forall k, n <= k -> ~ In (setname k) (map fst (sd_sets d))) ->
  (forall k, n' <= k -> ~ In (setname k) (map fst (sd_sets d'))).
Proof.
  intros n d c n' d' c' H Hfresh k Hk Hin.
  pose proof (optimize_chain_concatN_mono n d c n' d' c' H) as Hmono.
  destruct (optimize_chain_concatN_keys_bound n d c n' d' c' k H Hin) as [Hin_d | Hlt].
  - apply (Hfresh k); [lia | exact Hin_d].
  - lia.
Qed.

(** *** concatM chain-level wrappers. *)
Lemma optimize_chain_concatM_correct_uncond : forall n d c n' d' c' base p,
  ...
  = eval_chain c  (set_env p (env_with_sets base d)).
Proof.
  intros n d c n' d' c' base p H Hfs Hrf. unfold optimize_chain_concatM in H.
  destruct (optimize_rules_concatM (Datatypes.length (c_rules c)) n d (c_rules c))
    as [[m'' dd''] rr''] eqn:E.
  inversion H; subst n' d' c'. unfold eval_chain. cbn [c_rules c_policy].
  rewrite (optimize_rules_concatM_correct_uncond (Datatypes.length (c_rules c)) (c_rules c) n d
             m'' dd'' rr'' base p E Hfs Hrf). reflexivity.
Qed.

Lemma optimize_chain_concatM_output_set_fresh : forall n d c n' d' c',
  optimize_chain_concatM n d c = (n', d', c') ->
  Forall (rule_set_fresh n) (c_rules c) -> Forall (rule_set_fresh n') (c_rules c').
Proof.
  intros n d c n' d' c' H Hrf. unfold optimize_chain_concatM in H.
  destruct (optimize_rules_concatM (Datatypes.length (c_rules c)) n d (c_rules c)) as [[m'' dd''] rr''] eqn:E.
  inversion H; subst. cbn [c_rules].
  apply (optimize_rules_concatM_output_set_fresh _ _ _ _ _ _ _ E Hrf).
Qed.

Lemma optimize_chain_concatM_output_vmap_fresh : forall n d c n' d' c',
  optimize_chain_concatM n d c = (n', d', c') ->
  Forall (rule_vmap_fresh n) (c_rules c) -> Forall (rule_vmap_fresh n') (c_rules c').
Proof.
  intros n d c n' d' c' H Hrf. unfold optimize_chain_concatM in H.
  destruct (optimize_rules_concatM (Datatypes.length (c_rules c)) n d (c_rules c)) as [[m'' dd''] rr''] eqn:E.
  inversion H; subst. cbn [c_rules].
  apply (optimize_rules_concatM_output_vmap_fresh _ _ _ _ _ _ _ E Hrf).
Qed.

(** concatM mints [setname]s bounded by [n']; thread setname-freshness past concatM
    into the following ivset stage. *)
Lemma optimize_chain_concatM_fresh_setname : forall n d c n' d' c',
  optimize_chain_concatM n d c = (n', d', c') ->
  (forall k, n <= k -> ~ In (setname k) (map fst (sd_sets d))) ->
  (forall k, n' <= k -> ~ In (setname k) (map fst (sd_sets d'))).
Proof.
  intros n d c n' d' c' H Hfresh k Hk Hin.
  pose proof (optimize_chain_concatM_mono n d c n' d' c' H) as Hmono.
  destruct (optimize_chain_concatM_keys_bound n d c n' d' c' k H Hin) as [Hin_d | Hlt].
  - apply (Hfresh k); [lia | exact Hin_d].
  - lia.
Qed.

(** *** setg chain-level wrappers. *)
Lemma optimize_chain_setg_correct_uncond : forall n d c n' d' c' base p,
  ...
  = eval_chain c  (set_env p (env_with_sets base d)).
Proof.
  intros n d c n' d' c' base p H Hfs Hrf. unfold optimize_chain_setg in H.
  destruct (optimize_rules_setg (Datatypes.length (c_rules c)) n d (c_rules c))
    as [[m'' dd''] rr''] eqn:E.
  inversion H; subst n' d' c'. unfold eval_chain. cbn [c_rules c_policy].
  rewrite (optimize_rules_setg_correct_uncond (Datatypes.length (c_rules c)) (c_rules c) n d
             m'' dd'' rr'' base p E Hfs Hrf). reflexivity.
Qed.

Lemma optimize_chain_setg_output_set_fresh : forall n d c n' d' c',
  optimize_chain_setg n d c = (n', d', c') ->
  Forall (rule_set_fresh n) (c_rules c) -> Forall (rule_set_fresh n') (c_rules c').
Proof.
  intros n d c n' d' c' H Hrf. unfold optimize_chain_setg in H.
  destruct (optimize_rules_setg (Datatypes.length (c_rules c)) n d (c_rules c)) as [[m'' dd''] rr''] eqn:E.
  inversion H; subst. cbn [c_rules].
  apply (optimize_rules_setg_output_set_fresh _ _ _ _ _ _ _ E Hrf).
Qed.

Lemma optimize_chain_setg_output_vmap_fresh : forall n d c n' d' c',
  optimize_chain_setg n d c = (n', d', c') ->
  Forall (rule_vmap_fresh n) (c_rules c) -> Forall (rule_vmap_fresh n') (c_rules c').
Proof.
  intros n d c n' d' c' H Hrf. unfold optimize_chain_setg in H.
  destruct (optimize_rules_setg (Datatypes.length (c_rules c)) n d (c_rules c)) as [[m'' dd''] rr''] eqn:E.
  inversion H; subst. cbn [c_rules].
  apply (optimize_rules_setg_output_vmap_fresh _ _ _ _ _ _ _ E Hrf).
Qed.

Lemma optimize_chain_setg_fresh_setname : forall n d c n' d' c',
  optimize_chain_setg n d c = (n', d', c') ->
  (forall k, n <= k -> ~ In (setname k) (map fst (sd_sets d))) ->
  (forall k, n' <= k -> ~ In (setname k) (map fst (sd_sets d'))).
Proof.
  intros n d c n' d' c' H Hfresh k Hk Hin.
  pose proof (optimize_chain_setg_mono n d c n' d' c' H) as Hmono.
  destruct (optimize_chain_setg_keys_bound n d c n' d' c' k H Hin) as [Hin_d | Hlt].
  - apply (Hfresh k); [lia | exact Hin_d].
  - lia.
Qed.

(** *** ivset chain-level wrappers. *)
Lemma optimize_chain_ivset_correct_uncond : forall n d c n' d' c' base p,
  optimize_chain_ivset n d c = (n', d', c') ->
  (forall k, n <= k -> ~ In (setname k) (map fst (sd_sets d))) ->
  Forall (rule_set_fresh n) (c_rules c) ->
  eval_chain c' (set_env p (env_with_sets base d'))
  = eval_chain c  (set_env p (env_with_sets base d)).
Proof.
  intros n d c n' d' c' base p H Hfs Hrf. unfold optimize_chain_ivset in H.
  destruct (optimize_rules_ivset (Datatypes.length (c_rules c)) n d (c_rules c))
    as [[m'' dd''] rr''] eqn:E.
  inversion H; subst n' d' c'. unfold eval_chain. cbn [c_rules c_policy].
  rewrite (optimize_rules_ivset_correct_uncond (Datatypes.length (c_rules c)) (c_rules c) n d
             m'' dd'' rr'' base p E Hfs Hrf). reflexivity.
Qed.

Lemma optimize_chain_ivset_output_vmap_fresh : forall n d c n' d' c',
  optimize_chain_ivset n d c = (n', d', c') ->
  Forall (rule_vmap_fresh n) (c_rules c) -> Forall (rule_vmap_fresh n') (c_rules c').
Proof.
  intros n d c n' d' c' H Hrf. unfold optimize_chain_ivset in H.
  destruct (optimize_rules_ivset (Datatypes.length (c_rules c)) n d (c_rules c)) as [[m'' dd''] rr''] eqn:E.
  inversion H; subst. cbn [c_rules].
  apply (optimize_rules_ivset_output_vmap_fresh _ _ _ _ _ _ _ E Hrf).
Qed.

Lemma optimize_chain_ivset_fresh_setname : forall n d c n' d' c',
  optimize_chain_ivset n d c = (n', d', c') ->
  (forall k, n <= k -> ~ In (setname k) (map fst (sd_sets d))) ->
  (forall k, n' <= k -> ~ In (setname k) (map fst (sd_sets d'))).
Proof.
  intros n d c n' d' c' H Hfresh k Hk Hin.
  pose proof (optimize_chain_ivset_mono n d c n' d' c' H) as Hmono.
  destruct (optimize_chain_ivset_keys_bound n d c n' d' c' k H Hin) as [Hin_d | Hlt].
  - apply (Hfresh k); [lia | exact Hin_d].
  - lia.
Qed.

Lemma optimize_chain_ivset_output_set_fresh : forall n d c n' d' c',
  optimize_chain_ivset n d c = (n', d', c') ->
  Forall (rule_set_fresh n) (c_rules c) -> Forall (rule_set_fresh n') (c_rules c').
Proof.
  intros n d c n' d' c' H Hrf. unfold optimize_chain_ivset in H.
  destruct (optimize_rules_ivset (Datatypes.length (c_rules c)) n d (c_rules c)) as [[m'' dd''] rr''] eqn:E.
  inversion H; subst. cbn [c_rules].
  apply (optimize_rules_ivset_output_set_fresh _ _ _ _ _ _ _ E Hrf).
Qed.
...
(** *** ivsett chain-level wrappers (host-order interval-set pass). *)
Lemma optimize_chain_ivsett_correct_uncond : forall n d c n' d' c' base p,
  optimize_chain_ivsett n d c = (n', d', c') ->
  (forall k, n <= k -> ~ In (setname k) (map fst (sd_sets d))) ->
  Forall (rule_set_fresh n) (c_rules c) ->
  eval_chain c' (set_env p (env_with_sets base d'))
  = eval_chain c  (set_env p (env_with_sets base d)).
Proof.
  intros n d c n' d' c' base p H Hfs Hrf. unfold optimize_chain_ivsett in H.
  destruct (optimize_rules_ivsett (Datatypes.length (c_rules c)) n d (c_rules c))
    as [[m'' dd''] rr''] eqn:E.
  inversion H; subst n' d' c'. unfold eval_chain. cbn [c_rules c_policy].
  rewrite (optimize_rules_ivsett_correct_uncond (Datatypes.length (c_rules c)) (c_rules c) n d
             m'' dd'' rr'' base p E Hfs Hrf). reflexivity.
Qed.

Lemma optimize_chain_ivsett_output_vmap_fresh : forall n d c n' d' c',
  optimize_chain_ivsett n d c = (n', d', c') ->
  Forall (rule_vmap_fresh n) (c_rules c) -> Forall (rule_vmap_fresh n') (c_rules c').
Proof.
  intros n d c n' d' c' H Hrf. unfold optimize_chain_ivsett in H.
  destruct (optimize_rules_ivsett (Datatypes.length (c_rules c)) n d (c_rules c)) as [[m'' dd''] rr''] eqn:E.
  inversion H; subst. cbn [c_rules].
  apply (optimize_rules_ivsett_output_vmap_fresh _ _ _ _ _ _ _ E Hrf).
Qed.

Lemma optimize_chain_ivsett_fresh_setname : forall n d c n' d' c',
  optimize_chain_ivsett n d c = (n', d', c') ->
  (forall k, n <= k -> ~ In (setname k) (map fst (sd_sets d))) ->
  (forall k, n' <= k -> ~ In (setname k) (map fst (sd_sets d'))).
Proof.
  intros n d c n' d' c' H Hfresh k Hk Hin.
  pose proof (optimize_chain_ivsett_mono n d c n' d' c' H) as Hmono.
  destruct (optimize_chain_ivsett_keys_bound n d c n' d' c' k H Hin) as [Hin_d | Hlt].
  - apply (Hfresh k); [lia | exact Hin_d].
  - lia.
Qed.

Lemma optimize_chain_ivsett_output_set_fresh : forall n d c n' d' c',
  optimize_chain_ivsett n d c = (n', d', c') ->
  Forall (rule_set_fresh n) (c_rules c) -> Forall (rule_set_fresh n') (c_rules c').
Proof.
  intros n d c n' d' c' H Hrf. unfold optimize_chain_ivsett in H.
  destruct (optimize_rules_ivsett (Datatypes.length (c_rules c)) n d (c_rules c)) as [[m'' dd''] rr''] eqn:E.
  inversion H; subst. cbn [c_rules].
  apply (optimize_rules_ivsett_output_set_fresh _ _ _ _ _ _ _ E Hrf).
Qed.

(** *** ivsetg chain-level wrappers (guarded interval-set pass). *)
Lemma optimize_chain_ivsetg_correct_uncond : forall n d c n' d' c' base p,
  ...
  = eval_chain c  (set_env p (env_with_sets base d)).
Proof.
  intros n d c n' d' c' base p H Hfs Hrf. unfold optimize_chain_ivsetg in H.
  destruct (optimize_rules_ivsetg (Datatypes.length (c_rules c)) n d (c_rules c))
    as [[m'' dd''] rr''] eqn:E.
  inversion H; subst n' d' c'. unfold eval_chain. cbn [c_rules c_policy].
  rewrite (optimize_rules_ivsetg_correct_uncond (Datatypes.length (c_rules c)) (c_rules c) n d
             m'' dd'' rr'' base p E Hfs Hrf). reflexivity.
Qed.

Lemma optimize_chain_ivsetg_output_set_fresh : forall n d c n' d' c',
  optimize_chain_ivsetg n d c = (n', d', c') ->
  Forall (rule_set_fresh n) (c_rules c) -> Forall (rule_set_fresh n') (c_rules c').
Proof.
  intros n d c n' d' c' H Hrf. unfold optimize_chain_ivsetg in H.
  destruct (optimize_rules_ivsetg (Datatypes.length (c_rules c)) n d (c_rules c)) as [[m'' dd''] rr''] eqn:E.
  inversion H; subst. cbn [c_rules].
  apply (optimize_rules_ivsetg_output_set_fresh _ _ _ _ _ _ _ E Hrf).
Qed.

Lemma optimize_chain_ivsetg_output_vmap_fresh : forall n d c n' d' c',
  optimize_chain_ivsetg n d c = (n', d', c') ->
  Forall (rule_vmap_fresh n) (c_rules c) -> Forall (rule_vmap_fresh n') (c_rules c').
Proof.
  intros n d c n' d' c' H Hrf. unfold optimize_chain_ivsetg in H.
  destruct (optimize_rules_ivsetg (Datatypes.length (c_rules c)) n d (c_rules c)) as [[m'' dd''] rr''] eqn:E.
  inversion H; subst. cbn [c_rules].
  apply (optimize_rules_ivsetg_output_vmap_fresh _ _ _ _ _ _ _ E Hrf).
Qed.

Lemma optimize_chain_ivsetg_fresh_setname : forall n d c n' d' c',
  optimize_chain_ivsetg n d c = (n', d', c') ->
  (forall k, n <= k -> ~ In (setname k) (map fst (sd_sets d))) ->
  (forall k, n' <= k -> ~ In (setname k) (map fst (sd_sets d'))).
Proof.
  intros n d c n' d' c' H Hfresh k Hk Hin.
  pose proof (optimize_chain_ivsetg_mono n d c n' d' c' H) as Hmono.
  destruct (optimize_chain_ivsetg_keys_bound n d c n' d' c' k H Hin) as [Hin_d | Hlt].
  - apply (Hfresh k); [lia | exact Hin_d].
  - lia.
Qed.

(** *** ivmixg chain-level wrappers (guarded mixed point+range set pass). *)
Lemma optimize_chain_ivmixg_correct_uncond : forall n d c n' d' c' base p,
  optimize_chain_ivmixg n d c = (n', d', c') ->
  (forall k, n <= k -> ~ In (setname k) (map fst (sd_sets d))) ->
  Forall (rule_set_fresh n) (c_rules c) ->
  eval_chain c' (set_env p (env_with_sets base d'))
  = eval_chain c  (set_env p (env_with_sets base d)).
Proof.
  intros n d c n' d' c' base p H Hfs Hrf. unfold optimize_chain_ivmixg in H.
  destruct (optimize_rules_ivmixg (Datatypes.length (c_rules c)) n d (c_rules c))
    as [[m'' dd''] rr''] eqn:E.
  inversion H; subst n' d' c'. unfold eval_chain. cbn [c_rules c_policy].
  rewrite (optimize_rules_ivmixg_correct_uncond (Datatypes.length (c_rules c)) (c_rules c) n d
             m'' dd'' rr'' base p E Hfs Hrf). reflexivity.
Qed.

Lemma optimize_chain_ivmixg_output_set_fresh : forall n d c n' d' c',
  optimize_chain_ivmixg n d c = (n', d', c') ->
  Forall (rule_set_fresh n) (c_rules c) -> Forall (rule_set_fresh n') (c_rules c').
Proof.
  intros n d c n' d' c' H Hrf. unfold optimize_chain_ivmixg in H.
  destruct (optimize_rules_ivmixg (Datatypes.length (c_rules c)) n d (c_rules c)) as [[m'' dd''] rr''] eqn:E.
  inversion H; subst. cbn [c_rules].
  apply (optimize_rules_ivmixg_output_set_fresh _ _ _ _ _ _ _ E Hrf).
Qed.

Lemma optimize_chain_ivmixg_output_vmap_fresh : forall n d c n' d' c',
  optimize_chain_ivmixg n d c = (n', d', c') ->
  Forall (rule_vmap_fresh n) (c_rules c) -> Forall (rule_vmap_fresh n') (c_rules c').
Proof.
  intros n d c n' d' c' H Hrf. unfold optimize_chain_ivmixg in H.
  destruct (optimize_rules_ivmixg (Datatypes.length (c_rules c)) n d (c_rules c)) as [[m'' dd''] rr''] eqn:E.
  inversion H; subst. cbn [c_rules].
  apply (optimize_rules_ivmixg_output_vmap_fresh _ _ _ _ _ _ _ E Hrf).
Qed.

Lemma optimize_chain_ivmixg_fresh_setname : forall n d c n' d' c',
  optimize_chain_ivmixg n d c = (n', d', c') ->
  (forall k, n <= k -> ~ In (setname k) (map fst (sd_sets d))) ->
  (forall k, n' <= k -> ~ In (setname k) (map fst (sd_sets d'))).
Proof.
  intros n d c n' d' c' H Hfresh k Hk Hin.
  pose proof (optimize_chain_ivmixg_mono n d c n' d' c' H) as Hmono.
  destruct (optimize_chain_ivmixg_keys_bound n d c n' d' c' k H Hin) as [Hin_d | Hlt].
  - apply (Hfresh k); [lia | exact Hin_d].
  - lia.
Qed.
...
Lemma optimize_rules_concatK_output_nat_map_fresh : forall rs n d n' d' rs',
  optimize_rules_concatK n d rs = (n', d', rs') ->
  Forall (rule_nat_map_fresh n) rs ->
  Forall (rule_nat_map_fresh n') rs'.
Proof.
...
Lemma optimize_chain_concatK_output_nat_map_fresh : forall n d c n' d' c',
  optimize_chain_concatK n d c = (n', d', c') ->
  Forall (rule_nat_map_fresh n) (c_rules c) -> Forall (rule_nat_map_fresh n') (c_rules c').
Proof.
  intros n d c n' d' c' H Hrf. unfold optimize_chain_concatK in H.
  destruct (optimize_rules_concatK n d (c_rules c)) as [[m'' dd''] rr''] eqn:E.
  inversion H; subst. cbn [c_rules].
  apply (optimize_rules_concatK_output_nat_map_fresh _ _ _ _ _ _ E Hrf).
Qed.
...
(* ================================================================== *)
(** *** mapN stage (the data-value-map merge, Optimize_Mapn).  Its VERDICT effect is
    ENV-INDEPENDENT (the merged Continue rules fall through for any env), so the
    chain correctness is the env-indep [optimize_rules_mapn_eval] composed with the
...
Lemma optimize_rules_mapn_output_set_fresh : forall rs n d n' d' rs',
...
Proof.
  ...
  - rewrite optimize_rules_mapn_cons2 in H.
    ...
    + remember (optimize_rules_mapn (S n)
        ...
        as t eqn:E.
      destruct t as [[m'' dd''] rr'']. injection H as Hn Hd Hr; subst n' d' rs'.
      pose proof (optimize_rules_mapn_mono rest (S n) _ _ _ _ (eq_sym E)) as Hmono.
      constructor.
    ...
    + remember (optimize_rules_mapn n d (r2 :: rest)) as t eqn:E.
      destruct t as [[m'' dd''] rr'']. injection H as Hn Hd Hr; subst n' d' rs'.
      pose proof (optimize_rules_mapn_mono (r2 :: rest) n d _ _ _ (eq_sym E)) as Hmono.
      constructor;
        [apply (rule_set_fresh_mono n m'' r1 Hmono Hf1)
        |apply (IH (r2 :: rest) ltac:(unfold ltof; cbn; lia) _ _ _ _ _ (eq_sym E));
           constructor; assumption].
...
Lemma optimize_rules_mapn_output_vmap_fresh : forall rs n d n' d' rs',
  optimize_rules_mapn n d rs = (n', d', rs') ->
  Forall (rule_vmap_fresh n) rs -> Forall (rule_vmap_fresh n') rs'.
Proof.
  induction rs as [rs IH] using (induction_ltof1 _ (@List.length rule)).
...
Lemma optimize_chain_mapn_correct_uncond : forall n d c n' d' c' base p,
  ...
  = eval_chain c  (set_env p (env_with_sets base d)).
Proof.
  intros n d c n' d' c' base p H Hrf Hrfm. unfold optimize_chain_mapn in H.
  destruct (optimize_rules_mapn n d (c_rules c)) as [[m'' dd''] rr''] eqn:E.
  inversion H; subst n' d' c'. unfold eval_chain. cbn [c_rules c_policy].
  rewrite (optimize_rules_mapn_eval (c_rules c) n d m'' dd'' rr''
             (set_env p (env_with_sets base dd'')) E).
  erewrite (eval_rules_agree_gen (c_rules c) p base dd'' d); [reflexivity|].
  intros r Hr. apply (decls_agree_rule_mapseam base d dd'' r n).
  - apply (optimize_rules_mapn_vmaps (c_rules c) n d m'' dd'' rr'' E).
  - intros nm X Hf. apply (optimize_rules_mapn_assoc_stable (c_rules c) n d m'' dd'' rr'' nm X E Hf).
  - intros nm X Hf. apply (optimize_rules_mapn_maps_assoc_stable (c_rules c) n d m'' dd'' rr'' nm X E Hf).
  - rewrite Forall_forall in Hrf. apply Hrf; exact Hr.
  - rewrite Forall_forall in Hrfm. apply Hrfm; exact Hr.
Qed.

Lemma optimize_chain_mapn_output_set_fresh : forall n d c n' d' c',
  optimize_chain_mapn n d c = (n', d', c') ->
  Forall (rule_set_fresh n) (c_rules c) -> Forall (rule_set_fresh n') (c_rules c').
Proof.
  intros n d c n' d' c' H Hrf. unfold optimize_chain_mapn in H.
  destruct (optimize_rules_mapn n d (c_rules c)) as [[m'' dd''] rr''] eqn:E.
  inversion H; subst. cbn [c_rules].
  apply (optimize_rules_mapn_output_set_fresh _ _ _ _ _ _ E Hrf).
Qed.
Lemma optimize_chain_mapn_output_vmap_fresh : forall n d c n' d' c',
  optimize_chain_mapn n d c = (n', d', c') ->
  Forall (rule_vmap_fresh n) (c_rules c) -> Forall (rule_vmap_fresh n') (c_rules c').
Proof.
  intros n d c n' d' c' H Hrf. unfold optimize_chain_mapn in H.
  destruct (optimize_rules_mapn n d (c_rules c)) as [[m'' dd''] rr''] eqn:E.
  inversion H; subst. cbn [c_rules].
  apply (optimize_rules_mapn_output_vmap_fresh _ _ _ _ _ _ E Hrf).
Qed.
Lemma optimize_chain_mapn_fresh_setname : forall n d c n' d' c',
  optimize_chain_mapn n d c = (n', d', c') ->
  (forall k, n <= k -> ~ In (setname k) (map fst (sd_sets d))) ->
  (forall k, n' <= k -> ~ In (setname k) (map fst (sd_sets d'))).
Proof.
  intros n d c n' d' c' H Hfresh k Hk Hin.
  pose proof (optimize_chain_mapn_mono n d c n' d' c' H) as Hmono.
  destruct (optimize_chain_mapn_keys_bound n d c n' d' c' k H Hin) as [Hin_d | Hlt].
  - apply (Hfresh k); [lia | exact Hin_d].
  - lia.
Qed.
...
Proof.
  ...
  destruct (optimize_chain_concatM n2 d2 c2) as [[nG dG] cG] eqn:EG.
  destruct (optimize_chain_setg nG dG cG) as [[nGs dGs] cGs] eqn:EGs.
  destruct (optimize_chain_ivset nGs dGs cGs) as [[nI dI] cI] eqn:EI.
  destruct (optimize_chain_ivsett nI dI cI) as [[nIt dIt] cIt] eqn:EIt.
  destruct (optimize_chain_dscp nIt dIt cIt) as [[nDs dDs] cDs] eqn:EDs.
  destruct (optimize_chain_ivsetg nDs dDs cDs) as [[nIg dIg] cIg] eqn:EIg.
  destruct (optimize_chain_ivmixg nIg dIg cIg) as [[nMx dMx] cMx] eqn:EMx.
  destruct (optimize_chain_vmapNg nMx dMx cMx) as [[nVg dVg] cVg] eqn:EVg.
  destruct (optimize_chain_dscpv nVg dVg cVg) as [[nDv dDv] cDv] eqn:EDv.
  ...
  pose proof (optimize_chain_concatM_mono n2 d2 c2 nG dG cG EG) as HmnG.
  pose proof (optimize_chain_setg_mono nG dG cG nGs dGs cGs EGs) as HmnGs.
  pose proof (optimize_chain_ivset_mono nGs dGs cGs nI dI cI EI) as HmnI.
  pose proof (optimize_chain_ivsett_mono nI dI cI nIt dIt cIt EIt) as HmnIt.
  pose proof (optimize_chain_dscp_mono nIt dIt cIt nDs dDs cDs EDs) as HmnDs.
  pose proof (optimize_chain_ivsetg_mono nDs dDs cDs nIg dIg cIg EIg) as HmnIg.
  pose proof (optimize_chain_ivmixg_mono nIg dIg cIg nMx dMx cMx EMx) as HmnMx.
  (* setname-freshness threading through setsN, concatK, mapn, concatN (all mint setnames) *)
  ...
  pose proof (optimize_chain_concatN_fresh_setname nM dM cM n2 d2 c2 E2 HfsM) as Hfs2.
  pose proof (optimize_chain_concatM_fresh_setname n2 d2 c2 nG dG cG EG Hfs2) as HfsG.
  pose proof (optimize_chain_setg_fresh_setname nG dG cG nGs dGs cGs EGs HfsG) as HfsGs.
  (* sd_vmaps unchanged across setsN, concatK, mapn, concatN, concatM, setg, ivset *)
  assert (HvmG : sd_vmaps dG = sd_vmaps dS).
  { rewrite (optimize_chain_concatM_vmaps n2 d2 c2 nG dG cG EG).
  ...
  { rewrite (optimize_chain_setg_vmaps nG dG cG nGs dGs cGs EGs). exact HvmG. }
  assert (HvmI : sd_vmaps dI = sd_vmaps dS).
  { rewrite (optimize_chain_ivset_vmaps nGs dGs cGs nI dI cI EI). exact HvmGs. }
  assert (HvmIt : sd_vmaps dIt = sd_vmaps dS).
  { rewrite (optimize_chain_ivsett_vmaps nI dI cI nIt dIt cIt EIt). exact HvmI. }
  ...
  { rewrite (optimize_chain_ivsetg_vmaps nDs dDs cDs nIg dIg cIg EIg). exact HvmDs. }
  ...
  { rewrite (optimize_chain_ivmixg_vmaps nIg dIg cIg nMx dMx cMx EMx). exact HvmIg. }
  assert (HfvMx : forall k, nMx <= k -> ~ In (vmapname k) (map fst (sd_vmaps dMx))).
  { intros k Hk. rewrite HvmMx. apply Hfv_S. lia. }
  (* read-freshness threading: setsN -> concatK -> mapn -> concatN -> concatM *)
  ...
  pose proof (optimize_chain_mapn_output_vmap_fresh nK dK cK nM dM cM EM HrvK) as HrvM.
  pose proof (optimize_chain_concatN_output_set_fresh nM dM cM n2 d2 c2 E2 HrsM) as Hrs2.
  pose proof (optimize_chain_concatN_output_vmap_fresh nM dM cM n2 d2 c2 E2 HrvM) as Hrv2.
  pose proof (optimize_chain_concatM_output_vmap_fresh n2 d2 c2 nG dG cG EG Hrv2) as HrvG.
  pose proof (optimize_chain_concatM_output_set_fresh n2 d2 c2 nG dG cG EG Hrs2) as HrsG.
  pose proof (optimize_chain_setg_output_vmap_fresh nG dG cG nGs dGs cGs EGs HrvG) as HrvGs.
  pose proof (optimize_chain_setg_output_set_fresh nG dG cG nGs dGs cGs EGs HrsG) as HrsGs.
  pose proof (optimize_chain_ivset_output_vmap_fresh nGs dGs cGs nI dI cI EI HrvGs) as HrvI.
  pose proof (optimize_chain_ivset_fresh_setname nGs dGs cGs nI dI cI EI HfsGs) as HfsI.
  pose proof (optimize_chain_ivset_output_set_fresh nGs dGs cGs nI dI cI EI HrsGs) as HrsI.
  (* ivsett stage (host-order interval->set): mints [setname]s, preserves sd_vmaps,
     threads setname-decl-freshness + rule_set/vmap-freshness into dscp. *)
  pose proof (optimize_chain_ivsett_output_vmap_fresh nI dI cI nIt dIt cIt EIt HrvI) as HrvIt.
  pose proof (optimize_chain_ivsett_fresh_setname nI dI cI nIt dIt cIt EIt HfsI) as HfsIt.
  pose proof (optimize_chain_ivsett_output_set_fresh nI dI cI nIt dIt cIt EIt HrsI) as HrsIt.
  (* dscp stage (masked-payload value->set): mints [setname]s, preserves sd_vmaps,
     threads setname-decl-freshness + rule_set/vmap-freshness into ivsetg. *)
  ...
  pose proof (optimize_chain_ivsetg_output_vmap_fresh nDs dDs cDs nIg dIg cIg EIg HrvDs) as HrvIg.
  (* ivsetg setname-decl-freshness + rule_set_fresh, threaded into the ivmixg stage. *)
  pose proof (optimize_chain_ivsetg_fresh_setname nDs dDs cDs nIg dIg cIg EIg HfsDs) as HfsIg.
  pose proof (optimize_chain_ivsetg_output_set_fresh nDs dDs cDs nIg dIg cIg EIg HrsDs) as HrsIg.
  pose proof (optimize_chain_ivmixg_output_vmap_fresh nIg dIg cIg nMx dMx cMx EMx HrvIg) as HrvMx.
  (* vmapNg stage (guarded value+verdict->vmap): mints [vmapname]s onto sd_vmaps,
  ...
  rewrite (optimize_chain_vmapN_correct_uncond nDv dDv cDv n' d' c' base p H HfvDv HrvDv).
  rewrite (optimize_chain_dscpv_correct_uncond nVg dVg cVg nDv dDv cDv base p EDv HfvVg HrvVg).
  rewrite (optimize_chain_vmapNg_correct_uncond nMx dMx cMx nVg dVg cVg base p EVg HfvMx HrvMx).
  rewrite (optimize_chain_ivmixg_correct_uncond nIg dIg cIg nMx dMx cMx base p EMx HfsIg HrsIg).
  rewrite (optimize_chain_ivsetg_correct_uncond nDs dDs cDs nIg dIg cIg base p EIg HfsDs HrsDs).
  rewrite (optimize_chain_dscp_correct_uncond nIt dIt cIt nDs dDs cDs base p EDs HfsIt HrsIt).
  rewrite (optimize_chain_ivsett_correct_uncond nI dI cI nIt dIt cIt base p EIt HfsI HrsI).
  rewrite (optimize_chain_ivset_correct_uncond nGs dGs cGs nI dI cI base p EI HfsGs HrsGs).
  rewrite (optimize_chain_setg_correct_uncond nG dG cG nGs dGs cGs base p EGs HfsG HrsG).
  rewrite (optimize_chain_concatM_correct_uncond n2 d2 c2 nG dG cG base p EG Hfs2 Hrs2).
