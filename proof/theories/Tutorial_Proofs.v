...
(** ** The headline: the chain drops a packet IFF its source is in the range.
    ...
    bounds (the packet actually carries an IPv4 header).  NOTE there is no
    [pkt_env] hypothesis: the verdict is independent of conntrack/set state. *)
...
Theorem tutorial_accepts_rest : forall (p : packet) (a b c d : nat),
  fieldof FIp4Saddr p === ip4 a b c d ->
  read_payload_ok PNetwork 12 4 p = true ->
  ( tutorial_input accepts p under tutorial_chains budget tut_fuel
    <-> ~ (a = 192 /\ b = 168 /\ c = 100) ).
Proof.
  intros p a b c d Hs Hok.
  assert (Hok3 : read_payload_ok PNetwork 12 3 p = true)
    by (apply read_payload_ok_shorter with (w := 4); [lia | exact Hok]).
  eapply iff_trans.
  { apply (nft_prefix_chain_accept_iff 15 tutorial_chains _ _ _ _ _ Hok3). }
  rewrite (nft_saddr_prefix 3) by lia.
  unfold nft_field_is in Hs. rewrite Hs. cbn.
...
Example mk_tut_saddr : forall a b c d,
  fieldof FIp4Saddr (mk_tut a b c d) === ip4 a b c d.
...
Example tut_blocked_192_168_100_7 :
  tutorial_input denies (mk_tut 192 168 100 7)
    under tutorial_chains budget tut_fuel.
Proof. nft_decide. Qed.

Example tut_passes_192_168_101_7 :
  tutorial_input accepts (mk_tut 192 168 101 7)
    under tutorial_chains budget tut_fuel.
Proof. nft_decide. Qed.

Example tut_passes_8_8_8_8 :
  tutorial_input accepts (mk_tut 8 8 8 8)
    under tutorial_chains budget tut_fuel.
Proof. nft_decide. Qed.
...
Example tut_next_slash24_not_blocked :
  ~ (tutorial_input denies (mk_tut 192 168 101 7)
       under tutorial_chains budget tut_fuel).
