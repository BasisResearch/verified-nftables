(** * Optimize_Ivmixg: GUARDED MIXED point+range set consolidation.

    [Optimize_Setg] folds a GUARDED run of POINT heads [MCmp f CEq v]; [Optimize_Ivsetg]
    ...
    [setg] stops at the first range (its [value_mergeGs_pair] rejects the [MRange]
    head), and [ivsetg] never starts (its [head_rangeGr] fails on the leading [MCmp]).
    ...
    [Optimize_Ivsetg]) and each HEAD_i is EITHER a point compare [MCmp f CEq v]
    ...
    [Optimize_Setg.merged_ruleGs] VERBATIM.

    Verdict-preservation combines three certificates:
      - [Optimize_Ivset.concat_set_ivs_existsb]: a single-field [MConcatSet] over an
      ...
      - [eval_melem_mrange] (this file): each element's ORIGINAL head match equals the
        ...
        point-set pass uses);
      - [Optimize_Setg.existsb_guardhead_factor]: the shared head guard factors out of
        the run's [existsb].
    ...
    [Optimize_Ivsetg.all_disjoint].  A point that falls inside (or touches) another
...
From Nft Require Import Bytes Packet Verdict Syntax Bytecode Semantics
  Optimize Optimize_Merge Optimize_Concat Optimize_ConcatM Optimize_Setg Optimize_Ivset Optimize_Ivsetg.
...
(** Two guarded rules form an eligible MIX-merge pair iff their heads are
    ...
    run gate, not here — mirroring [Optimize_Ivsetg.range_mergeGr_pair] +
    [take_rangeg_run]. *)
...
Fixpoint optimize_rules_ivmixg (fuel : nat) (n : nat) (d : set_decls) (rs : list rule)
  ...
  | S fuel' =>
    ...
    | r1 :: ((_ :: _) as rest) =>
        ...
        | Some (gm, f, e1, body) =>
            ...
            | ([], _) =>
                let '(n'', d'', rest') := optimize_rules_ivmixg fuel' n d rest in
                (n'', d'', r1 :: rest')
            | ((_ :: _) as es, rest') =>
                ...
                let '(n'', d'', rest'') := optimize_rules_ivmixg fuel' (S n) d' rest' in
                (n'', d'', merged :: rest'')
            end
        | None =>
            let '(n'', d'', rest') := optimize_rules_ivmixg fuel' n d rest in
...
Definition optimize_chain_ivmixg (n : nat) (d : set_decls) (c : chain)
  : nat * set_decls * chain :=
  let '(n', d', rs') := optimize_rules_ivmixg (length (c_rules c)) n d (c_rules c) in
  (n', d', {| c_policy := c_policy c; c_rules := rs' |}).

Lemma optimize_rules_ivmixg_consSS : forall fuel n d r1 r2 rest,
  optimize_rules_ivmixg (S fuel) n d (r1 :: r2 :: rest) =
  match head_mixGm r1 with
  | Some (gm, f, e1, body) =>
      match take_mix_run r1 (r2 :: rest) with
      | ([], _) =>
          let '(n'', d'', rest') := optimize_rules_ivmixg fuel n d (r2 :: rest) in
          (n'', d'', r1 :: rest')
      | ((_ :: _) as es, rest') =>
          let name := setname n in
          let elems := map melem_iv (e1 :: es) in
...
Lemma optimize_rules_ivmixg_mono : forall fuel n d rs n' d' rs',
  optimize_rules_ivmixg fuel n d rs = (n', d', rs') -> n <= n'.
Proof.
  ...
  - destruct rs as [| r1 [| r2 rest] ].
    ...
    + rewrite optimize_rules_ivmixg_consSS in H.
      ...
      * destruct (take_mix_run r1 (r2 :: rest)) as [es rest'] eqn:Erun.
        ...
        -- cbv zeta in H.
           remember (optimize_rules_ivmixg fuel (S n) _ rest') as tt eqn:Erec.
...
Lemma optimize_rules_ivmixg_vmaps : forall fuel n d rs n' d' rs',
  optimize_rules_ivmixg fuel n d rs = (n', d', rs') -> sd_vmaps d' = sd_vmaps d.
Proof.
  induction fuel as [| fuel IH]; intros n d rs n' d' rs' H.
  - cbn in H. inversion H; subst; reflexivity.
  - destruct rs as [| r1 [| r2 rest] ].
...
Lemma optimize_rules_ivmixg_maps : forall fuel n d rs n' d' rs',
  optimize_rules_ivmixg fuel n d rs = (n', d', rs') -> sd_maps d' = sd_maps d.
Proof.
  induction fuel as [| fuel IH]; intros n d rs n' d' rs' H.
  - cbn in H. inversion H; subst; reflexivity.
  - destruct rs as [| r1 [| r2 rest] ].
...
Lemma optimize_rules_ivmixg_keys_bound : forall fuel n d rs n' d' rs' k,
        ...
        -- cbv zeta in H.
           ...
           ++ cbn [sd_sets map] in Hin_dn. destruct Hin_dn as [Heq | Hin_d].
              ** apply setname_inj in Heq. subst k. right.
                 pose proof (optimize_rules_ivmixg_mono fuel (S n) _ rest' m'' dd'' rr'' (eq_sym Erec)). lia.
              ** left; exact Hin_d.
           ++ right; exact Hlt.
      * remember (optimize_rules_ivmixg fuel n d (r2 :: rest)) as tt eqn:Erec.
        destruct tt as [[m'' dd''] rr'']. cbv zeta in H. injection H as Hn' Hd' Hr'.
...
Lemma optimize_rules_ivmixg_assoc_stable : forall fuel n d rs n' d' rs' nm X,
  optimize_rules_ivmixg fuel n d rs = (n', d', rs') ->
...
Lemma optimize_chain_ivmixg_mono : forall n d c n' d' c',
  optimize_chain_ivmixg n d c = (n', d', c') -> n <= n'.
Proof.
  intros n d c n' d' c' H. unfold optimize_chain_ivmixg in H.
  destruct (optimize_rules_ivmixg (length (c_rules c)) n d (c_rules c))
    as [[m'' dd''] rr''] eqn:E.
  inversion H; subst. apply (optimize_rules_ivmixg_mono _ _ _ _ _ _ _ E).
Qed.

Lemma optimize_chain_ivmixg_vmaps : forall n d c n' d' c',
  optimize_chain_ivmixg n d c = (n', d', c') -> sd_vmaps d' = sd_vmaps d.
Proof.
  intros n d c n' d' c' H. unfold optimize_chain_ivmixg in H.
  destruct (optimize_rules_ivmixg (length (c_rules c)) n d (c_rules c))
    as [[m'' dd''] rr''] eqn:E.
  inversion H; subst. apply (optimize_rules_ivmixg_vmaps _ _ _ _ _ _ _ E).
Qed.

Lemma optimize_chain_ivmixg_maps : forall n d c n' d' c',
  optimize_chain_ivmixg n d c = (n', d', c') -> sd_maps d' = sd_maps d.
Proof.
  intros n d c n' d' c' H. unfold optimize_chain_ivmixg in H.
  destruct (optimize_rules_ivmixg (length (c_rules c)) n d (c_rules c))
    as [[m'' dd''] rr''] eqn:E.
  inversion H; subst. apply (optimize_rules_ivmixg_maps _ _ _ _ _ _ _ E).
Qed.

Lemma optimize_chain_ivmixg_keys_bound : forall n d c n' d' c' k,
  optimize_chain_ivmixg n d c = (n', d', c') ->
  In (setname k) (map fst (sd_sets d')) ->
  In (setname k) (map fst (sd_sets d)) \/ k < n'.
Proof.
  intros n d c n' d' c' k H Hin. unfold optimize_chain_ivmixg in H.
  destruct (optimize_rules_ivmixg (length (c_rules c)) n d (c_rules c))
    as [[m'' dd''] rr''] eqn:E.
  inversion H; subst n' d' c'.
  apply (optimize_rules_ivmixg_keys_bound _ _ _ _ _ _ _ k E Hin).
Qed.
