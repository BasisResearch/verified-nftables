(* Regression for the iif/oif numeric-index fix.
   ...
   The parser now lowers `iif`/`oif` with kind KIfindex, encoding the RHS as the
   4-byte little-endian (host-endian on x86) index — NOT the ASCII name bytes
   (which remain correct for `iifname`/`oifname`).

   This file proves the fix on the REAL parser output: Optiplex_Gen's
   ...
   ASCII-meta packet (the bug the previous lowering exhibited). *)
...
(* A packet that genuinely arrived on lo (numeric iif index 1) MATCHES `iif lo`.
   Under the old ASCII lowering (MEq FMetaIif [108;111]) this was FALSE. *)
...
(* The impossible ASCII-meta packet (iif = ASCII "lo") no longer matches:
   the model now compares against the numeric index, not the name string. *)
