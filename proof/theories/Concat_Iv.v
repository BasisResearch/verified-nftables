...
(* ** Register-slot padding for a SUB-4-BYTE NON-LAST field (this round's fix).
   ...
   The model now splits the stored element by the 4-byte SLOT widths, so fed the
   kernel's actual element bytes it returns the kernel's verdict.  The OLD spec
   (split by RAW field widths [1;4], last-takes-remainder) instead expected the
   contiguous 5-byte layout [00][12 34 56 78] and REJECTED the kernel's padded
   element — a provable verdict divergence.  These theorems pin the fix. *)
...
(* THE FIX: the model now ACCEPTS the kernel-faithful padded element (the kernel
