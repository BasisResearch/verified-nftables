(* Regression gate: a single positive `tcp flags X` stays an implicit bitmask
   membership test ((flags & X) != 0), never an exact equality.
...
Definition m_syn_refuted : matchcond := MEq FTcpFlags [2].
...
Theorem refuted_meq_rejects_synack :
  eval_matchcond m_syn_refuted e0 (pkt_tcpflags 18) = false.
Proof. vm_compute. reflexivity. Qed.
...
Theorem refuted_alternative_diverges :
  eval_matchcond m_syn e0 (pkt_tcpflags 18)
    <> eval_matchcond m_syn_refuted e0 (pkt_tcpflags 18).
