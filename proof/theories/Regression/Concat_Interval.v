(* Regression gate: a concatenated interval set matches FIELD-BY-FIELD, as the
   cross-product of the per-field intervals.

   A concatenated set (NFT_SET_CONCAT) is matched FIELD-BY-FIELD: the kernel is
   given each field's length (NFTA_SET_FIELD_LEN, nf_tables.h:369-373) and the
   ...
   slot.)  Reference 2-D range bytecode tests/py/inet/sets.t.payload.inet:32:
...
Theorem flat_lex_wrongly_accepts : set_mem key_bad the_set = true.
