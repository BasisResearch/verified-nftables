(** * Correct: the compiler is semantics preserving — the theorem strata.
    ...
    restatements in [theories/Compiler/Main.v].
      ...
      1. [compile_chain_flat_verdict_correct]     — one base chain with in-traversal
                                           mutation (meta/ct set, dynset
                                           learning).
      2. [compile_chain_flat_env_correct] — + the env the chain LEAVES.
      3. [compile_seq_flat_verdict_correct]       — + threading that env across a
...
Lemma run_program_flat_env_compile_chain : forall rs e p,
  forallb rule_numgen_free rs = true ->
  run_program_flat_env h (map compile_rule rs) e p = eval_rules_flat_env h rs e p.
Proof.
  ...
  cbn [map]. rewrite ?run_program_flat_env_cons, ?run_program_flat_env_nil, ?eval_rules_flat_env_cons, ?eval_rules_flat_env_nil.
...
Theorem compile_chain_flat_env_correct : forall c e p,
  forallb rule_numgen_free (c_rules c) = true ->
  run_chain_flat_env h (compile_chain c) (c_policy c) e p = eval_chain_flat_env h c e p.
Proof.
  intros c e p Hall. unfold run_chain_flat_env, eval_chain_flat_env, compile_chain.
  rewrite run_program_flat_env_compile_chain by exact Hall. reflexivity.
Qed.

(** ** Stratum 2: verdict-only mutation preservation — the [fst] projection of
    stratum 3 ([eval_rules_flat_env_fst]/[run_program_flat_env_fst]), NOT a
    duplicated induction. *)
Lemma run_program_flat_verdict_compile_chain : forall rs e p,
  forallb rule_numgen_free rs = true ->
  run_program_flat_verdict h (map compile_rule rs) e p = eval_rules_flat_verdict h rs e p.
Proof.
  intros rs e p Hall.
  rewrite <- run_program_flat_env_fst, <- eval_rules_flat_env_fst.
  rewrite (run_program_flat_env_compile_chain rs e p Hall). reflexivity.
Qed.

Theorem compile_chain_flat_verdict_correct : forall c e p,
  forallb rule_numgen_free (c_rules c) = true ->
  run_chain_flat_verdict h (compile_chain c) (c_policy c) e p = eval_chain_flat_verdict h c e p.
Proof.
  intros c e p Hall. unfold run_chain_flat_verdict, eval_chain_flat_verdict, compile_chain.
  rewrite run_program_flat_verdict_compile_chain by exact Hall. reflexivity.
Qed.
...
(** ** Stratum 4 (HEADLINE, mutation/sequence axis): stateful cross-packet
    ...
    ([eval_chain_flat_env h]), never by an external caller-supplied step; the
    control-flow-complete form is [compile_seq_hook_correct] (stratum 8). *)
Theorem compile_seq_flat_verdict_correct : forall c e packets,
  forallb rule_numgen_free (c_rules c) = true ->
  seq_eval_env (run_chain_flat_env h (compile_chain c) (c_policy c)) e packets
  = seq_eval_env (eval_chain_flat_env h c) e packets.
Proof.
  intros c e packets Hall. apply seq_eval_env_ext. intros e' p.
  apply compile_chain_flat_env_correct. exact Hall.
Qed.
...
(** ** Why the mutation strata (2-4) carry [rule_numgen_free] but NO
    ...
    [compile_chain_flat_verdict_correct]/[compile_seq_flat_verdict_correct] DO instantiate on a
    ...
    (2) That bridge EXISTS: the jump-aware mutating evaluator is the unified
        ...
        semantics"), and [Semantics.eval_rules_flat_verdict_proj] /
        [eval_table_flat_verdict_proj] prove the flat mutation strand is its
...
(** (A) the flat mutation strand treats the realised [Jump] as a fall-through:
    ...
    instantiates on it), yet [eval_chain_flat_verdict h] returns the base policy [Accept] —
...
Example mut_strand_jump_pin : forall e p,
  forallb rule_numgen_free (c_rules rg_base) = true /\ eval_chain_flat_verdict h rg_base e p = Accept.
Proof. intros e p. split; reflexivity. Qed.

(** (B) …and the license correctly EXCLUDES it: the jump rule fails
    [rule_plain], so [Semantics.eval_table_flat_verdict_proj] does not certify the
...
Print Assumptions compile_chain_flat_env_correct.
Print Assumptions compile_seq_flat_verdict_correct.
