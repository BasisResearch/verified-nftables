(** * Optimize_Dscpv: masked-payload value+VERDICT->vmap fold for `ip dscp` / `ip6 dscp`.
    ...
    [optimize_rules_vmapN] (which folds an UNGUARDED plain single-selector run of
...
From Nft Require Import Bytes Packet Verdict Syntax Bytecode Semantics
  Optimize Optimize_Merge Optimize_Concat Optimize_Vmap Optimize_Dscp.
...
(** ** The merged transform-keyed VMAP rule.
    ...
    [apply_transforms ts (field_value f p)] instead of the raw [field_value f p]. *)
...
Lemma orig_rule_m_loadable_eq : forall f mask xor v body w p,
  rule_loadable (orig_rule_m f mask xor v body w) p
...
Lemma orig_rule_m_applies : forall f mask xor v body w p,
  rule_applies (orig_rule_m f mask xor v body w) p
  = eval_matchcond (MMasked f false mask xor v) p && rule_applies_walk body p.
Proof. intros. unfold orig_rule_m. apply rule_applies_mk_head. Qed.

Lemma orig_rule_m_outcome_clean : forall f mask xor v body w p,
  body_synproxy_stops body p = false ->
  body_has_notrack body = false ->
  terminal w = true ->
  outcome (orig_rule_m f mask xor v body w) p = Some w.
Proof.
  intros f mask xor v body w p Hsp Hnt Hw.
...
Lemma eval_mmasked_point : forall f mask xor v q,
  field_loadable f q = true ->
  length (data_bitops (field_value f q) mask xor) = length v ->
  eval_matchcond (MMasked f false mask xor v) q
  = data_eqb (data_bitops (field_value f q) mask xor) v.
Proof.
  intros f mask xor v q Hld Hlen.
...
Lemma dscpv_key_width : forall f (mask xor v : data) p,
  field_fixed_len f = Some (length v) ->
  length mask = length v -> length xor = length v ->
  field_loadable f p = true ->
  length (data_bitops (field_value f p) mask xor) = length v.
Proof.
  intros f mask xor v p Hfx Hm Hx Hld.
  pose proof (field_fixed_len_loaded f (length v) p Hfx Hld) as Hfv.
  rewrite (data_bitops_length_eq (field_value f p) mask xor).
  - exact Hfv.
...
Fixpoint first_match_m (f : field) (mask xor : data) (q : packet)
    (l : list (data * verdict)) : option verdict :=
  match l with
  | [] => None
  | (v, w) :: tl => if eval_matchcond (MMasked f false mask xor v) q then Some w
                    else first_match_m f mask xor q tl
  end.

(** [assoc_verdict] over the N-entry point map with the MASKED key = [first_match_m]. *)
Lemma assoc_verdict_points_m : forall es f mask xor q,
  (forall v w, In (v, w) es -> field_loadable f q = true ->
     length (data_bitops (field_value f q) mask xor) = length v) ->
  field_loadable f q = true ->
  assoc_verdict (data_bitops (field_value f q) mask xor) (map vmap_pt es)
  = first_match_m f mask xor q es.
Proof.
  intros es f mask xor q Hlen Hld.
  ...
  rewrite (eval_mmasked_point f mask xor v q Hld (Hlen v w (or_introl eq_refl) Hld)).
  destruct (data_eqb (data_bitops (field_value f q) mask xor) v) eqn:E; [reflexivity|].
...
Lemma outcome_core_dscpvN : forall es f mask xor q nm body,
  e_vmap (pkt_env q) nm = map vmap_pt es ->
  (forall v w, In (v, w) es -> field_loadable f q = true ->
     length (data_bitops (field_value f q) mask xor) = length v) ->
  field_loadable f q = true ->
  outcome_core (mk_vmap_rule_t f [TBitAnd mask xor] nm body) q
  = first_match_m f mask xor q es.
Proof.
  intros es f mask xor q nm body Hvm Hlen Hld.
  ...
  rewrite (assoc_verdict_points_m es f mask xor q Hlen Hld).
  destruct (first_match_m f mask xor q es) eqn:Efm; [reflexivity|].
...
(** ** The N-way masked verdict-map collapse, verdict-preserving.
    ...
    masked key [data_bitops (field_value f p) mask xor]. *)
Lemma eval_rules_dscpv_mergeN : forall f mask xor nm es body rest p,
  ...
  body_synproxy_stops body p = false ->
  body_has_notrack body = false ->
  eval_rules (mk_vmap_rule_t f [TBitAnd mask xor] nm body :: rest) p
  = eval_rules (map (fun vw => orig_rule_m f mask xor (fst vw) body (snd vw)) es ++ rest) p.
Proof.
  intros f mask xor nm es body rest p Hvm Hfx Hmw Hxw Hterm Hsp Hnt.
  (* merged rule loadable / applies *)
  assert (HmL : rule_loadable (mk_vmap_rule_t f [TBitAnd mask xor] nm body) p
  ...
  { unfold rule_loadable, mk_vmap_rule_t. cbn [r_body]. rewrite Hsp.
    ...
    - destruct (assoc_verdict (apply_transforms [TBitAnd mask xor] (field_value f p))
                              (e_vmap (pkt_env p) nm));
        rewrite ?Bool.andb_true_r; reflexivity.
    - rewrite Bool.andb_false_r. reflexivity. }
  assert (HmA : rule_applies (mk_vmap_rule_t f [TBitAnd mask xor] nm body) p
                = rule_applies_walk body p) by reflexivity.
  cbn [eval_rules]. rewrite HmL, HmA.
  destruct (field_loadable f p) eqn:Hfld; cbn [andb].
  - (* f loads: merged outcome = first_match_m; the run scans the same keys *)
    rewrite Bool.andb_true_r.
    assert (Hmout : outcome (mk_vmap_rule_t f [TBitAnd mask xor] nm body) p
                    = first_match_m f mask xor p es).
    { unfold outcome, mk_vmap_rule_t. cbn [r_body]. rewrite Hsp.
      ...
      apply (outcome_core_dscpvN es f mask xor p nm body Hvm
               (fun v w Hin Hld =>
                  dscpv_key_width f mask xor v p (Hfx v w Hin) (Hmw v w Hin) (Hxw v w Hin) Hld)
               Hfld). }
    rewrite Hmout.
      ...
      * (* body loads & applies: induct on es, matching first_match_m to the run *)
        ...
        -- cbn [eval_rules].
           rewrite orig_rule_m_loadable_eq, orig_rule_m_applies,
                   (orig_rule_m_outcome_clean f mask xor v body w p Hsp Hnt
           ...
           destruct (eval_matchcond (MMasked f false mask xor v) p) eqn:Ev.
...
(** ** Executable N-WAY masked-vmap pass (mirrors [Optimize_Vmap.optimize_rules_vmapN]). *)
...
Fixpoint optimize_rules_dscpv (fuel n : nat) (d : set_decls) (rs : list rule)
        ...
        | Some (f, mask, xor, v1, body) =>
            ...
            | ((_ :: _) as es, rest') =>
                if has_distinct_verdict (r_verdict r1) es && body_vmap_safe body then
                  ...
                  let '(n'', d'', rest'') := optimize_rules_dscpv fuel' (S n) d' rest' in
                  (n'', d'', merged :: rest'')
                else
                  let '(n'', d'', rest') := optimize_rules_dscpv fuel' n d rest in
                  (n'', d'', r1 :: rest')
            | ([], _) =>
                let '(n'', d'', rest') := optimize_rules_dscpv fuel' n d rest in
                (n'', d'', r1 :: rest')
...
Definition optimize_chain_dscpv (n : nat) (d : set_decls) (c : chain)
  : nat * set_decls * chain :=
  let '(n', d', rs') := optimize_rules_dscpv (length (c_rules c)) n d (c_rules c) in
  (n', d', {| c_policy := c_policy c; c_rules := rs' |}).

Lemma optimize_rules_dscpv_consSS : forall fuel n d r1 r2 rest,
  optimize_rules_dscpv (S fuel) n d (r1 :: r2 :: rest) =
  match head_dscp r1 with
  | Some (f, mask, xor, v1, body) =>
      ...
      | ((_ :: _) as es, rest') =>
          if has_distinct_verdict (r_verdict r1) es && body_vmap_safe body then
            ...
            let '(n'', d'', rest') := optimize_rules_dscpv fuel n d (r2 :: rest) in
            (n'', d'', r1 :: rest')
      | ([], _) =>
          let '(n'', d'', rest') := optimize_rules_dscpv fuel n d (r2 :: rest) in
          (n'', d'', r1 :: rest')
...
Lemma optimize_rules_dscpv_assoc_stable : forall fuel n d rs n' d' rs' nm X,
...
Lemma optimize_rules_dscpv_mono : forall fuel n d rs n' d' rs',
  optimize_rules_dscpv fuel n d rs = (n', d', rs') -> n <= n'.
Proof.
      ...
      * destruct (take_dscpv_run r1 (r2 :: rest)) as [es rest'] eqn:Erun.
        ...
        -- destruct (has_distinct_verdict (r_verdict r1) (e :: es') && body_vmap_safe body) eqn:Hdv.
           2:{ remember (optimize_rules_dscpv fuel n d (r2 :: rest)) as tt eqn:Erec.
               ...
               eapply (IH n d (r2 :: rest)); symmetry; exact Erec. }
           cbv zeta in H.
           remember (optimize_rules_dscpv fuel (S n) _ rest') as tt eqn:Erec.
           destruct tt as [[m'' dd''] rr'']. cbv zeta in H.
...
Lemma optimize_rules_dscpv_sets : forall fuel n d rs n' d' rs',
  optimize_rules_dscpv fuel n d rs = (n', d', rs') -> sd_sets d' = sd_sets d.
Proof.
  induction fuel as [| fuel IH]; intros n d rs n' d' rs' H.
  - cbn in H. inversion H; subst; reflexivity.
  - destruct rs as [| r1 [| r2 rest] ].
...
Lemma optimize_rules_dscpv_maps : forall fuel n d rs n' d' rs',
  optimize_rules_dscpv fuel n d rs = (n', d', rs') -> sd_maps d' = sd_maps d.
Proof.
  induction fuel as [| fuel IH]; intros n d rs n' d' rs' H.
  - cbn in H. inversion H; subst; reflexivity.
  - destruct rs as [| r1 [| r2 rest] ].
...
Lemma optimize_rules_dscpv_keys_bound : forall fuel n d rs n' d' rs' k,
  optimize_rules_dscpv fuel n d rs = (n', d', rs') ->
        ...
        -- destruct (has_distinct_verdict (r_verdict r1) (e :: es') && body_vmap_safe body) eqn:Hdv.
           ...
           ++ cbn [sd_vmaps map] in Hin_dn. destruct Hin_dn as [Heq | Hin_d].
              ** apply vmapname_inj in Heq. subst k. right.
                 pose proof (optimize_rules_dscpv_mono fuel (S n) _ rest' m'' dd'' rr'' (eq_sym Erec)). lia.
              ** left; exact Hin_d.
           ++ right; exact Hlt.
      * remember (optimize_rules_dscpv fuel n d (r2 :: rest)) as tt eqn:Erec.
        destruct tt as [[m'' dd''] rr'']. cbv zeta in H. injection H as Hn' Hd' Hr'.
...
Lemma optimize_chain_dscpv_mono : forall n d c n' d' c',
  optimize_chain_dscpv n d c = (n', d', c') -> n <= n'.
Proof.
  intros n d c n' d' c' H. unfold optimize_chain_dscpv in H.
  destruct (optimize_rules_dscpv (List.length (c_rules c)) n d (c_rules c))
    as [[m'' dd''] rr''] eqn:E.
  inversion H; subst. apply (optimize_rules_dscpv_mono _ _ _ _ _ _ _ E).
Qed.

Lemma optimize_chain_dscpv_sets : forall n d c n' d' c',
  optimize_chain_dscpv n d c = (n', d', c') -> sd_sets d' = sd_sets d.
Proof.
  intros n d c n' d' c' H. unfold optimize_chain_dscpv in H.
  destruct (optimize_rules_dscpv (List.length (c_rules c)) n d (c_rules c))
    as [[m'' dd''] rr''] eqn:E.
  inversion H; subst. apply (optimize_rules_dscpv_sets _ _ _ _ _ _ _ E).
Qed.

Lemma optimize_chain_dscpv_maps : forall n d c n' d' c',
  optimize_chain_dscpv n d c = (n', d', c') -> sd_maps d' = sd_maps d.
Proof.
  intros n d c n' d' c' H. unfold optimize_chain_dscpv in H.
  destruct (optimize_rules_dscpv (List.length (c_rules c)) n d (c_rules c))
    as [[m'' dd''] rr''] eqn:E.
  inversion H; subst. apply (optimize_rules_dscpv_maps _ _ _ _ _ _ _ E).
Qed.

Lemma optimize_chain_dscpv_keys_bound : forall n d c n' d' c' k,
  optimize_chain_dscpv n d c = (n', d', c') ->
  In (vmapname k) (map fst (sd_vmaps d')) ->
  In (vmapname k) (map fst (sd_vmaps d)) \/ k < n'.
Proof.
  intros n d c n' d' c' k H Hin. unfold optimize_chain_dscpv in H.
  destruct (optimize_rules_dscpv (List.length (c_rules c)) n d (c_rules c))
    as [[m'' dd''] rr''] eqn:E.
  inversion H; subst n' d' c'.
  apply (optimize_rules_dscpv_keys_bound _ _ _ _ _ _ _ k E Hin).
Qed.
...
Example dscpv_folds :
  optimize_rules_dscpv 2 0 {| sd_sets := []; sd_vmaps := []; sd_maps := [] |}
