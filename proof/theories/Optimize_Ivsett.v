(** * Optimize_Ivsett: host-order INTERVAL / range set consolidation (`ct mark`, …).
    ...
    Unlike a plain network-order range `ip saddr a-b` (which [Optimize_Ivset] handles as
    ...
    The [MRangeT] head (transformed range) is what [Optimize_Ivset]'s bare-[MRange]
    ...
    fixed-width side-condition ([data_le] does not truncate).  We reuse
    [eval_rules_run_merge_abs] (Optimize_Merge) VERBATIM.
    ...
    [Optimize_Ivsetg]) — otherwise the object is unloadable (the `nft --optimize` defect
...
From Nft Require Import Bytes Packet Verdict Syntax Bytecode Semantics
  Optimize Optimize_Merge Optimize_Ivset Optimize_Ivsetg.
...
Fixpoint optimize_rules_ivsett (fuel : nat) (n : nat) (d : set_decls) (rs : list rule)
  ...
  | S fuel' =>
    ...
    | r1 :: ((_ :: _) as rest) =>
        ...
        | Some (f, ts, lo1, hi1, body) =>
            ...
            | ([], _) =>
                let '(n'', d'', rest') := optimize_rules_ivsett fuel' n d rest in
                (n'', d'', r1 :: rest')
            | ((_ :: _) as ivs, rest') =>
                ...
                let '(n'', d'', rest'') := optimize_rules_ivsett fuel' (S n) d' rest' in
                (n'', d'', merged :: rest'')
            end
        | None =>
            let '(n'', d'', rest') := optimize_rules_ivsett fuel' n d rest in
...
Definition optimize_chain_ivsett (n : nat) (d : set_decls) (c : chain)
  : nat * set_decls * chain :=
  let '(n', d', rs') := optimize_rules_ivsett (length (c_rules c)) n d (c_rules c) in
  (n', d', {| c_policy := c_policy c; c_rules := rs' |}).

Lemma optimize_rules_ivsett_consSS : forall fuel n d r1 r2 rest,
  optimize_rules_ivsett (S fuel) n d (r1 :: r2 :: rest) =
  match head_ivsett r1 with
  | Some (f, ts, lo1, hi1, body) =>
      match take_ivsett_run r1 (r2 :: rest) with
      | ([], _) =>
          let '(n'', d'', rest') := optimize_rules_ivsett fuel n d (r2 :: rest) in
          (n'', d'', r1 :: rest')
      | ((_ :: _) as ivs, rest') =>
          let name := setname n in
          let elems := (lo1, hi1) :: ivs in
...
Lemma optimize_rules_ivsett_mono : forall fuel n d rs n' d' rs',
  optimize_rules_ivsett fuel n d rs = (n', d', rs') -> n <= n'.
Proof.
  ...
  - destruct rs as [| r1 [| r2 rest] ].
    ...
    + rewrite optimize_rules_ivsett_consSS in H.
      ...
      * destruct (take_ivsett_run r1 (r2 :: rest)) as [ivs rest'] eqn:Erun.
        ...
        -- remember (optimize_rules_ivsett fuel n d (r2 :: rest)) as tt eqn:Erec.
           destruct tt as [[m'' dd''] rr'']. cbv zeta in H. inversion H; subst.
           eapply (IH n d (r2 :: rest)); symmetry; exact Erec.
        -- cbv zeta in H.
           remember (optimize_rules_ivsett fuel (S n) _ rest') as tt eqn:Erec.
...
Lemma optimize_rules_ivsett_vmaps : forall fuel n d rs n' d' rs',
  optimize_rules_ivsett fuel n d rs = (n', d', rs') -> sd_vmaps d' = sd_vmaps d.
Proof.
  induction fuel as [| fuel IH]; intros n d rs n' d' rs' H.
  - cbn in H. inversion H; subst; reflexivity.
  - destruct rs as [| r1 [| r2 rest] ].
...
Lemma optimize_rules_ivsett_maps : forall fuel n d rs n' d' rs',
  optimize_rules_ivsett fuel n d rs = (n', d', rs') -> sd_maps d' = sd_maps d.
Proof.
  induction fuel as [| fuel IH]; intros n d rs n' d' rs' H.
  - cbn in H. inversion H; subst; reflexivity.
  - destruct rs as [| r1 [| r2 rest] ].
...
Lemma optimize_rules_ivsett_keys_bound : forall fuel n d rs n' d' rs' k,
        ...
        -- cbv zeta in H.
           ...
           ++ cbn [sd_sets map] in Hin_dn. destruct Hin_dn as [Heq | Hin_d].
              ** apply setname_inj in Heq. subst k. right.
                 pose proof (optimize_rules_ivsett_mono fuel (S n) _ rest' m'' dd'' rr'' (eq_sym Erec)). lia.
              ** left; exact Hin_d.
           ++ right; exact Hlt.
      * remember (optimize_rules_ivsett fuel n d (r2 :: rest)) as tt eqn:Erec.
        destruct tt as [[m'' dd''] rr'']. cbv zeta in H. injection H as Hn' Hd' Hr'.
...
Lemma optimize_rules_ivsett_assoc_stable : forall fuel n d rs n' d' rs' nm X,
  optimize_rules_ivsett fuel n d rs = (n', d', rs') ->
...
Lemma optimize_chain_ivsett_mono : forall n d c n' d' c',
  optimize_chain_ivsett n d c = (n', d', c') -> n <= n'.
Proof.
  intros n d c n' d' c' H. unfold optimize_chain_ivsett in H.
  destruct (optimize_rules_ivsett (length (c_rules c)) n d (c_rules c))
    as [[m'' dd''] rr''] eqn:E.
  inversion H; subst. apply (optimize_rules_ivsett_mono _ _ _ _ _ _ _ E).
Qed.

Lemma optimize_chain_ivsett_vmaps : forall n d c n' d' c',
  optimize_chain_ivsett n d c = (n', d', c') -> sd_vmaps d' = sd_vmaps d.
Proof.
  intros n d c n' d' c' H. unfold optimize_chain_ivsett in H.
  destruct (optimize_rules_ivsett (length (c_rules c)) n d (c_rules c))
    as [[m'' dd''] rr''] eqn:E.
  inversion H; subst. apply (optimize_rules_ivsett_vmaps _ _ _ _ _ _ _ E).
Qed.

Lemma optimize_chain_ivsett_maps : forall n d c n' d' c',
  optimize_chain_ivsett n d c = (n', d', c') -> sd_maps d' = sd_maps d.
Proof.
  intros n d c n' d' c' H. unfold optimize_chain_ivsett in H.
  destruct (optimize_rules_ivsett (length (c_rules c)) n d (c_rules c))
    as [[m'' dd''] rr''] eqn:E.
  inversion H; subst. apply (optimize_rules_ivsett_maps _ _ _ _ _ _ _ E).
Qed.

Lemma optimize_chain_ivsett_keys_bound : forall n d c n' d' c' k,
  optimize_chain_ivsett n d c = (n', d', c') ->
  In (setname k) (map fst (sd_sets d')) ->
  In (setname k) (map fst (sd_sets d)) \/ k < n'.
Proof.
  intros n d c n' d' c' k H Hin. unfold optimize_chain_ivsett in H.
  destruct (optimize_rules_ivsett (length (c_rules c)) n d (c_rules c))
    as [[m'' dd''] rr''] eqn:E.
...
Example ivsett_folds :
  optimize_rules_ivsett 2 0 {| sd_sets := []; sd_vmaps := []; sd_maps := [] |}
