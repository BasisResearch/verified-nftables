...
Lemma e_vmap_map0 : forall e, e = gen_env ->
...
Definition m_icmpproto (e : env) (p : packet) : bool := eval_matchcond_body (MEq FMetaL4proto [1]) e p.
Definition m_icmpecho (e : env) (p : packet) : bool := eval_matchcond_body (MEq FIcmpType [8]) e p.
...
Definition icmp_under (e : env) (p : packet) : bool := eval_matchcond_body (MLimit icmp_spec) e p.

Lemma icmp_under_lim : forall e p, icmp_under e p = lim_under e p icmp_spec.
Proof. reflexivity. Qed.

(** [icmp_ok]: the icmp rule fires (accepts) iff it is an icmp echo-request AND the
    rate limit is not yet exhausted. *)
Definition icmp_ok (e : env) (p : packet) : bool :=
  m_icmpproto e p && m_icmpecho e p && icmp_under e p.

(** The concat-vmap key the parser builds for rule 2: [ip protocol] ++ [th dport]. *)
Definition svc_key (e : env) (p : packet) : data :=
...
Definition svc_hit (e : env) (p : packet) : option verdict :=
  assoc_verdict (svc_key e p) map0.
...
Proof.
  intros e p. unfold svc_hit, map0. set (k := svc_key e p). cbn [assoc_verdict].
...
Lemma inbound_eth1_eval : forall e p,
  e = gen_env ->
...
Lemma svc_hit_iff : forall e p,
  (svc_hit e p = Some Accept) <->
  ( svc_key e p = [6;0;22] \/ svc_key e p = [17;0;53]
    \/ svc_key e p = [6;0;53] \/ svc_key e p = [17;0;67] ).
Proof.
  intros e p.
  assert (Hexp : svc_hit e p =
    (if data_eqb [6;0;22]  (svc_key e p) then Some Accept
     else if data_eqb [17;0;53] (svc_key e p) then Some Accept
     else if data_eqb [6;0;53]  (svc_key e p) then Some Accept
     else if data_eqb [17;0;67] (svc_key e p) then Some Accept
     else None)).
  { unfold svc_hit, map0. cbn [assoc_verdict]. now rewrite !data_in_iv_point. }
  rewrite Hexp. split.
  - intro H.
    destruct (data_eqb [6;0;22] (svc_key e p)) eqn:E1.
    { left. apply data_eqb_true_iff in E1. now rewrite E1. }
    destruct (data_eqb [17;0;53] (svc_key e p)) eqn:E2.
    { right; left. apply data_eqb_true_iff in E2. now rewrite E2. }
    destruct (data_eqb [6;0;53] (svc_key e p)) eqn:E3.
    { right; right; left. apply data_eqb_true_iff in E3. now rewrite E3. }
    destruct (data_eqb [17;0;67] (svc_key e p)) eqn:E4.
...
(** THE CRUX (mirrors [forward_accept_iff]): for a NEW-connection packet ingressing
    ...
    the box. *)
Theorem inbound_eth1_accept_iff : forall e p,
  e = gen_env ->
  field_loadable FCtState p = true ->
  field_loadable FMetaIifname p = true ->
  world_loads p = true ->
...
Theorem inbound_eth1_unlisted_dropped : forall e p,
  e = gen_env ->
  field_loadable FCtState p = true ->
  field_loadable FMetaIifname p = true ->
  world_loads p = true ->
...
Theorem inbound_icmp_ratelimited_dropped : forall e p,
  e = gen_env ->
  field_loadable FCtState p = true ->
  field_loadable FMetaIifname p = true ->
  world_loads p = true ->
...
Theorem inbound_eth1_service_accept : forall e p,
  e = gen_env ->
  field_loadable FCtState p = true ->
  field_loadable FMetaIifname p = true ->
  world_loads p = true ->
...
Lemma pkt_lan_dns_facts :
  ...
  field_value FCtState env_lan pkt_lan_dns = cts_new /\
  field_value FMetaIifname env_lan pkt_lan_dns = if_eth1 /\
  svc_key env_lan pkt_lan_dns = [17;0;53].
...
Lemma pkt_lan_smtp_facts :
  ...
  field_value FCtState env_lan pkt_lan_smtp = cts_new /\
  field_value FMetaIifname env_lan pkt_lan_smtp = if_eth1 /\
