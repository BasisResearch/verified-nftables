...
- ✅ **(2) Control flow** (C): `jump`/`goto`/`return` + user chains
  ...
  `compile_hook_u_correct` / `compile_seq_hook_u_correct`): a jumped-to
...
| `compile_table_u_correct` / `compile_ruleset_u_correct` / `compile_hook_u_correct` / `compile_seq_hook_u_correct` | **THE UNIFIED SEMANTICS** (stratum 8): mutation × jump/goto/return × multi-chain × hook dispatch × cross-packet env carry, jointly — one effect-threading, jump-following evaluator per side (`eval_rules_u`/`run_rules_u`), verdict AND left state preserved |
